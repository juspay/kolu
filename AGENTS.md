...
- **DO NOT add flake inputs** to `flake.nix`. Each input adds ~1.5s to `nix develop` cold start. The flake intentionally has zero inputs — nixpkgs and other sources are managed by [nixtamal](https://nixtamal.toast.al/) and imported via `fetchTarball`. Use `nixtamal refresh && nixtamal lock` (from `nix/tamal/`) to update sources.
