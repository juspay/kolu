...
The debate reasoned over TWO consumers (kolu, drishti). **odu (github.com/juspay/odu) is a THIRD** — it imports `getHostSession`, `HostSessionState`, and `destroyAllSessions` from `@kolu/surface-nix-host` (`src/coordinator/lane.ts`, `run.ts`). This STRENGTHENS every prove-then-extract call (three live consumers) and changes three specifics: `getHostSession` stays public (returns `Session`, optional `admit?`); `destroyAllSessions` is kept; `HostSessionState`→`SessionState` moves in odu's PR. **The change lands as THREE paired PRs (kolu · drishti · odu), all CI-green before any merge.** OPEN design point for implementation: kolu's remote-padi arm adds `admit` via `getHostSession({..., admit})` (preferred — one ssh entry) vs hand-composing `makeSession + sshConnector` (rejected unless the option shape proves awkward).
