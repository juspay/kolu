...
Then('the header should contain {string}', async function (this: KoluWorld, text: string) {
  const header = this.page.locator('header');
  await header.waitFor({ state: 'visible' });
  const content = await header.textContent();
  assert.ok(content?.includes(text), `Header "${content}" does not contain "${text}"`);
});

Then('the response should be {string}', async function (this: KoluWorld, expected: string) {
