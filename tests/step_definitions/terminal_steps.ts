...
Then('the canvas should fill at least {int}% of its container', async function (this: KoluWorld, pct: number) {
  ...
  const ratio = pct / 100;
  assert.ok(canvas.width > container.width * ratio,
    `Canvas width ${canvas.width} < ${ratio * 100}% of container ${container.width}`);
  assert.ok(canvas.height > container.height * ratio,
    `Canvas height ${canvas.height} < ${ratio * 100}% of container ${container.height}`);
});

// ── Font size assertions ──

Then('the font size should be larger than before', async function (this: KoluWorld) {
  const current = await this.fontSize();
  assert.ok(this.savedFontSize !== undefined, 'No saved font size');
  assert.ok(current > this.savedFontSize!, `Font size ${current} not larger than ${this.savedFontSize}`);
});

Then('the font size should be smaller than the original', async function (this: KoluWorld) {
  const current = await this.fontSize();
  assert.ok(this.savedFontSize !== undefined, 'No saved font size');
  assert.ok(current < this.savedFontSize!, `Font size ${current} not smaller than ${this.savedFontSize}`);
});
...
Then('no raw keystroke {string} {string} {string} should have been sent via WebSocket', async function (
  this: KoluWorld, k1: string, k2: string, k3: string
) {
  const messages: string[] = await this.page.evaluate(() => (window as any).__wsSent);
...
Then('a Resize message with cols greater than {int} should have been sent', async function (
  this: KoluWorld, minCols: number
) {
  const messages: string[] = await this.page.evaluate(() => (window as any).__wsSent);
  ...
  assert.ok(parsed.cols > minCols, `Resize cols ${parsed.cols} not greater than ${minCols}`);
});
