...
- A deferral is a scheduling decision, not an escape hatch: every "defer" ruling names its owner, its trigger, AND a venue on which the trigger is satisfiable today — a gate that cannot currently be met is "someday" wearing a condition, and "someday" is banned. (A follow-up's e2e gate was once pointed at a downed host pool; the standard suite venue existed all along.)
- "Equivalent to current behavior" is a valid done-criterion only when current behavior is itself verified correct. Where two implementations of one fact can disagree, the criterion is ground truth against the named authority — never equivalence with the suspect copy. (An equivalence-under-zoom criterion nearly enshrined a hand-rolled tap divisor over xterm's own cell mapping.)
