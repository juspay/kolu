...
**The sync is not a sub-step of CI, so nothing that cancels CI cancels it.** It is
about where the branch's final `HEAD` sits, not about what the pipeline happens to
see — so it still runs when CI is skipped by the invocation, blocked by a venue
outage, or already spent on an earlier SHA, and it runs *again* on any later return
to a still-open PR. Reading it as "step one of running CI" is how the interjection
this paragraph exists to pre-empt got typed **twice in one session**, on a run whose
pipeline never went green. Picking a `/be` run back up on an existing PR: fetch and
merge master before anything else.
