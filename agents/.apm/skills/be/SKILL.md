...
**An interruption is a detour, not an exit — re-enter the phase you left, unasked.** A `[Request interrupted by user]`, a bug report that pulls you off-piste, a session/rate limit, a `/compact` — each *suspends* the phase machine; none of them ends the run, and putting it back is yours, not the user's. So the moment the detour is handled, name the phase you were in and resume it **in the same turn**. A step that was in flight when the detour hit did **not** run: restart that step (`/be-review --tracks` re-enters one reviewer) and never count it as done — reporting a phase you began but didn't finish is the same defect as skipping it. Never close a turn on "let me know if you want me to continue"; that is the §0 interview reopening under a different name, and the user typing "continue with /be" is the intervention this clause exists to delete. [recorded: a §4 gauntlet interrupted to chase a field bug was never restarted, and the run went on to describe the diff as hickey-reviewed — the correction was a flat "I never ran hickey." The same run then took "continue with /be" three times in seventy seconds, a "Did you finish /be workflow?", and finally a hand-installed session Stop hook before §5 would finish.]
...
**These three branches key to the change in hand, not to the phase you're in.** A bug that surfaces *later* in the run — the human deploys the in-flight PR mid-§4 and reports what they see, a gauntlet commit breaks something, CI reds on behavior rather than tooling — re-enters the **Bug** bullet's (1)–(4) **from the top**, red repro included, before any fix commit and before any push. "We're already mid-gauntlet", "CI is burning", "the diff is fresh so the cause is obvious" are never grounds to fix-and-push straight: a fresh diff is exactly when a guess feels most like knowledge, because you recognize the code the symptom points at and stop looking for the mechanism. [recorded: a run answered a field bug by repainting a terminal from a single sampled state value, pushed it, and had to `git revert` the whole commit once the user checked the live process — the agent had been working the entire time. Two further "still buggy" rounds followed, each shipped without a red repro, and the user had to demand repro-before-fix three separate times.]
...
**Heavy work runs on a pu box, never locally — a kolu is live on this machine.**
Builds, the dev server, and evidence capture all go on an ephemeral pu box
whenever **any** kolu is running here (the normal case). A prior run piled local
`just dev-auto` + nix builds beside a live kolu and the **OOM-killer
`SIGKILL`ed** it; random ports dodged its *ports* but not its *RAM*. Load
**`/dev-server`** §0 for the local-vs-pu venue gate before launching the app for
*any* reason — including an interactive "let me SEE it" check during §2 — and use
**its** probe, not `systemctl --user is-active kolu`, which is blind to a `just
dev` kolu and reports `inactive` even where no `kolu.service` unit exists.
`/ci` and `/evidence` already run on pu; keep it that way.
...
green before capturing. **Concretely: the evidence agent launches BEFORE your
first blocking `wait_for_settle`, not after it.** `odu run` returns immediately;
`wait_for_settle` is where the turn parks, so any evidence call placed after it is
serial no matter what you intended. The order is `odu run` → launch `/evidence` →
*then* `wait_for_settle`. [recorded: a run announced "kicking off evidence
concurrently:" and in the very next tool call blocked on `wait_for_settle` for 386s
before launching evidence — announcing concurrency is not achieving it, and the
6.4 minutes were lost to instruction *order*, not to intent.]
