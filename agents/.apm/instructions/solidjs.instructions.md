...
- **Unified state stream**: All server state (terminals, preferences, session, repos) flows through one `state.get` live query (`experimental_liveOptions`). Derive slices with `createMemo`. Optimistic updates use `setQueryData` on the unified query key. Activity sparklines stay as separate `experimental_streamedOptions` queries (high-frequency accumulating data that doesn't fit the entity model).
