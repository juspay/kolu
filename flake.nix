      ...
      devShells = eachSystem (pkgs: {
        default = pkgs.mkShell {
          ...
          shellHook = ''
            ...
            export KOLU_THEMES_JSON="${self.packages.${pkgs.system}.ghosttyThemes}/themes.json"
            export KOLU_FONTS_DIR="${self.packages.${pkgs.system}.fonts}"
            if root=$(git rev-parse --show-toplevel 2>/dev/null); then
              ln -sfn "$KOLU_FONTS_DIR" "$root/client/public/fonts"
            fi
            export KOLU_CLIPBOARD_SHIM_DIR="${self.packages.${pkgs.system}.clipboard-shims}/bin"
