...
**How it works:** when the foreground process is `codex`, the provider queries Codex's SQLite database at `~/.codex/state_5.sqlite` to find the most recently updated thread whose `cwd` matches the terminal's CWD. State is derived from the thread's JSONL rollout transcript (path stored in the `rollout_path` column): a `task_started` event without a matching `task_complete` means the agent is active; unresolved `function_call` entries (no corresponding `function_call_output`) indicate _tool use_; a `task_complete` event means _waiting_. The tile chrome shows the running token count from `token_count` events in the rollout. Live updates come from `fs.watch` on both the rollout JSONL file and the SQLite WAL file (`state_5.sqlite-wal`), plus a `subscribeExternalChanges` hook that re-resolves when new threads appear in the DB.

**What we detect:**

| State    | Indicator          | How                                                                         |
| -------- | ------------------ | --------------------------------------------------------------------------- |
| Thinking | Pulsing accent dot | `task_started` seen, no `task_complete` for that turn, no active tool calls |
| Tool use | Spinning yellow    | Thinking + unresolved `function_call` entries in the rollout                |
| Waiting  | Dim dot            | `task_complete` seen for the latest `task_started` turn                     |

**What we can't detect (yet):**

- **Same-directory disambiguation** — if multiple Codex threads share a working directory, we pick the most recently updated one
- **Non-default home location** — set `KOLU_CODEX_HOME` to override `~/.codex`
- **Task progress** — Codex doesn't expose a structured todo list in its local data
...
| `packages/integrations/codex/`       | Codex detection — reads Codex's SQLite database + JSONL rollout transcripts; exports a `codexProvider` `AgentProvider`                             |
