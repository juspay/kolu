...
| Running in background | Spinning working ring | Claude ended its turn while a [dynamic workflow](https://code.claude.com/docs/en/workflows) it launched (a backed run — it carries a `Run ID`) is still running — it is busy-waiting on that workflow, not awaiting you, so it buckets as *working* rather than *awaiting* and never falsely pings. A detached `Bash` command or background `Task`/`Agent` is *not* enough: its launch marker is permanent in the transcript and its completion notification can be lost to a restart, so promoting on it would spin the ring forever — those settle as *waiting* once the turn ends |
| Waiting               | Dim dot             | Claude finished responding — or the turn was interrupted with Esc — and is idle, waiting for user input                                                              |

**How it works:** asks each terminal for its current foreground process pid via `tcgetpgrp(fd)` (exposed by node-pty's `foregroundPid` accessor), then checks whether `~/.claude/sessions/<fgpid>.json` exists. If it does, that terminal is running claude-code — we tail the session's JSONL transcript to derive state from the last message. The tail is also scanned for background tasks the agent launched — a dynamic `Workflow`, a backgrounded `Bash` command, or a background `Task`/`Agent` — that haven't yet reported a terminal status via a `queue-operation` completion; when an outstanding task is a `Workflow` (it carries a `Run ID`, an observable backed run), a bare end-of-turn is promoted from *waiting* to *running in background* so a busy-waiting agent doesn't read as needing you. A bare backgrounded `Bash`/`Task`/`Agent` (no `Run ID`) is deliberately *not* promoted — its launch marker outlives the process and its completion notification can be lost forever to a restart, which would otherwise spin the ring indefinitely (the phantom *running in background* bug). An interrupted turn (Esc) appends a trailing `user` entry carrying an explicit interrupt marker (`[Request interrupted by user]`, or an errored `tool_result` for a mid-tool-call Esc); that marker reads as *waiting* (idle), not *thinking* — so the dock settles instead of animating a phantom spinner that persists across `claude -c`. Cross-platform (Linux + macOS) since `tcgetpgrp` is POSIX. Each card also surfaces the session's display title (custom title › auto-generated summary › first prompt) via the [Claude Agent SDK](https://platform.claude.com/docs/en/api/agent-sdk/typescript)'s `getSessionInfo()`, refreshed best-effort on each transcript change. The tile chrome also shows a running token count (compact, e.g. `47K`) summed from the latest assistant entry's `message.usage` — `input_tokens + cache_creation_input_tokens + cache_read_input_tokens`. Raw count only; window size isn't inferable from the JSONL (1M beta strips its suffix, so a `%` would lie), and the raw number is the useful signal anyway.
