...
| `packages/pty-host/`                 | The multi-client PTY-owner primitive + its wire contract — [node-pty](https://github.com/microsoft/node-pty) + a headless screen mirror via [`@kolu/solid-xterm/headless`](packages/solid-xterm/) + VT-derived taps (OSC 7 cwd, OSC 0/2 title, OSC 633 command-run, foreground, exit), each fanned out to many consumers via a bounded `Channel`. Owns *only* the PTY; race-free `attach` (snapshot+deltas). `ptyHostSurface` is the typed contract for consuming it — in-process today, the identical shape rides a socket / ssh later. Imports no `@xterm/*` directly |
| `packages/solid-xterm/`              | **The only package that imports [xterm.js](https://xtermjs.org/) (`@xterm/*`) at runtime** — the terminal "electricity grid". Browser socket `createXterm` (Terminal + 8 addons, WebGL-context lifecycle, scroll-lock, touch-scroll, buffer probes, disposal ordering) and server socket `createHeadlessMirror` (`./headless`, the no-render mirror pty-host uses for late-join snapshots). Consumers inject domain via callbacks (attach, sendInput, resize, file-ref matching, key policy, upload) and never touch xterm; new xterm work lands here. `terminal-themes` keeps a type-only `ITheme` import — the lone exception, and only a type |
...
| `packages/client/`                   | [SolidJS](https://www.solidjs.com/) + [Tailwind CSS v4](https://tailwindcss.com/); renders terminals through [`@kolu/solid-xterm`](packages/solid-xterm/)'s `createXterm` (no direct `@xterm/*` dependency)                                                  |
...
flowchart TB
  subgraph Client["Client (SolidJS)"]
    User((User)):::user
    Xterm["@kolu/solid-xterm\n(xterm.js render + input)"]:::client
  ...
  subgraph Server["Server (Hono)"]
    subgraph PtyHost["@kolu/pty-host (in-process today; socket / ssh later — same ptyHostSurface contract)"]
      Host["node-pty + headless mirror\n(@kolu/solid-xterm) + VT taps"]:::server
    end
...
**Terminal I/O** (solid lines) — keystrokes go through `sendInput` RPC to the PTY owned by [`@kolu/pty-host`](packages/pty-host/), which kolu-server drives over the `ptyHostSurface` contract; shell output flows back as an `attach` stream — a screen-state snapshot followed by the live delta stream, partitioned race-free at a single point so nothing is lost or double-painted — to the browser's xterm.js terminal, which is created and owned by [`@kolu/solid-xterm`](packages/solid-xterm/)'s `createXterm` (the client imports no `@xterm/*` directly). The host's headless xterm mirror — from the same package's `./headless` entry point — parses VT sequences server-side for those snapshots[^lazy-attach], and fans the output out to every attached client through a bounded `Channel`.
