...
| Live query         | each push replaces previous value | `experimental_liveOptions`     | Unified server state (preferences, session, terminal list with metadata) |
...
[^rr]: Mutations update the TanStack cache on success — optimistically via `qc.setQueryData` (reorder, theme, create, kill). The terminal list and metadata are part of the unified `state.get` live query pushed by the server on any state change. A few high-frequency calls (`sendInput`, `resize`) bypass TanStack and use the raw oRPC client directly.
...
flowchart TB
  ...
  Providers -.->|"unified state stream\n(live query)"| TQ
  Pub -.->|"activity stream\n(streamed query)"| TQ
  TQ -.-> UI
...
**User actions** — command palette and sidebar dispatch mutations ([`useTerminalCrud`](client/src/useTerminalCrud.ts), [`useWorktreeOps`](client/src/useWorktreeOps.ts)) via `createMutation` + `orpc.*.mutationOptions()`. Mutations write optimistically to the TanStack cache for instant UI; the server's live push arrives moments later with authoritative data. A single `state.get` live query delivers the terminal list with embedded metadata — [`useTerminalMetadata`](client/src/useTerminalMetadata.ts) derives IDs, ordering, and display info from this unified stream. Per-terminal activity queries remain separate for sparkline rendering[^client-state].
