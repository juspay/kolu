...
| `packages/server/`                   | [Hono](https://hono.dev/); runs the per-terminal provider DAG + terminal lifecycle, and is the unix-socket client of the `kolu --stdio` PTY-host daemon it spawns                     |
| `packages/pty-host/`                 | The multi-client PTY-owner primitive — [node-pty](https://github.com/microsoft/node-pty) + [@xterm/headless](https://www.npmjs.com/package/@xterm/headless) screen mirror + VT-derived taps (OSC 7 cwd, OSC 0/2 title, OSC 633 command-run, exit, foreground), each fanned out to many consumers via a bounded `Channel`. Owns *only* the PTY; race-free `attach` (snapshot+deltas). Since [#951 R4c](docs/plans/remote-terminals.html) it runs as a long-lived `kolu --stdio` daemon that **survives a kolu-server restart** (so terminals persist across a deploy, replacing tmux/zmx — [#671](https://github.com/juspay/kolu/issues/671)); kolu-server is its socket client |
...
flowchart TB
  ...
  subgraph Daemon["kolu --stdio PTY-host daemon (survives a kolu-server restart; ssh-remote in a later phase)"]
    Host["@kolu/pty-host\nnode-pty + @xterm/headless mirror"]:::server
  end

  subgraph Server["Server (Hono) — restarts every deploy"]
    Providers["Provider DAG\ngit · PR · agent-detect"]:::server
  ...
  Backend -->|"write / attach\n(over unix socket)"| Host
  Host -->|"attach stream\n(snapshot + deltas)"| Xterm

  %% Metadata loop
  Host -.->|"raw taps over socket\n(cwd · title · cmd · foreground · exit)"| Backend
  Backend -.->|"feeds channels"| Providers
  Providers -.->|"terminalMetadata · activity"| Backend
  ...
  style Daemon fill:none,stroke:#e9c46a,stroke-width:1px,color:#264653
```
...
**Metadata** (dashed lines) — every per-terminal observer lives in the provider DAG at [`packages/server/src/terminalBackend/providers.ts`](packages/server/src/terminalBackend/providers.ts), parameterized over `ProviderHooks` + `ProviderChannels` so the host is the only thing that varies. Since [#951 R4c](docs/plans/remote-terminals.html) the DAG runs in **kolu-server** against the `kolu --stdio` PTY-host daemon's raw VT taps streamed over the socket: [`LocalTerminalBackend`](packages/server/src/terminalBackend/local.ts) — the concrete `TerminalBackend` for "this kolu process" — bridges each daemon tap (cwd · title · command-run · foreground) onto a fresh per-terminal `ProviderChannel`, constructs a `ProviderRecord`, wires `ProviderHooks` that write onto the `terminalMetadata` collection, and calls `startProviders`. The `updateServerMetadata` / `updateServerLiveMetadata` fence fires the `terminals:dirty` autosave trigger only for the persisted half (so the server-persisted-vs-live distinction governs the autosave firehose). Keeping the volatile DAG in kolu-server — fresh on every deploy — while *only* the thin pty-host survives a restart is what makes a surviving daemon safe: it can never serve stale detection (the [R4c redo](docs/plans/remote-terminals.html), correcting the dropped first attempt). Co-locating the providers with the PTY — where this host's filesystem, process table, and agent-state files are — is what lets a future ssh agent reuse them unchanged; `Local`- and `RemoteTerminalBackend` then differ only in transport. The DAG itself: CWD changes (OSC 7) → git provider (`.git/HEAD` watcher) → GitHub provider (`gh pr view` polling). Agent detection uses a single generic orchestrator (`startAgentProvider` in `providers.ts`) driven by per-agent `AgentProvider` instances from each integration package. Today three instances are registered: `claudeCodeProvider` (from `kolu-claude-code`) wakes on title events (OSC 2) and its own `fs.watch` on `~/.claude/sessions/`; `codexProvider` (from `kolu-codex`) queries the highest-numbered `~/.codex/state_<N>.sqlite` for thread metadata and tails the matched rollout JSONL for state transitions; `opencodeProvider` (from `kolu-opencode`) queries OpenCode's SQLite database directly and watches its WAL file for live state updates. Adding a new agent CLI is one new `AgentProvider` and one line in `startProviders` — no server-side adapter file. All providers publish onto the `terminalMetadata` collection, which the client subscribes to[^providers]. Separately, kolu's preexec hook emits an `OSC 633;E` command mark before each user command; `@kolu/pty-host` surfaces it on its command-run tap, kolu-server bridges that (over the socket) onto a per-terminal `commandRun` channel, and the agent-command tracker (in `providers.ts`) subscribes to match the first token against a known-agents allowlist and fan out to both (a) a `recentAgent` signal feeding kolu-server's bounded recent-agents MRU for the agent-aware command palette and (b) a per-terminal stash keyed by terminal id, so codex/opencode session detection still matches when the agent is an interpreter shim (e.g. npm-installed `codex`, whose kernel-level process name is `node`). No `/proc` lookups or argv scraping. The `TerminalBackend` interface itself lives in [`kolu-common/terminalBackend`](packages/common/src/terminalBackend.ts) — `LocalTerminalBackend` is one concrete shape; a future `RemoteTerminalBackend` (for terminals running on SSH hosts) is the other, and every call site downstream goes through `getTerminalBackendFor(location)` so neither asks "which kind?".
...
**Local PTY daemon** — local terminals run in a long-lived `kolu --stdio` PTY-host daemon (one per `$KOLU_STATE_DIR`, on `pty-host.sock`), so they **survive a kolu-server restart**: after a deploy the new server reattaches to the daemon's PTYs by id (`reattachLocalTerminals` at boot), restores scrollback, and runs a fresh provider DAG against them. On a systemd service the supervisor spawns the daemon via `systemd-run --user --unit=kolu-pty-host` so it lands in its own cgroup and isn't killed by `systemctl --user restart kolu`; on macOS launchd a detached spawn already survives. This needs **linger** enabled (`loginctl enable-linger $USER`, or `users.users.<name>.linger = true` on NixOS) so the user manager keeps the daemon alive between deploys. A pty-host build mismatch after a deploy is detected on a build-identity handshake (not the static `pkgVersion`) and logged as "update pending" — the user-facing nudge + a restart command are a follow-up; an incompatible wire-contract forces a one-time restart. tmux/zmx are not used — see [#671](https://github.com/juspay/kolu/issues/671).
