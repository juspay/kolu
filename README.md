...
- Workspace-switcher peek — each expanded switcher card surfaces the most-recent assistant utterance or active tool call (server-derived from the Claude Code JSONL transcript), so you can scan "what is each agent doing right now" without focusing the terminal. Cards in the **Awaiting you** column render an inline reply input that forwards your text straight to the agent's PTY — handle a permission prompt or follow-up question without leaving the switcher
- Launch in background — a parallel command palette group mirrors **New terminal** but creates the worktree without stealing focus; the new tile lands on the canvas while your current session keeps the keyboard. Use it to dispatch a long-running agent and stay where you are
- Ready-for-review filter — a `PR <count>` chip in the switcher header narrows the visible cards to terminals whose worktree has an open pull request, regardless of agent state. Composes with the repo facet sidebar — a quick triage when you want the list of branches ready to merge
...
- **Auto-park stale terminals** — when a terminal's last observed agent transition is more than 4 hours old, the switcher routes it into the `Idle` column (regardless of its prior agent state), the collapsed pill fades and drops its agent-state border, and the canvas tile fades too — so a wall of "awaiting" tiles you parked yesterday doesn't drown out the one that genuinely needs you now. Idle terminals also stop counting toward the awaiting bucket and are dropped from the OS/PWA dock badge. Any fresh agent transition unparks automatically — pure derivation off `lastActivityAt`, no persisted state
