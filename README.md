...
- **Dock — two-level navigator** — the left-edge dock is the canonical live-terminal surface, with two progressive levels of detail (#903):
  ...
  - **Cards** (default) — rows grouped by repo. Each repo gets a small section header (uppercase name + repo-color swatch + row count); rows below stack as `dot · branch · pips · time` lines. The dot color carries bucket state — awaiting (red, breathing), working (amber, pulsing), idle (green, flat), none (grey, flat). Pips carry presence-only signal — PR state icon, agent-kind logo, sub-terminal chip — so the row scans as "has a PR, has an agent" without any labels or numbers. The active row gets a quiet highlight (lifted surface + accent left-edge stripe); row geometry stays constant so the dock never reflows when the active terminal changes.
