...
| Awaiting input        | Pulsing alert dot   | Claude called `AskUserQuestion` or `ExitPlanMode` and is blocked on your answer — surfaced via an auto-installed `PreToolUse`/`PostToolUse` hook side-channel (#905), since the SDK keeps the `tool_use` block off-disk during the wait. Hover the indicator (or open the inspector) to see the pending question and options |
| Waiting               | Dim dot             | Claude finished responding — or the turn was interrupted with Esc — and is idle, waiting for user input                                                              |

**How it works:** asks each terminal for its current foreground process pid via `tcgetpgrp(fd)` (exposed by node-pty's `foregroundPid` accessor), then checks whether `~/.claude/sessions/<fgpid>.json` exists. If it does, that terminal is running claude-code — we tail the session's JSONL transcript to derive state from the last message. The tail is also scanned for background tasks the agent launched — a dynamic `Workflow`, a backgrounded `Bash` command, or a background `Task`/`Agent` — that haven't yet reported a terminal status via a `queue-operation` completion; when one is outstanding, a bare end-of-turn is promoted from *waiting* to *running in background* so a busy-waiting agent doesn't read as needing you. An interrupted turn (Esc) appends a trailing `user` entry carrying an explicit interrupt marker (`[Request interrupted by user]`, or an errored `tool_result` for a mid-tool-call Esc); that marker reads as *waiting* (idle), not *thinking* — so the dock settles instead of animating a phantom spinner that persists across `claude -c`. Because the Claude Agent SDK buffers the `tool_use` message for `AskUserQuestion` / `ExitPlanMode` until the user answers, that wait window has no on-disk JSONL signal — so on first launch the server idempotently installs a `PreToolUse`/`PostToolUse` hook in `~/.claude/settings.json` (matcher `AskUserQuestion|ExitPlanMode`) that writes/clears a per-session sidecar at `~/.claude/kolu-awaiting/<sessionId>.json`; the watcher reads that sidecar alongside the transcript and, while it's present, overrides state to *awaiting input* (it wins even over *running in background*). The hook writer is a zero-dependency script that always exits 0, so it can never block the agent; install is fail-open and merges without touching your other hooks. If `settings.json` is declaratively managed (a home-manager symlink into the read-only Nix store, a dotfiles linker, …) it is never rewritten — kolu logs the exact hook block to add to your managed config instead, and the writer assets still install so a one-time declarative add is all that's needed. Opt out with `KOLU_DISABLE_CLAUDE_HOOKS=1` or a `~/.claude/kolu-hooks/disabled` marker file. Cross-platform (Linux + macOS) since `tcgetpgrp` is POSIX. Each card also surfaces the session's display title (custom title › auto-generated summary › first prompt) via the [Claude Agent SDK](https://platform.claude.com/docs/en/api/agent-sdk/typescript)'s `getSessionInfo()`, refreshed best-effort on each transcript change. The tile chrome also shows a running token count (compact, e.g. `47K`) summed from the latest assistant entry's `message.usage` — `input_tokens + cache_creation_input_tokens + cache_read_input_tokens`. Raw count only; window size isn't inferable from the JSONL (1M beta strips its suffix, so a `%` would lie), and the raw number is the useful signal anyway.

**What we can't detect:**

- **Permission prompts** — when Claude blocks asking you to approve a tool it hasn't been pre-authorized for, that block isn't surfaced. Only `AskUserQuestion` / `ExitPlanMode` are (via the #905 hook — see *Awaiting input* above), because those are the two tools that declare `requiresUserInteraction(){return true}` and so get a deterministic `PreToolUse`/`PostToolUse` matcher
