...
- **Dock — two-level navigator** — the left-edge dock is the canonical live-terminal surface, with two progressive levels of detail (#903):
  ...
  - **Cards** (default) — rows grouped by repo. Each repo gets a small section header (uppercase name + repo-color swatch + row count); rows below stack as `dot · branch · time` lines. The dot color carries bucket state — awaiting (red, breathing), working (amber, pulsing), idle (green, flat), none (grey, flat). The active row floods to accent and reveals an inline detail line — agent indicator + PR badge + sub-terminal chip — so triage info is one click away rather than crowding every row.
