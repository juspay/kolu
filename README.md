...
### Terminal UI — `kolu-tui` (beta)

- `kolu-tui` is a terminal-side CLI client that connects to a **running kolu-server** over a local unix socket (`$XDG_RUNTIME_DIR/kolu/pty-host.sock`) and drives the same in-process PTYs from the shell — no browser. The first beta is **read-only**: `kolu-tui list` prints your live terminals (id · pid · idle · cmd · cwd — the `cmd` is the OSC title or the foreground command; `--json` carries the full metadata incl. raw title + foreground process) and `kolu-tui snapshot <id>` dumps a terminal's current scrollback. Run it with `nix run github:juspay/kolu#kolu-tui -- list` while a server is running (`--socket <path>` points at a non-default server; `list --json` is machine-readable). It speaks the *raw* `ptyHostSurface` contract directly — the CLI face of kolu and the seed of a `tmux` replacement; `attach` / `spawn` / `kill` land in later phases. Design: [`docs/plans/remote-terminals.pty-daemon.tui.html`](docs/plans/remote-terminals.pty-daemon.tui.html)
...
| `packages/pty-tui/`                  | `kolu-tui` (beta) — a terminal-side CLI client of the in-process pty-host. Dials kolu-server's unix socket via `stdioLink`, speaks `ptyHostSurface` directly (the *raw* client; the browser is the *rich* one over the full contract), and ships `list` + `snapshot`. The second, independent consumer that proves the contract is transport- and consumer-agnostic |
...
All **browser** traffic flows over a single WebSocket (`/rpc/ws`) via [oRPC](https://orpc.dev/). The contract in `packages/common/` is shared by both sides — types checked at compile time, payloads validated by Zod at runtime. _(The CLI client `kolu-tui` is the exception: it reaches the in-process pty-host over a separate local **unix socket** speaking the `ptyHostSurface` contract — same oRPC framing, a different transport. See the `pty-tui` row above.)_ Two communication patterns:
