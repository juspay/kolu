...
| `packages/integrations/anyagent/`    | Agent-agnostic shared contract (`AgentProvider` interface, `agentInfoEqual`), types (Logger, TaskProgress), agent CLI parsing, and runtime diagnostics registry        |
...
| ------------------ | ------------------------------------------ | ------------------------------------- | ------------------------------------------------------ |
| Request / response | one-shot RPC call                          | plain `client.*` calls                | `terminal.create`, `terminal.kill`, `server.diagnostics` |
...
For a live snapshot, open `Debug → Diagnostic info` from the command palette. The dialog groups browser facts, xterm/WebGL state, and server runtime state, including uptime, Node memory, PTY processes, publisher pressure, active `fs.watch` handles, timers, subscriptions, and DB handles. Copy JSON from that dialog when filing support tickets.
