...
| `packages/solid-icons/`              | The app's inline SVG icon set as SolidJS components (`Component<{ class?: string }>`), sized/coloured via Tailwind `class`. Pure presentation, no domain coupling |
| `packages/solid-ui/`                 | Presentation atoms carrying the Kolu design system but no domain logic — panel chrome (`surface`), `Toggle`, `Kbd`, `SegmentedControl`, `Row`, `Section`, `Tip`, z-index layering. Each module is also reachable via a subpath (`@kolu/solid-ui/Surface`) |
| `packages/solid-overlay/`            | Anchored-positioning primitives for floating UI: `useAnchoredPopover` (portal-safe open/dismiss + viewport-flip placement) and `OptionMenu` (anchored option list built on it) |
| `packages/solid-xterm/`              | SolidJS-native primitives over [`@xterm/xterm`](https://xtermjs.org/). `createXtermWebgl` owns the WebGL-context lifecycle — including the `loseContext()` GPU release xterm omits — so multi-tile apps don't exhaust Chrome's ~16-context budget |
| `packages/platform/`                 | Framework-agnostic browser/OS adapters (no `solid-js` dep): `keyboard` (chord matching), `os` (Apple-modifier detection), `clipboard` (writes that survive non-secure contexts + an xterm clipboard provider) |
...
| `packages/server/`                   | [Hono](https://hono.dev/); hosts the in-process agent (`@kolu/pty-host` + the provider DAG) and consumes its metadata stream for terminal lifecycle                     |
...
flowchart TB
  ...
  subgraph Server["Server (Hono)"]
    subgraph Agent["Agent (in-process today; ssh-remote in a later phase)"]
  ...
  style Agent fill:none,stroke:#e9c46a,stroke-width:1px,color:#264653
```
...
**Metadata** (dashed lines) — every per-terminal observer lives in the provider DAG at [`packages/server/src/terminalBackend/providers.ts`](packages/server/src/terminalBackend/providers.ts), parameterized over `ProviderHooks` + `ProviderChannels` so the host is the only thing that varies. Since [#951 R4b](docs/plans/remote-terminals.html) the DAG runs inside an in-process **agent** ([`agent.ts`](packages/server/src/terminalBackend/agent.ts)) that also owns `@kolu/pty-host`: the agent constructs a `ProviderRecord`, wires `ProviderHooks` that turn each metadata write into a stream event, and calls `startProviders`. [`LocalTerminalBackend`](packages/server/src/terminalBackend/local.ts) — the concrete `TerminalBackend` for "this kolu process" — is a pure *consumer* of that stream: it mirrors each event onto the `terminalMetadata` collection and fires the `terminals:dirty` autosave trigger only for the persisted half (the stream splits into `metadataPersisted` / `metadataLive` events, so the server-persisted-vs-live fence rides the event type across the boundary). Co-locating the providers with the PTY — where this host's filesystem, process table, and agent-state files are — is what lets a future ssh agent reuse them unchanged; `Local`- and `RemoteTerminalBackend` then differ only in transport. The DAG itself: CWD changes (OSC 7) → git provider (`.git/HEAD` watcher) → GitHub provider (`gh pr view` polling). Agent detection uses a single generic orchestrator (`startAgentProvider` in `providers.ts`) driven by per-agent `AgentProvider` instances from each integration package. Today three instances are registered: `claudeCodeProvider` (from `kolu-claude-code`) wakes on title events (OSC 2) and its own `fs.watch` on `~/.claude/sessions/`; `codexProvider` (from `kolu-codex`) queries the highest-numbered `~/.codex/state_<N>.sqlite` for thread metadata and tails the matched rollout JSONL for state transitions; `opencodeProvider` (from `kolu-opencode`) queries OpenCode's SQLite database directly and watches its WAL file for live state updates. Adding a new agent CLI is one new `AgentProvider` and one line in `startProviders` — no server-side adapter file. All providers feed the agent's single metadata stream, which the backend mirrors to the client as a subscription[^providers]. Separately, kolu's preexec hook emits an `OSC 633;E` command mark before each user command; `@kolu/pty-host` surfaces it on its command-run tap, the agent bridges that onto its internal `commandRun` channel, and the agent-command tracker (in `providers.ts`) subscribes to match the first token against a known-agents allowlist and fan out to both (a) a `recentAgent` signal the agent forwards to kolu-server's bounded recent-agents MRU for the agent-aware command palette and (b) a per-terminal stash keyed by terminal id, so codex/opencode session detection still matches when the agent is an interpreter shim (e.g. npm-installed `codex`, whose kernel-level process name is `node`). No `/proc` lookups or argv scraping. The `TerminalBackend` interface itself lives in [`kolu-common/terminalBackend`](packages/common/src/terminalBackend.ts) — `LocalTerminalBackend` is one concrete shape; a future `RemoteTerminalBackend` (for terminals running on SSH hosts) is the other, and every call site downstream goes through `getTerminalBackendFor(location)` so neither asks "which kind?".
