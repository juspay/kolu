...
| `packages/integrations/git/`         | Pure git operations — `simple-git` + shared `chokidar` watchers: repo resolution, worktree lifecycle, diff review, live file-tree browsing, path security; schemas re-exported by `kolu-common` |
...
| Subscription       | server pushes values over WebSocket stream | `createSubscription` → SolidJS signal | Terminal list, metadata, server state, Code tab file tree |

Subscriptions use [`createSubscription`](packages/client/src/rpc/createSubscription.ts) — a 150-line primitive that converts an `AsyncIterable` into a SolidJS signal via `createStore` + `reconcile` for fine-grained reactivity. Per-terminal subscriptions use SolidJS's `mapArray` for automatic lifecycle management. Streams that accumulate state, such as the Code tab file tree, yield an explicit `snapshot` first and then `delta` events so reconnects replace stale state before applying live changes.
...
**Code browsing** — the Code tab combines one-shot git RPCs for file content and diffs with a shared `fs.watch` subscription for the repo file tree. The server keeps one debounced watcher per repo path and refreshes through git's own `ls-files --cached --others --exclude-standard` view, so `.gitignore` remains the source of truth. Browse mode applies file-tree deltas directly to Pierre's `@pierre/trees` mutation API; Local and Branch modes use the same stream as a trigger to refresh their git-status projections.
