...
| `packages/integrations/claude-code/` | Claude Code detection — executor-routed session-file reads, JSONL transcript tailing, and Claude Agent SDK summaries; exports a `claudeCodeProvider` `AgentProvider` |
| `packages/integrations/anyagent/`    | Agent-agnostic shared contract (`AgentProvider` interface, `agentInfoEqual`), TaskProgress schema, and agent CLI parsing                                             |
| `packages/integrations/codex/`       | Codex detection — queries the highest-numbered `~/.codex/state_<N>.sqlite` through an `Executor` and tails the matched rollout JSONL for state; exports a `codexProvider` |
| `packages/integrations/opencode/`    | OpenCode detection — queries OpenCode's SQLite database through an `Executor`; exports an `opencodeProvider` `AgentProvider`                                         |
| `packages/integrations/git/`         | Pure git operations — executor-backed `git` invocations for repo resolution, worktree lifecycle, diff review, path security; schemas re-exported by `kolu-common`     |
...
| `packages/integrations/io/`          | Filesystem & I/O primitives — `Executor` / `localExecutor`, `Host extends Executor`, and refcounted shared `fs.watch` keyed by directory (`createDirFilenameWatcher`) |
...
**Metadata** (dashed lines) — shell activity triggers a provider DAG: CWD changes (OSC 7) → git provider (.git/HEAD watcher) → GitHub provider (`gh pr view` polling). Agent detection uses a single generic orchestrator ([`meta/agent.ts`](packages/server/src/meta/agent.ts)) driven by per-agent `AgentProvider` instances from each integration package. IO-bearing provider methods receive an `Executor`; the current local path passes `localExecutor`, and future host implementations can satisfy the same shape. Today three instances are registered: `claudeCodeProvider` (from `kolu-claude-code`) wakes on title events (OSC 2) and watches `~/.claude/sessions/` through the executor; `codexProvider` (from `kolu-codex`) queries the highest-numbered `~/.codex/state_<N>.sqlite` through `executor.queryDb` and tails the matched rollout JSONL for state transitions; `opencodeProvider` (from `kolu-opencode`) queries OpenCode's SQLite database through the executor and watches its WAL file for live state updates. Adding a new agent CLI is one new `AgentProvider` and one line in `startProviders` — no server-side adapter file. All providers feed a single metadata channel streamed to the client as a subscription[^providers]. Separately, kolu's preexec hook emits an `OSC 633;E` command mark before each user command; the pty handler republishes the raw payload on a `commandRun` channel, and [`meta/agent-command.ts`](packages/server/src/meta/agent-command.ts) subscribes to match the first token against a known-agents allowlist and fan out to both (a) a bounded recent-agents MRU for the agent-aware command palette and (b) a per-terminal stash keyed by terminal id, so codex/opencode session detection still matches when the agent is an interpreter shim (e.g. npm-installed `codex`, whose kernel-level process name is `node`). No `/proc` lookups or argv scraping.
...
[^providers]: Git provider invokes `git` through the shared `Executor`; GitHub provider derives combined CI status from `CheckRun` + `StatusContext`. Agent providers implement the shared `AgentProvider` contract (`anyagent`): `resolveSession(terminalState, executor)` → `sessionKey(session)` for dedup → `createWatcher(session, executor, onChange)` for per-session state derivation, with an optional `externalChanges: { isPresent, install }` pair for out-of-band match triggers. `claudeCodeProvider` asks the pty for `tcgetpgrp(fd)`, reads `~/.claude/sessions/<fgpid>.json` through the executor, watches `~/.claude/sessions/` as its external-change signal, then tails the matched session's JSONL transcript through the same executor for state updates; the session display title comes from a fire-and-forget [`getSessionInfo()`](https://platform.claude.com/docs/en/api/agent-sdk/typescript) call piggybacking on the same transcript watcher. `codexProvider` matches when either the foreground process basename is `codex` or the preexec stash names `codex` (interpreter-shim fallback), queries Codex's threads SQLite DB (highest-numbered `~/.codex/state_<N>.sqlite`) filtered to `source = 'cli'` for the cwd's latest thread, reads mutable metadata (title, model) from indexed columns, and tails the thread's rollout JSONL (`threads.rollout_path`) to derive state from `task_started` / `task_complete` boundaries and open `function_call` call_ids — one executor watch on the SQLite WAL file doubles as both the external-change signal and the per-session refresh trigger, since Codex writes the WAL and appends the JSONL atomically. `opencodeProvider` matches via the same dual signal (foreground basename or preexec stash), queries `~/.local/share/opencode/opencode.db` for sessions in the terminal's CWD through `executor.queryDb`, and watches the WAL file (`opencode.db-wal`) for live state updates — it has no external-change subscription because title events cover every match transition.
