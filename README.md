...
- **Activity window — one tolerance, every surface** — a single per-device choice (`All / 4h / 12h / 24h / 48h`, default `24h`) governs how aggressively terminals fade into the parked bucket once their last agent transition is past the threshold. The dock header carries a compact pill that opens the picker; the minimap's matching pill drives the same shared signal, so tightening one tightens the other. **Stale terminals never lose their identity** — parked rows carry the agent indicator (`Claude Code · waiting · 22.0K`) on a compact one-liner, so the screen-after-laptop-sleep case still reads as "Claude was waiting" rather than a plain shell, but a wall of yesterday's awaiting cards doesn't dominate the dock height. The canvas tile fades, the parked terminal stops counting toward the OS/PWA dock badge, and any fresh agent transition unparks automatically.
...
- GitHub PR detection — shows the merge-state icon (open / merged / closed), CI check status dot (pass/pending/fail), `#N`, and PR title on the tile chrome, on the inspector, **and on the dock row** so the merge state is visible from the canonical navigator without focusing the tile
