...
- **Activity window — one tolerance, every surface** — a single per-device choice (`All / 4h / 12h / 24h / 48h`, default `24h`) governs how aggressively non-awaiting terminals fade into the parked bucket. The dock header carries a compact pill that opens the picker; the minimap's matching pill drives the same shared signal, so tightening one tightens the other. **Attention-state agents (`waiting` / `awaiting_user`) are exempt regardless of age** — a user-blocking agent never loses its identity to staleness, even after a laptop slept overnight. Past the threshold, non-awaiting rows fade and drop their agent-state border, the canvas tile fades, and the terminal stops counting toward the OS/PWA dock badge. Any fresh agent transition unparks automatically.
