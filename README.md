...
- **Inline composer at the line** — click a line in comment mode and a tiny composer pops up anchored next to that line (GitHub-PR style). <kbd>Enter</kbd> submits, <kbd>Shift+Enter</kbd> inserts a newline, <kbd>Esc</kbd> dismisses. No mouse trip to a bottom textbox; the popover comes to you
...
- **Plain Markdown clipboard payload** — flushed text is a Markdown bullet list with a code-spanned GitHub-style line ref per entry: `` - `path:Lstart-end` — note ``. No envelope or wrapper — paste it raw, or prefix your own framing (`"Apply these comments:"`, `"## Review notes"`) before pasting. Renders cleanly in agent terminals, GitHub issues, and chat
