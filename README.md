...
- **Floating workspace switcher** — a compact repo/branch pill strip sits at the top of the canvas, ghosted at rest and behind any tile that overlaps it; hover opens a searchable panel grouped by live agent state (`Awaiting you`, `Working`, `No agent`) with repo facets. The expanded panel orders entries by **agent recency** (most recent agent semantic-key transition first) with canvas position as the secondary key. The collapsed pill strip always shows every active-agent terminal, fills remaining slots with up to five idle terminals per repo, then **alphabetizes** repos and branches so pill positions don't shift when one of their peers transitions a state. Recency is persisted, so a restored session lands in the order it left off. Click a pill or card to focus and center its tile
