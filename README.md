...
Detects terminal [Codex](https://github.com/openai/codex) sessions and shows their state alongside Claude Code and OpenCode on the tile chrome.

**How it works:** when the foreground process is `codex`, the provider reads Codex's local state database from `~/.codex/state_*.sqlite`, finds the most recently updated top-level CLI thread whose `cwd` matches the terminal's CWD, and treats that thread ID as the stable session identity. Per-session state then comes from the thread's rollout JSONL (`rollout_path`) plus live thread metadata from SQLite. We watch both the rollout file and the SQLite WAL, so title/model changes and task-state changes propagate without polling.

**State derivation:** Codex writes explicit `task_started` / `task_complete` lifecycle events into the rollout JSONL. Kolu uses those as turn boundaries. If the latest boundary is `task_complete`, the session is _waiting_. If the current turn has an outstanding `response_item` `function_call` without a matching later `function_call_output`, the session is _running tools_. Otherwise it is _thinking_.

**What we detect:**

| State    | Indicator          | How                                                                       |
| -------- | ------------------ | ------------------------------------------------------------------------- |
| Thinking | Pulsing accent dot | Latest task boundary is `task_started` and no tool call is currently open |
| Tool use | Spinning yellow    | Current task has an unmatched `function_call`                             |
| Waiting  | Dim dot            | Latest task boundary is `task_complete`                                   |

**What we can't detect (yet):**

- **Todo progress** — Codex exposes no first-class task checklist equivalent to OpenCode's `todo` table
- **Context-window tokens** — `threads.tokens_used` is cumulative usage, not the same semantic as Kolu's current-window token badge
- **Non-default state DB path** — set `KOLU_CODEX_STATE_DB` or `KOLU_CODEX_HOME` to override discovery
...
| `packages/integrations/codex/`       | Codex detection — reads Codex's SQLite thread store plus rollout JSONL; exports a `codexProvider` `AgentProvider`                                  |
