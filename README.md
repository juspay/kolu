...
- **Minimap heatmap & focus toggle** — the canvas minimap dots any tile whose agent is currently `waiting` (alert color) or `thinking`/`tool_use` (accent color), suppressed once auto-parked, so you can scan a 20-tile workspace for "who needs me" or "who is making progress" without opening the switcher. A moon-icon toggle in the zoom bar demotes parked tiles to small ghost markers — they stay reachable, but visual weight shifts onto the agents that are still in play
