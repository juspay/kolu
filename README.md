...
- **Dock — two-level navigator** — the left-edge dock is the canonical live-terminal surface, with two progressive levels of detail (#903):
  ...
  - **Cards** (default) — rows grouped by repo. Each repo gets a small section header (uppercase name + repo-color swatch + row count); rows below stack as `dot · branch · pips · time` lines. The dot color carries bucket state — awaiting (red, breathing), working (amber, pulsing), idle (green, flat), none (grey, flat). Inactive rows carry presence-only pips (PR state icon, agent-kind logo, sub-terminal chip) so a quick scan reads as "has a PR, has an agent" at a glance. The active row gets a quiet highlight (lifted surface + accent left-edge stripe) and unfolds the full detail line beneath — PR badge with checks status, agent indicator with token count.
  ...
  Ordering is **repo → bucket → recency**: rows group by repo, then within each repo sort by bucket priority (awaiting first), then by recency. Repos themselves sort by the priority of their top row, so a repo with an awaiting agent floats above a repo whose hottest row is merely working. Click any dock row or palette workspace card to focus and center its tile. In maximized-tile mode the dock renders as an opaque flush-left sidebar and the maximized terminal reflows next to it (#904); in tiled mode the dock floats over the canvas as an equally opaque card so tiles never bleed through the seams.
