...
[PartySocket](https://docs.partykit.io/reference/partysocket-api/) handles WebSocket auto-reconnect; `@kolu/surface/solid`'s `surfaceClient` (wired up in `packages/client/src/wire.ts`) installs oRPC's [`ClientRetryPlugin`](https://orpc.dev/docs/plugins/client-retry) so every Cell/Collection/Stream/Event hook (and the `streamCall` escape hatch for raw streams like terminal `attach`) transparently re-subscribes after a drop — every server-side streaming handler is already snapshot-then-deltas and the reducer in `useTerminalMetadata.ts` pattern-matches an `ActivityStreamEvent` discriminated union (`snapshot` replaces, `delta` appends) so re-subscribe resume is structural, not defensive. Transport events (`connecting` / `connected` / `disconnected` / `reconnected` / `restarted`) are exposed as a single `ServerLifecycleEvent` signal in `packages/client/src/rpc/rpc.ts`. `TransportOverlay` renders one dim-backdrop card (pointer-events-none, so users can still scroll and read buffers underneath): `disconnected` shows "Reconnecting…". Applying a new client build is a separate concern owned by `packages/client/src/pwa.ts`, which takes one of two paths by origin. **On HTTPS** the service worker runs in `prompt` mode so a deploy never reloads an open terminal session unbidden — a server restart (`restarted`) nudges the worker to check for a new build (`checkForUpdate`), and once one is installed and waiting the `swUpdateReady` signal swaps the card to "App updated" with a Reload button that goes through `registerSW`'s `updateServiceWorker(true)`, activating the waiting worker and reloading only once it controls the page (closing the "stale assets until a force-reload" race where the old code reloaded *before* the new worker had activated). **Everywhere else** — plain HTTP, LAN, a bare-hostname deploy, or `http://localhost`; anything but HTTPS, where a service worker would only add a stale-serving precache, so it is deliberately disabled — the same card is driven by the *durable* signal that the running bundle's baked-in commit differs from the server's (`clientIsStale`, the comparison behind the chrome bar's `≠ srv` badge) or by a server restart, and Reload is a plain `location.reload()` that always lands on the fresh bundle because the server serves the SPA shell (`index.html`) `no-store` (`packages/server/src/cacheControl.ts`) — so a normal reload can never replay a stale shell across a deploy.
