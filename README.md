...
- **Dock — two-level navigator** — the left-edge dock is the canonical live-terminal surface, with two progressive levels of detail (#903):
  ...
  - **Cards** (default) — recency-sorted rows: awaiting terminals get full cards with a tail of xterm buffer + a reply input wired straight to the PTY, working terminals get compact pills, idle terminals get faded one-liners. Parked terminals (past the activity window threshold) are hidden — the activity-window selector itself controls visibility, so picking "Show all terminals" surfaces them again.

  Rows are **auto-grouped by repo → branch** — terminals fall into a foldable header for their repo (and, when that repo has more than one live branch, a nested branch sub-header beneath it). Single-branch repos collapse to a `"REPO · branch"` header so the common case stays compact. Within each leaf group the existing recency + bucket-priority order applies; group fold state persists per-device via `localStorage`. Folded terminals remain in the `Cmd+1..9` depth-first walk so the keystroke targets the Nth terminal regardless of fold state.
  ...
  Ordering across groups is **max-recency per group descending** so the most-recently-active repo leads; within a leaf, the existing **agent recency** order (most recent agent semantic-key transition first) is preserved. Recency is persisted, so a restored session lands where it left off. Click any dock row or palette workspace card to focus and center its tile. In maximized-tile mode the dock renders as an opaque flush-left sidebar and the maximized terminal reflows next to it (#904), so an agent on another terminal needing attention is still visible.
...
- **Activity window — one tolerance, every surface** — a single per-device choice (`All / 4h / 12h / 24h / 48h`, default `24h`) governs which terminals stay visible in the dock. Past the threshold, a terminal is hidden from the dock entirely; **picking "Show all terminals" surfaces every parked row again**. The dock header carries a compact pill that opens the picker; the minimap's matching pill drives the same shared signal, so tightening one tightens the other. With repo→branch grouping carrying the organization, hiding (rather than dimming) parked rows keeps every branch sub-section focused on what's still in play. The canvas tile fades, the parked terminal stops counting toward the OS/PWA dock badge, and any fresh agent transition surfaces it again automatically.
