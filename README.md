...
**How it works:** when the foreground process is `codex`, the provider queries Codex's threads SQLite DB directly at `~/.codex/state_5.sqlite` to find the most recently updated non-archived `cli` thread whose `cwd` matches the terminal's CWD. Thread metadata (title, model, `tokens_used`) comes straight from indexed columns — Codex pre-sums the running context-token total from its `token_count` events, so no JSON parsing is needed for any of the three. For the one signal SQLite doesn't expose (thinking / tool_use / waiting), the watcher tails the per-thread rollout JSONL at `threads.rollout_path` and pattern-matches `task_started` / `task_complete` / `function_call` / `function_call_output` events. Live updates come from `fs.watch` on the SQLite WAL file (`state_5.sqlite-wal`); Codex writes the WAL and appends the JSONL in the same cycle (verified: nanosecond-identical mtimes), so one signal covers both sources.

**Why both SQLite and JSONL?** SQLite alone gives us metadata but has no `state` column — those transitions live only in the event stream. JSONL alone would force re-parsing the entire file on every update to recover title and token counts Codex already maintains. The split mirrors what OpenCode and Claude Code do individually — OpenCode reads everything from SQLite, Claude Code reads everything from JSONL — combined because Codex writes them atomically but specializes them by purpose.
...
- **Schema version drift** — the `state_5` suffix is Codex's current schema version; a future `state_6.sqlite` would need a version bump in `kolu-codex`'s config. Set `KOLU_CODEX_DB` to override the path meanwhile
...
| `packages/integrations/codex/`       | Codex detection — reads `~/.codex/state_5.sqlite` for thread metadata and tails the matched rollout JSONL for state; exports a `codexProvider`     |
