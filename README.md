...
- **Floating workspace switcher** — a compact repo/branch pill strip sits at the top of the canvas, ghosted at rest and behind any tile that overlaps it; hover opens a searchable panel grouped by live agent state (`Awaiting you`, `Working`, `No agent`) with repo facets. The expanded panel orders entries by **terminal recency** (most recently typed-in or agent-state-changed first) with canvas position as the secondary key. The collapsed pill strip uses recency to *select* which terminals appear (top 5 per repo) but **alphabetizes** repos and branches for predictability — pill positions don't shift when one of their peers gets typed in. Recency is persisted, so a restored session lands in the order it left off. Click a pill or card to focus and center its tile
