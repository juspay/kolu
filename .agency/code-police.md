...
- **(a) A `T | null` / `T | undefined` whose absent value is tested for two distinct meanings** — ≥2 read sites handle the absence *divergently* (one treats it as "not yet", another as "errored"; one as "disconnected", another as "empty"), **or** a name/comment/JSDoc admits two absence-reasons (`absent-vs-error`, `disconnected-vs-empty`, `not-yet-vs-never`) that a consumer *should* tell apart. One nullable, two questions.
...
Guidance: prefer a `{ kind: … }` discriminated union — one arm per state, each carrying exactly the data that state has, dispatched with `ts-pattern` (`prefer-ts-pattern`). `SurfaceIdentity` (`packages/surface/src/identity.ts`) is the canonical exemplar. It began as `identity(): T | null` (null = no link) **and** `baked: T | null` (null = no build) **and** `commit: string | null` (null conflating a dev tree with a real commit) — three nulls, three overlapping questions — and became:
