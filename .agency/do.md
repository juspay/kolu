...
**Cover mobile/touch, not just desktop.** Kolu runs on phones and tablets — the right panel becomes a bottom **drawer** and coarse-pointer hit targets must clear the WCAG 2.2 **24px** floor (`isTouch()` drives roomier sizing across the UI). So for **any** visible UI change, a desktop-only capture is **not enough**: also capture (or at minimum verify) the **mobile/touch** rendering — drive a phone-width viewport (e.g. `resize_page` to ~390×844 in the chrome-devtools path, or the mobile drawer) and confirm the change is reachable, **not clipped**, and tappable. Desktop-only evidence has already shipped a touch regression (20px segmented-control targets a desktop screenshot looked fine at). When the desktop and mobile layouts differ, post **both**; when they're identical, one line confirming the mobile rendering was checked suffices. Skip only for changes with no on-screen surface at all.
