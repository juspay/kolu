...
Mechanically: add or edit one **Markdown list item** under the right `### Added` / `### Fixed` / `### Changed` / `### Heads-up` (the editorial home for disruptive changes — a removed feature, a changed default, a migration) heading — `- <Change title="…" pr={n}>…</Change>` for a single PR, `prs={[…]}` for several — the **whole entry on ONE line** (no wrapping) so a section of many entries merges without conflict. `title` is the scannable, product-language headline; the children are the supporting prose. `<Change>` / `<PR>` are **auto-injected** (`changelog.astro`'s `components` prop) — no import line. The PR chip lands during doc-sync and its number is backfilled right after the PR is created (the step that also finalizes the Atlas note). Skip only a genuinely user-invisible change (pure refactor / internal).
