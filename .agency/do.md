...
- **`website/src/content/changelog/unreleased.mdx`** — the open "Unreleased" changelog entry shown on kolu.dev/changelog. Add a user-facing bullet under the right `### Added` / `### Fixed` / `### Changed` heading for any change a user would notice, in plain product language (not implementation detail). **Always link the shipping PR** with the reusable `<PR id={n} />` component (`website/src/components/PR.astro`) at the end of the bullet — add `import PR from "../../components/PR.astro";` after the frontmatter if it isn't already imported. The PR number isn't known until the PR is opened, so the entry lands during doc-sync and the `<PR id={n} />` link is filled in right after the PR is created (the same step that finalizes the Atlas note).
