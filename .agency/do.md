...
**A companion repo's darwin lane uses `--host`, never inline `$ODU_HOSTS`.** A `@kolu/surface` change drags a downstream repo's CI along (e.g. the drishti PR `surface.md` requires), and that repo's CI is the shelled-out `nix run … odu -- run` path, not `mcp__odu__run`. Its own `hosts.json` may name a *different*, possibly-dark darwin host (drishti's `zest`); pin the override with **`--host aarch64-darwin=<the arbitrated darwin host — see above>`** (per the `/ci` skill) — **never** by exporting inline JSON into `$ODU_HOSTS`, which odu reads as a *file path*, not a value: an inline `$ODU_HOSTS='{…}'` is **silently ignored**, the lane falls back to the repo's on-disk `zest`, and you burn a full CI run on the dead host. If you must set `$ODU_HOSTS`, write a real hosts *file* and point at it.
...
**The changelog is in scope for every user-facing change** — `website/src/content/changelog/unreleased.mdx`. Add one **Markdown list item** under the right `### Added` / `### Fixed` / `### Changed` / `### Heads-up` (the editorial home for disruptive changes — a removed feature, a changed default, a migration) heading: `- <Change title="…" pr={n}>…</Change>`, the **whole entry on ONE line** (no wrapping) so a section of many entries merges without conflict. `title` is the scannable, product-language headline; the children are the supporting prose. `<Change>` / `<PR>` are **auto-injected** (`changelog.astro`'s `components` prop) — no import line. `pr={n}` is the shipping-PR chip: the entry lands during doc-sync and the `pr` prop is backfilled right after the PR is created (the step that also finalizes the Atlas note). Skip only a genuinely user-invisible change (pure refactor / internal).
