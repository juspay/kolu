...
Run **check** and **fmt**, then commit (conventional message) and push the feature branch. **`just check` (tsc + biome) green is not proof the shipped artifact *builds*** — when the change adds or edits a bundler/server entrypoint (a `vite.config.ts`, a `nix run` server wrapper, any module the real build loads) that **imports a workspace package**, tsc resolves extensionless imports that native ESM / the bundler will *reject*, so a clean typecheck can sit on top of a `vite build` / `nix run .#<pkg>` that doesn't build at all. For that kind of change the §5 venue gate fires early: actually run the real build on a pu box (`nix run .#<pkg>` / `vite build`), don't infer it from the typecheck. Leaving it for CI/evidence to surface is how a non-building entrypoint reaches the gauntlet. **And a green *build* is itself not proof when the change touches the *dev runtime* — a dev-server-only or experimental flag (a Vite `experimental`/bundled-dev mode, a new/edited `dev` recipe, any config that only takes effect under `vite dev`), whose failure surfaces at dev-server *startup*, never at `vite build` and never in CI (CI exercises the build, not `just dev`).** For that kind of change the same pu-box gate covers the runtime too: **boot the dev runtime** — `just dev-auto` per `/dev-server` — and confirm it serves, don't infer it from a green build. Stopping at "builds green / CI green" while an adopted dev-mode feature was never launched is the runtime twin of the non-building entrypoint above, and is what forces the user to step in and say "but does the dev server actually start?".
