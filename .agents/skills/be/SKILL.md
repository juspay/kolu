...
**Surface the state, not just the mechanism — a silent self-heal is half the feature.** When the change adds a mechanism that *detects, recovers, or transitions through* a state the user cares about — a connection watchdog / reconnect / liveness ping, a background sync or retry/backoff, a degraded or offline mode, anything that quietly changes what's true under the user — making that state **observable in the UI is part of the feature, not a follow-up.** A reconnect that heals the socket but shows the user nothing is built half-way: they can't see it working, can't tell when it's degraded, and the §5 evidence gate has nothing to point a camera at (a recovery with no visible surface is the gate's own "backend by cause, visible by effect" — the effect just hasn't been wired yet). Two rules make the indicator complete: **(a) show the healthy/positive state too, not only errors** — a status light that's invisible when connected and only appears on a problem is a bug, not a minimal version; render green-when-live exactly as the amber/red degraded states render. **(b) Reuse the codebase's existing indicator** — this is the *reuse-the-source-of-truth* clause above applied to UX: if a sibling surface already shows this state (kolu's `IdentityRail` "srv" dot fed by the reactive `wsStatus` memo is the canonical connection example), expose the same reactive state and mirror that component, never hand-roll a one-off that diverges. If you genuinely judge no surface is warranted, say so explicitly in the PR body (one line, like the evidence skill's "no visual impact") — never decide it silently by omission.
