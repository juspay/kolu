...
#
# Usage:
#   codex-review.sh <base-branch> <rebuttal-file|-> <out-json> [transcript-html]
#
...
#   [transcript-html] resolved live-transcript output path to exclude from
#                    review scope (defaults to codex-debate-transcript.html).
#                    The debate re-renders this file between rounds, so it must
#                    be ignored even when pointed at a committable custom path.
#
# Notes:
...
base="${1:?usage: codex-review.sh <base-branch> <rebuttal-file|-> <out-json> [transcript-html]}"
rebuttal_file="${2:?missing rebuttal-file (use - for none)}"
out="${3:?missing out-json path}"
transcript_html="${4:-codex-debate-transcript.html}"
...
# Unquoted heredoc: only $base, $transcript_html, and $rebuttal_block expand. No
# backticks and no other $/$(...) appear in the body, so nothing else interprets.
prompt="$(cat <<EOF
...
IGNORE the debate's own artifacts if present — '$transcript_html'
(this debate's live transcript, re-rendered between rounds) and anything under
'.codex-debate/'. They are generated by this review process, not part of the
change under review.
