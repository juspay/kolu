...
3. **Stage the release — uncommitted** — in the working tree only: promote the current `unreleased.mdx` body to `website/src/content/changelog/<X-Y-Z>.mdx` with `{ version, date }` frontmatter — consolidate duplicate product-area `###` headings (first-seen order, merge their bullets — never regroup by `kind`), and open with one editorial summary paragraph above the first `###` (the build fails without it). Reset `unreleased.mdx` to an empty open section. Set the version in `packages/server/package.json`.
