...
4. **Decide where it runs — pu box, not locally, whenever ANY kolu is live.** `just test-quick` builds the client and spawns a server: that is **heavy work**, and it goes on an ephemeral pu box (see `/pu` / `/evidence`) any time a kolu is running on this machine (the normal case). A pile-up of local e2e runs OOM-`SIGKILL`ed production `kolu.service` beside this command before — "fast" is not "safe to run beside a live kolu." **`/dev-server` §0 owns the local-vs-pu venue gate — apply it, don't re-derive it here**, and note it is deliberately *not* `systemctl --user is-active kolu`: that probe is blind to a `just dev` kolu and answers `inactive` even when no `kolu.service` unit exists at all. Run locally **only** when §0's both-shapes probe comes back empty.
