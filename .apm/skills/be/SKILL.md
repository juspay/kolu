...
**Build the smallest thing that satisfies the §0 contract — match the ask's altitude, don't gold-plate.** A terse ask ("create the skill, open a draft PR") wants a focused diff, not a multi-workflow design exercise or an artifact several times the size of its peers; before reaching for a heavy mechanism (an extra design fan-out, a new abstraction, a sprawling doc), check the literal scope and the size of comparable existing files, and stop at *done*, not *exhaustive*. Over-reaching scope is `overengineering-or-scope`, a recurring defect class — the human shouldn't have to interrupt with "you forgot my original ask."
