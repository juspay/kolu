...
sole editor while it runs: `/lens-debate` applying the agreed structural fixes,
then `/codex-debate` (its per-round commits are the debate — and its cross-family
review covers the lens step's rewrites, which is why it runs second), then
`/simplify`, then code-police. Each step reads a clean tree (the previous step
has committed) and applies its own fixes directly — no snapshot, no apply pass.
be-review pushes once at the end and *then* posts the PR comments (lens, codex,
and a code-police summary), so no comment advertises a local-only commit.

- Pass `base` and the change **`rationale`**. The rationale informs the lenses'
  *cross-examination* (adjudication), never their independent reviews — a
  deliberate decision is dispositioned on the record during the debate, not
  pre-loaded into the reviewers (author framing measurably suppresses findings;
  see the `review-orchestration` Atlas note).
- Lens-debate commits its agreed fixes; codex's rounds commit `fix(…)`; simplify
  ...
  summary.
- On an **unresolved** lens or codex finding, adjudicate it yourself before
  moving on.
