...
- **A plan-of-record is build-ready or it is not done — write it for the *implementing* agent, not for the reviewer.** The bar for a `feature`/`analysis` plan is that **another agent (a GLM, a fresh Claude) can implement it straight through without stopping to ask you anything.** Two failure modes recur and both are defects to fix *before* you present, never to leave for the user to catch (don't make the user the linter):
  ...
  - **The note names things it never pins down — coined terms and asserted specifics left ungrounded.** A plan is read by someone who wasn't in your head, so ground it *in the note itself*, not in the user's follow-up questions: **define every noun you invent where it first appears** (a `mirror`, an `attach()`, a `reconnect storm` left unglossed is a stop-the-read defect); **back every specific claim beside the claim** — assert "four jobs" only next to the enumerated four, describe "typed records `DATA·RESIZE·CKPT`" only next to the actual `type` in code; **give each PR of a multi-PR plan its own one-line user-visible delta** — and when the truth is "internal-only, nothing the user sees yet," say *that* explicitly rather than leaving the user to ask "after a full reload, do I still see my history?"; and the **lead diagram must show every package/module the prose names** — a `@kolu/segment-log` in the text but missing from the SVG is prose⇄diagram drift, a defect to fix before presenting. This is the same "don't make the user the linter" defect as the bullets above, one level down — from open *decisions* to *vocabulary and claims*.
