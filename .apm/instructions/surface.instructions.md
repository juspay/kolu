...
applyTo: "packages/{surface,surface-app,surface-remote,surface-daemon,surface-daemon-supervisor}/**"
---
...
**Any API-facing change** to `packages/surface/`, `packages/surface-app/`, `packages/surface-remote/`, `packages/surface-daemon/`, or `packages/surface-daemon-supervisor/` — exported types, function signatures, the Effect RPC contract (the `defineSurface` spec, the wire tags an `RpcGroup` mints from it, a member's payload/success/error `Schema`), package `exports`, runtime behaviour a consumer depends on, or anything else that changes how a downstream consumer wires these packages — **requires a corresponding PR to [`github.com/srid/drishti`](https://github.com/srid/drishti) that updates drishti for the change and passes full CI.**
