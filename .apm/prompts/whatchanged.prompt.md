---
description: List features added to Kolu during a given timeframe
input:
  - arguments
---

List features added to Kolu during: ${input:arguments} (e.g. "last week", "last 2 weeks", "since 2026-03-01")

## Instructions

1. Determine the repo from `git remote get-url origin`. Run `gh pr list --repo <owner/repo> --state merged --search "merged:>=$(date -d '${input:arguments} ago' +%Y-%m-%d 2>/dev/null || date -d '${input:arguments}' +%Y-%m-%d 2>/dev/null)" --json number,title,mergedAt,url --limit 200` to get merged PRs in the timeframe. If the date parsing fails, fall back to `git log --since="${input:arguments}" --merges --oneline` and correlate with `gh pr list --state merged`.
...
```
## What changed in Kolu (${input:arguments})
