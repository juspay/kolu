...
def main() -> int:
    ...
    for name, r in recipes.items():
        ...
        if name.startswith("_"):
            continue
        # `[metadata("a", "b", ...)]` attributes show up as
        # `{"metadata": ["a", "b", ...]}` in just's JSON dump. Multiple
        # `[metadata]` lines produce separate entries, each with its own
        # list of values. Flatten them into one list of tags.
        tags = []
        for a in r.get("attributes", []):
            if isinstance(a, dict) and "metadata" in a:
                tags.extend(a["metadata"])
        ...
        # that might use [metadata("...")] for unrelated reasons.
        sys_tags = [
            t.split(":", 2)[2]
            for t in tags
            if t.startswith("localci:system:")
        ]
        if not sys_tags:
            # A recipe with no system tag is invisible to the scheduler.
            continue
        # Intra-lane ordering comes from [metadata("localci:depends:<step>")],
        ...
        deps = [
            t.split(":", 2)[2]
            for t in tags
            if t.startswith("localci:depends:")
        ]
