...
- **Prefer live queries for mutable server state**: Use `experimental_liveOptions()` (streaming) over `queryOptions()` (request-response) when the client needs instant reactive updates after `setQueryData`. Standard queries schedule notifications via `setTimeout(0)` — a macrotask delay that breaks synchronous SolidJS reactivity. Live queries drive reactivity through the streaming connection, making `setQueryData` optimistic updates work instantly. Don't escape to local SolidJS stores to work around this — use the right query type.
