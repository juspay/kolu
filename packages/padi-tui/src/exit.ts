/**
 * padi-tui's EXIT CONTRACT, as values.
 *
 ...
 *   2    `wait` ran out of time — the agent never settled
 *   3    `wait`'s terminal exited before reaching the state
...
/** `wait` ran out of time — the agent never settled. Its own code (2) so a
...
/** The watched terminal exited before reaching the state — the wait can never
...
export const failure = (message: string): CliFailure =>
  new CliFailure({ stderr: `padi-tui: ${message}\n` });
...
export function reportOf(error: unknown): string {
  ...
  return `padi-tui: ${typeof e?.message === "string" ? e.message : String(error)}\n`;
}
