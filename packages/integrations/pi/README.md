...
- **Paths** (`config.ts`) — env-resolved `~/.pi/agent` layout; `KOLU_PI_DIR` names the PARENT of the sessions tree (`SESSIONS_DIR` is always `<set>/sessions`), used by tests/e2e fixtures and by users running pi with a custom `PI_CODING_AGENT_SESSION_DIR` / `--session-dir` (pi's own override axes).
