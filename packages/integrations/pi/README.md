...
- **The state fold** (`derivePiState` in `core.ts`) — one backward walk over the transcript tail: `assistant stopReason toolUse` → `tool_use`; `stop` / `length` / `error` / `aborted` → `waiting`; a trailing `user`/`toolResult` → `thinking` (pi persists assistant messages only on completion, so a mid-turn tail is honest work-in-flight). A tail of only `model_change` entries (a fresh or model-cycled pi at an idle prompt) publishes NOTHING — never a fabricated `thinking`. `model` comes from whichever carries the newest timestamp among `model_change` entries and assistant entries' own `model` fields — strict newest-first, so an old startup switch never masks what the latest turn actually ran (idle `/model` switches included: state and model are independent signals, so a thinking tail still shows the badge). `contextTokens` from the newest usage OBJECT's disjoint `input + cacheRead + cacheWrite` (an all-zero usage means 0, not an older turn's number), `summary` from the newest `session_info` in-window: a name publishes it, a nameLESS entry is pi's explicit clear (published as none, cache dropped), and no entry in the window is "unknown", over which the watcher merges its last-known name.
- **Transcript export** (`transcript.ts`) — renders the ACTIVE branch of pi's entry tree: the parentId chain from the file's last entry, in file order (pi's own `buildContextEntries()` precedent). Abandoned `/tree` branches are dropped.
- **Schemas** (`schemas.ts`) — `PiInfoSchema`, browser-safe, re-exported into the `terminal-vocab` `AgentInfoSchema` union.
- **Store resolution** (`session-root.ts`) — pi's session store moves per invocation, and those overrides live in the pi process, not in padi: the adapter resolves the foreground pi's real store root each reconcile by reading `/proc/<pid>` (or `ps` on macOS) for `--session-dir` / `PI_CODING_AGENT_SESSION_DIR` / `PI_CODING_AGENT_DIR` (which moves sessions too — it is NOT config-only, verified against pi 0.84.2 and a live redirected session), plus `settings.json`'s `sessionDir`, following pi's own precedence. An unreadable process resolves to the default root — never an error answer. The tree watcher covers every root ever resolved, and the transcript exporter searches them.
- **Paths** (`config.ts`) — the default `~/.pi/agent` layout only; `KOLU_PI_DIR` overrides the default scan root for tests/e2e fixtures (deliberately not consulted as a stand-in for a pi process's own `PI_*` overrides).
...
Functions accept `log?: Logger` (from `kolu-shared`). Expected absence (`ENOENT` before first run) is silent by design; real I/O or malformed-data faults surface at error — an unreadable store answers `null` from `findSessionsByDirectory`, never `[]` (the arbiter releases sessions on `[]`).
