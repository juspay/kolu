...
| `github.ts`  | `deriveCheckStatus`, `extractChecks`, `classifyGhError`, `prInfoFromGhView`, `GH_PR_VIEW_JSON_FIELDS` | Pure helpers — no I/O                                            |
...
`startPrSensor` (`packages/padi/src/terminalWorkspace/sensors.ts`) wires anyforge's `subscribePr` to a dispatching `ForgeAdapter` that routes each resolve through the `FORGE_ADAPTERS` registry keyed by `detectForge(remoteUrl)`. **Only `github.com` resolves here** — every other remote (Codeberg, a self-hosted forge, an unknown host, or none) maps to the `unsupported` arm instead, so it never reaches `gh` (a non-GitHub remote can't have a GitHub PR; asking `gh` only produces log noise, kolu#1627). `detectForge` deliberately doesn't guess that an arbitrary host is GitHub, so a **GitHub Enterprise** remote is out of scope (no PR pill) until per-host config lands. `KOLU_GH_BIN` is pinned by Nix in `nix/env.nix` and read lazily by `resolve.ts` (first call, not module load). A second forge (kolu#1240 phase 1) adds a `FORGE_ADAPTERS` entry and a `detectForge` arm; this adapter is unchanged.
