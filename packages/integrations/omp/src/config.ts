...
/** oh-my-pi's per-user agent directory — the root of its state tree, and the
 *  directory holding the `terminal-sessions` breadcrumbs kolu anchors
 *  detection on. This is the directory omp's own `PI_CODING_AGENT_DIR` env var
 *  overrides; its default is `~/.omp/agent` (omp 18.1.21's `getAgentDir()`,
 *  `packages/utils/src/dirs.ts`), itself a function of omp's `PI_CONFIG_DIR`
 *  (default `.omp`).
 *
 *  `KOLU_OMP_DIR` overrides it for kolu's DEFAULT-profile resolution only
 *  (tests/e2e fixtures, users who keep their whole omp config elsewhere). It is
 *  deliberately the sole kolu env knob and overrides nothing per-invocation:
 *  omp's own directory chain (`PI_CODING_AGENT_DIR`, `--profile` /
...
export const AGENT_DIR = process.env.KOLU_OMP_DIR
