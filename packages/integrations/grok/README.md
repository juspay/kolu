...
- **The `AgentAdapter`** (`agent-adapter.ts`) — the `kind: "grok"` contract the padi sensors consume: `resolveSessions`, `createWatcher`, and the `externalChanges` (active_sessions) install.
