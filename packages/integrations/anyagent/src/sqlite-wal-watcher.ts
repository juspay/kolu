...
/** Subscribe to a SQLite WAL file with a shared refcounted watcher.
 *  The underlying fs.watch is keyed by dbPath so multiple sessions
 *  observing the same database share one watcher. */
export function subscribeSqliteWal(
  dbPath: string,
  walPath: string,
  onChange: () => void,
  onError: (err: unknown) => void,
...
function tryWatchWal(
  ...
  try {
    ...
    if ((err as NodeJS.ErrnoException).code !== "ENOENT") {
      log?.debug({ err, path: walPath }, "sqlite wal fs.watch failed");
    }
    return null;
  }
}
...
function installWalWatcher(
  ...
  const direct = tryWatchWal(walPath, onChange, log);
  ...
  try {
    dirWatcher = fs.watch(dir, () => {
      if (promoted) return;
      const attached = tryWatchWal(walPath, onChange, log);
      if (!attached) return;
      promoted = attached;
      dirWatcher?.close();
      dirWatcher = null;
    ...
    log?.debug({ err, dir }, "sqlite db dir fs.watch failed");
  }
