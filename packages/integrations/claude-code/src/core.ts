...
/** Rel-paths under `$HOME` for the claude-code state directories. The
 *  `resolveClaudeCodeDirs` helper joins these with the executor's `$HOME`
 *  (so remote terminals get the *remote* HOME, not the controller's). */
const SESSIONS_REL = ".claude/sessions";
const PROJECTS_REL = ".claude/projects";

/** Test-time overrides — only consulted when the executor is the
 *  controller's local one (these env vars are process-scoped to kolu;
 *  remote helpers don't observe them). */
...
/** Back-compat: legacy local-only constants. Computed once at import using
 *  the controller's own `$HOME` / env overrides. Kept for any consumer that
 *  still wants the controller-local path without going through an executor
 *  (e.g. the one-shot transcript loader in `transcript.ts`). New code
 *  should call `resolveClaudeCodeDirs(executor)` instead. */
export const SESSIONS_DIR =
  LOCAL_SESSIONS_DIR_OVERRIDE ?? path.join(os.homedir(), SESSIONS_REL);
export const PROJECTS_DIR =
  LOCAL_PROJECTS_DIR_OVERRIDE ?? path.join(os.homedir(), PROJECTS_REL);
 ...
 *  filesystem. Honors the env overrides only for the controller's local
 *  process (the env vars are process-scoped, so remote helpers can't see
 *  them anyway). For everything else, resolves `$HOME` via
 *  `printenv HOME` and joins with the rel-paths.
 *
 *  Returns null when `$HOME` couldn't be resolved on the remote — callers
 *  treat that the same as "claude has never run here". */
export async function resolveClaudeCodeDirs(
  executor: Executor,
  log?: Logger,
 ...
 * file doesn't exist (the common case — most pids are not claude-code
 * sessions) or if the file is unreadable / malformed / missing required
 * fields.
 */
 ...
 * line is dropped because the byte cut likely sliced mid-line — the
 * 256 KB window plus modern Claude entry sizes (20–55 KB typical) means
 * the dropped line is virtually always either the start of the file (on
 * a small file) or a partial entry (on a larger one).
 */
...
// --- Sessions-dir subscription ---

/**
 * Subscribe to changes in the sessions dir for an executor. Used by
 * `externalChanges.install` so the orchestrator can re-resolve sessions
 * when a new `~/.claude/sessions/{pid}.json` appears (or disappears).
 *
 * The old refcounted shared-watcher dance is gone — the orchestrator
 * memoizes external-change installs by `{kind, executor}`, so a single
 * `executor.watch` per executor is enough. The first non-existent-dir
 * failure is logged at debug and returns a no-op handle; the
 * orchestrator gates `install` behind `isPresent`, so we should only
 * reach this when the dir exists.
 *
 * `onError` receives any exception thrown by `onChange` so that a
 * faulty listener doesn't break the underlying watch.
 */
...
/** Fetch the display summary from the Claude Agent SDK. Returns null on
 *  failure. Local-only — the SDK reads the controller's own ~/.claude,
 *  not a remote helper's. Remote sessions don't get summaries through
 *  this path; the watcher's title field is the fallback. */
