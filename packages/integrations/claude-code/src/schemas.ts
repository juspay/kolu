...
export const ClaudeCodeInfoSchema = z.object({
   ...
   *    or `ExitPlanMode`. The Claude Agent SDK buffers these tools' assistant
   *    messages in memory until the user resolves them, so the `tool_use` block
   *    isn't on disk while the prompt is pending — the JSONL classifier
   *    (`deriveState`'s `toolUseOrAwaitingUser`) reads the prior `end_turn` and
   *    reports `waiting` throughout the wait. #905 recovers the missing signal
   *    by recognizing the prompt on the *rendered screen* (`screen.ts`): the
   *    server's screen-scrape poll promotes `waiting → awaiting_user` while the
   *    dialog is visible, and the JSONL watcher lowers it again once the user
