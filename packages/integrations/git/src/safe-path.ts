...
/**
 * Write-side authority guard — the guard to use whenever a caller-supplied
 * `child` path is about to be *created or overwritten*. Unlike
 * `resolveExistingUnder` (which fails *open* on a missing realpath target — sound
 * for a read that would 404 anyway), a create-write must NOT fail open: a
 * dangling repo-local symlink, or a leaf under a directory symlink that escapes
 * the repo, has no realpath target yet, so `assertRealpathUnder` would pass it
 * and `writeFile` would follow the link and create the file *outside* the root.
 *
 * Instead we realpath the *parent directory* — which must already exist for any
 * write — and assert the parent's real path stays under the root. That rejects
 * an escaping intermediate directory symlink (the realpath of the parent lands
 * outside). A symlinked *leaf* (parent is in-repo, leaf is a link pointing out)
 * is not caught here; the caller MUST open the leaf with `O_NOFOLLOW` so the
 * kernel refuses to follow it. We return `{ abs, rel }` like `resolveUnder` so
 * the caller writes to the canonical lexical path under the now-verified parent.
 */
export async function resolveForWriteUnder(
  root: string,
  child: string,
  log?: Logger,
): Promise<GitResult<{ abs: string; rel: string }>> {
  const lexical = resolveUnder(root, child, log);
  if (!lexical.ok) return lexical;
  const parent = path.dirname(lexical.value.abs);
  let realRoot: string;
  ...
  try {
    ...
    // The parent directory must exist for a write — a missing/unreadable
    // parent is a real escape risk (dangling symlink, escaping directory
    // symlink with a non-existent leaf), so reject rather than fail open.
    log?.error({ root, child }, "safe-path: write parent not resolvable");
    return err({ code: "PATH_ESCAPES_ROOT", root, child });
  }
  ...
  if (rel.startsWith("..") || path.isAbsolute(rel)) {
    log?.error(
      { root, child },
      "safe-path: write parent escapes root (symlink)",
    );
    return err({ code: "PATH_ESCAPES_ROOT", root, child });
  }
  return lexical;
