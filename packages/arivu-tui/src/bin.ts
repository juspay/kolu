 ...
 *
 *   arivu-tui            an OpenTUI dashboard of ONE endpoint — one row per
 ...
 *   arivu-tui fleet      a LIVE multi-host board — every terminal across N hosts
...
const argv = cli({
  ...
  // `arivu-tui fleet` is the LIVE multi-host board (PR2b). The bare `arivu-tui`
  ...
  commands: [
    command({
      ...
      help: {
        description:
          "A LIVE multi-host board: every terminal across N hosts in one screen, with every agent awaiting you floated to the top. Repeat --host to add machines; the local arivu is included unless --no-local. Flags go AFTER `fleet`.",
      },
      flags: {
        host: {
          type: [String],
        ...
        noLocal: {
          type: Boolean,
          description: "exclude the local arivu from the fleet.",
          default: false,
        },
        by: {
          type: String,
...
/** `arivu-tui fleet` — resolve the host list, then either dump one-shot JSON or
...
async function main(): Promise<void> {
  // `arivu-tui fleet` is its own world (the live multi-host board); everything
