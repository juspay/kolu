...
| `buildHostRegistry({ buildEntry })` | **The N-host fan-out.** A keyed `Map<host, { session, handler, cells? }>` a `?host=` upgrade dispatcher reads, with `add`/`remove`/`reconnect`/`recheckAll` + per-host socket eviction. The app supplies `buildEntry(host) → { session, handler, cells? }` (provisioning + its oRPC handler + optional entry-scoped cells) and an optional `persist` hook; the registry is generic over the handler type (no `@orpc/server/ws` dep). **`cells?`** is a per-entry served-cell fragment DERIVED from that entry's session (e.g. kolu's per-host padi diagnostics) — the registry disposes it beside `session.destroy()` at EVERY teardown site (`remove`, a persist-reject `add` rollback, `destroyAll`), so a per-host sampler's timers + subscriptions share the entry's exact lifetime and can't leak past it. The registry stays ignorant of *which* cells or what they carry — the same discipline it applies to `session` (it knows only `.destroy()`). |
...
- **Fan out over N hosts**: a parent that dials *many* hosts (a browser fleet view) keeps them in `buildHostRegistry` — one `{ session, handler, cells? }` per host, with `add`/`remove`/`reconnect`/`recheckAll` + socket eviction — and dispatches a `?host=<id>` WebSocket upgrade to the right handler. Each host's `buildEntry` wires `makeSession({ connectOnce: sshConnector(...) })` + a `pumpRemoteSurface`-fed re-serve + its oRPC handler, and (optionally) a per-entry `cells` fragment derived from that host's session — the registry disposes it with the entry, so per-host served diagnostics never outlive their host. The two consumers today are [`drishti`](https://github.com/srid/drishti)'s process monitor and `@kolu/pulam-web`'s terminal fleet — both consume one shared copy rather than re-rolling the loop + registry.
