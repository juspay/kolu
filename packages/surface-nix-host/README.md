...
| `mirrorOnce({ source, client, sink, … })` | **One live client's mirror lifecycle** — the body `pumpRemoteSurface` (per spawn) and a non-`HostSession` reconnect loop (e.g. `@kolu/pulam-web`'s local-kolu mirror over a plain WebSocket) both need: mirror the surface into `sink`, publish the `liveProcedures` / `liveClient` holders, block on the mirror until the link dies, then — in a `finally`, so a `SinkError` can't skip it — clear the holders and fire `onLinkDown`. Encodes the #1549 (rebuild-from-snapshot) and #1564 (clear-on-death) ordering invariants once. The caller keeps its OWN outer loop (the client source — a session cursor vs a reconnecting link — and the connection-state model differ); only this ordering body is shared. Optional `signal` threads cancellation for a loop that owns teardown (the pump omits it; the session does). |
