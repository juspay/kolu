...
| `relayHoldOpenStream(policy, member, holder, select)` · `relayFailThroughStream(policy, member, holder, select)` | The two per-subscriber input-keyed stream relays `reServeSurface` routes to. `relayHoldOpenStream` HOLDS the downstream open across a respawn (rebinds to the next spawn — for a `value` pulse / event); `relayFailThroughStream` forwards the current spawn's stream 1:1, propagating its clean end AND its error, and NEVER rebinds (for a `delta` byte stream). `member` is type-constrained to the matching policy class, so **holding open a byte stream is a compile error** — the splice-a-snapshot-into-a-live-terminal corruption is unspellable, not a rule to remember. |
