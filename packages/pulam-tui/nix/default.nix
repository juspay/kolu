# pulam-tui viewer — Bun-built tree (pulam P3 PR1).
#
# Produces $out/lib/pulam-tui ready to be exec'd by the `bun` wrapper in the
# repo-root `default.nix`. Co-located with the viewer it builds (this whole
# packages/pulam-tui/nix/ tree) so the package is self-contained. This is the
...
# to drift. It carries packages/pulam-tui (the viewer) and the five hydrated
# @kolu packages.
...
let
  ...
  # `!packages/pulam-tui/nix/**` — so pnpm, whose workspace glob is packages/**,
...
stdenv.mkDerivation (finalAttrs: {
  ...
  # build time — see packages/pulam-tui/build.ts for why.
  ...
  # packages/pulam-tui/build.ts for what gets bundled vs left native and why.
  ...
  buildPhase = ''
    runHook preBuild
    cp -rL ${koluSrc}/packages/pulam-tui/src ./src
    cp -L ${koluSrc}/packages/pulam-tui/build.ts ./build.ts
  ...
  installPhase = ''
    runHook preInstall
    mkdir -p $out/lib/pulam-tui
    cp -rL dist $out/lib/pulam-tui/dist
    cp -r node_modules $out/lib/pulam-tui/
    cp package.json $out/lib/pulam-tui/
    runHook postInstall
  '';
  ...
  installCheckPhase = ''
    ...
    ${bun}/bin/bun $out/lib/pulam-tui/dist/bin.js --help > /dev/null
  ...
  # the wrapper consumes `pulamTuiBuilt.entryPath` — one value, so the two files
  # cannot silently desync (no cross-file string to keep in step, no build-time
  # guard).
  passthru.entryPath = "${finalAttrs.finalPackage}/lib/pulam-tui/dist/bin.js";

  meta = {
    description = "pulam-tui viewer — Bun-built tree (viewer source + hydrated @kolu/* + node_modules)";
