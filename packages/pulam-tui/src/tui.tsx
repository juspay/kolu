/**
 * The OpenTUI/Solid view for pulam-tui — pulam P3 PR2a.
 *
 * `pulam-tui` (no args) opens a compact, truecolour table: one row per terminal,
...
export function AwarenessTable(props: {
  ...
  return (
    <box flexDirection="column" padding={1}>
      <text fg={TITLE}>
        {`pulam  ·  ${count} terminal${count === 1 ? "" : "s"}  ·  ${props.clock()}  ·  Ctrl-C to quit`}
      </text>
      ...
      {count === 0 ? (
        <text fg={TONE_COLOR.muted}>
          no terminals — is kaval running, with pulam watching it?
        </text>
      ) : (
        <For each={props.rows}>
          {(r) => (
