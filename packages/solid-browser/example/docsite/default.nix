...
# `nix build ..#solid-browser-example-docsite`
# (and CI's `ci::solid-browser-example-nix`) keeps the
...
#
# Inputs come from the independent Solid Browser example flake, which reuses
# the canonical workspace pnpm fetch and source fileset from `nix/workspace.nix`:
