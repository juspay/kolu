...
// The lifetime union, its serializable projection, and the fold between them
// live in the LIFETIME LEAF (`daemonLifetime.ts`) — imported here and
// re-exported below, so this file stays the in-repo door while a browser-safe
// consumer reaches the published type WITHOUT compiling this module. See that
// file's header for what that saved.
import {
  type DaemonLifetime,
  type DaemonLifetimeInfo,
  lifetimeInfo,
} from "./daemonLifetime.ts";

export { type DaemonLifetime, type DaemonLifetimeInfo, lifetimeInfo };
