...
The **durable-daemon spine**: the lifecycle mechanism every long-lived process that owns a unix socket and serves a typed [`@kolu/surface`](../surface/README.md) repeats — both halves of the daemon *binary*. **Serve it** — pid-gated single-instance entry, a `gate → serve → teardown` skeleton, a lifetime policy knob. And **front it** over ssh-stdio so a remote session outlives the link (`frontDaemonOverStdio`, the durable counterpart to `serveOverStdio`). It also carries the shared **daemon-identity** recipe (`readBakedIdentity` + its Nix half `mkDaemonIdentity` in `nix/daemon-identity.nix`) and the **convergence comparators** the supervisor's convergence kit imports up (#L3) — daemon-identity code that must sit on the daemon side of the arrow so the supervisor keeps its zero-`@kolu/surface` boundary. A zero-`kolu-*`-dependency package.
...
| `readBakedIdentity(prefix)` | The ONE recipe both daemons' build-id reads share (kaval's `KAVAL_*`, padi's `PADI_*`), replacing the per-package `buildId.ts` twins: reads `<PREFIX>_BUILD_ID` / `<PREFIX>_COMMIT_HASH` into `{ staleKey, navigableCommit }`, `""` off-nix. Its Nix half is `nix/daemon-identity.nix`'s `mkDaemonIdentity` (the `hashString`-over-behavioral-fileset + `--set` bake). |
| `ConvergenceIdentity` · `contractIsNewer` / `contractIsCompatible` / `buildIdMatches` | The two convergence axes a supervisor compares. Contract versions are **ordered** (`contractIsCompatible` reuses `@kolu/surface`'s predicate unchanged); build ids are **match-only** — no ordering (store hashes don't order). They live here (not the supervisor) so the supervisor keeps its zero-`@kolu/surface` boundary; its convergence kit imports them up. |
...
> **Only code in the daemon *binary* — serve *and* front — may live here.** Both halves run in the one daemon binary a restart loads: the serve half (`daemonMain`/pid-gate) runs *in* the daemon process, and the front half (`frontDaemonOverStdio`) runs in the per-link proxy reached from that binary's `--stdio` dispatch — both reached from the consumer's daemon entries (`bin.ts`/`index.ts`). What stays out is the **supervisor** half — it runs in the *client* process, never the daemon, so it lives in its **own** `@kolu/surface-daemon-supervisor` package (born in kaval B2, not a subpath here): the package boundary is the hash boundary, with no subdir glob to mis-scope.
>
> **Currency-slice caveat (#L3).** This package runs in the kaval binary but is **NOT** hashed into kaval's **currency** key (`KAVAL_BUILD_ID`): kaval's staleKey drives only its human "update available" nudge, and acting on it *recycles kaval — killing live PTYs*, so the spine is a deliberate stable **leaf** for it (`default.nix`'s `kavalIdentity.behavioralFileset`, pinned by `kaval/src/buildId.closure.test.ts`). A contract-compatible spine change is behaviorally interchangeable by the wire contract's definition (`PTY_HOST_CONTRACT_VERSION`, in kaval, *is* hashed → recycle-on-skew converges a wire-breaking change), so keying kaval currency on the spine over-fires the nudge (the `zest` incident). **padi** keeps the spine in its full closure (`PADI_BUILD_ID`) — its staleness response is a cheap auto-drain, so over-firing is harmless. The shared identity recipe (`readBakedIdentity` / `mkDaemonIdentity`) and the convergence comparators added here in L3 are, likewise, spine — a change to them flips padi's staleKey but not kaval's currency.
