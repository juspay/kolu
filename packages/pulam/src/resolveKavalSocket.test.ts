/**
 * Unit tests for `resolveKavalSocket` — which kaval the pulam daemon dials. The
 ...
 * real seeded sockets); pulam only owns RENDERING the ambiguous case as its own
 * `--kaval`-flavored error. So here we mock that seam and assert pulam's three
