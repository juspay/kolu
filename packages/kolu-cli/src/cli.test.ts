...
describe("kolu command tree", () => {
  ...
  describe("an omitted switch is its OFF position, not a missing flag", () => {
    ...
    it("`send` without `--submit` still reaches the endpoint check", async () => {
      // The same pin for the switch THIS branch added. `--submit` arrived while
      // the tree was still on beta.106, where a bare `Flag.boolean` defaulted to
      // false; merging rc.110 would have made every plain
      // `kolu send <id> "text"` refuse itself with "Missing required flag:
      // --submit" — the flag inverting its own meaning, and a usage error
      // for the ORDINARY send rather than for the new one.
    ...
    it("`create` without `--json` still reaches the endpoint check", async () => {
      // `create` grew `--message` on this branch beside its existing `--json`
      // switch; this is the same absent-switch proof for the verb the brief
      // rides on.
      const err = await failureOf([
        "create",
        "--toplevel",
