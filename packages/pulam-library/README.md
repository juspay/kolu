...
**Assembly.** `createPulam(deps)` is the **one assembly** (R9·lib) that turns a
dialed kaval into a live, served `terminalWorkspace` surface. It owns the
per-terminal lifecycle — seed → `makeAwarenessSink` (the publish sink) →
`bridgeKavalTaps` (the tap feed) → `startAwareness` (the sensors) → the
