 ...
 * `runProgress`/`runCapture`. An unowned child is UNSPELLABLE: omitting the policy (or
...
// @ts-expect-error — policy is required on runProgress too.
void runProgress("nix", ["copy"], { env: {} });

// With a policy, both typecheck.
void runCapture("nix-store", ["-q", "--outputs"], {
  policy: { kind: "deadline", ms: 1000 },
});
void runProgress("nix", ["copy"], {
