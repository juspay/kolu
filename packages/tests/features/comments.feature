Feature: Comment mode (annotate files, copy to clipboard)
  ...
  Scenario: comments tray is hidden by default and revealed by the toggle
    Given a Code tab in "browse" mode showing file "src/a.ts" with content "alpha\nbeta\ngamma\n"
  ...
  Scenario: add a code comment, copy to clipboard, tray clears
    Given a Code tab in "browse" mode showing file "src/a.ts" with content "alpha\nbeta\ngamma\n"
    When I enable comment mode
    And I open file "src/a.ts" in the Code tab
    And I click the line number 2 in the file content
    ...
    And the clipboard text should contain "src/a.ts  L2"
