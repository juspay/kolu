...
describe("parseSshConfig", () => {
  ...
  it("parses a single Host block with all fields", () => {
    const cfg = `Host srid-remote-terminal
...
`;
    const parsed = parseSshConfig(cfg);
    // Aliases a + b get bare defaults; only the last alias (c) absorbs the body.
    expect(parsed).toHaveLength(3);
    expect(parsed[0]).toEqual({ alias: "a", hostname: "a" });
    expect(parsed[1]).toEqual({ alias: "b", hostname: "b" });
  ...
  it("ignores comments and blank lines", () => {
    const cfg = `# global comment

# This host is for testing
Host test
  # inline comment
  HostName test.example.com
`;
    expect(parseSshConfig(cfg)).toEqual([
      { alias: "test", hostname: "test.example.com" },
    ]);
  });

  it("accepts both space- and `=`-separated key/value syntax", () => {
    const cfg = `Host eq
...
`;
  ...
  it("rejects a non-numeric Port without crashing", () => {
    const cfg = `Host bad
