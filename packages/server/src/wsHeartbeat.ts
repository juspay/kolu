 ...
 * monkey-patched onto the socket object. Only the stale-tab gate
 * (`@kolu/surface-app/server`) closes sockets BEFORE upgrade; those are mid-close
 * and never register here, so #1231's protection is untouched.
 */
 ...
 *  `OPEN` are skipped — a stale tab the gate closed at the handshake is mid-close
 *  and is neither pinged nor terminated here. Pure over its injected deps (no
 *  timers, no `wss`), so it's unit-testable without a real server. */
