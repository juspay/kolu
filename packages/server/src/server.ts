...
// Forward the nix-shell whitelist to the PTY-host daemon (a separate process
// the supervisor spawns with `env: process.env`) — the daemon spawns the PTY
// shells now, so it needs the same devshell-env filter kolu-server resolved.
...
// Connect to (or spawn) the `kolu --stdio` daemon, then reattach the PTYs it
// still owns from a previous kolu-server — BEFORE the HTTP port binds, so
// reattached terminals are registered (and their fresh provider DAG started)
// by the time the first client connects and its `attach`-by-id streams arrive.
try {
