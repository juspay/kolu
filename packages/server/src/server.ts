/**
 * kolu-server boot — the HTTP/WS server process (the non-`--stdio` branch of
 * `./index.ts`'s dispatcher).
 *
 * Since #951 R4c the PTYs no longer run in this process: they run in the
 * `kolu --stdio` daemon (`./agent/main.ts`), which this process spawns and
 * connects to over `$KOLU_STATE_DIR/agent.sock`. The R4c additions to the
 * boot sequence, all BEFORE the HTTP port binds:
 *
 *   1. `ensureDaemon()` — connect to the running daemon, or spawn one (it
 *      survives a kolu-server restart, so this is the reattach substrate).
 *   2. `reattachLocalTerminals()` — re-register the PTYs the daemon still
 *      owns, matched to the saved session by id (NO spawn).
 *   3. `startLocalTerminalBridge()` — subscribe to the daemon's
 *      `agentMetadata` stream; its snapshot replays warm metadata onto the
 *      just-reattached entries.
 *
 * Running these before `serve(...)` means reattached terminals are in the
 * registry the moment the first browser connects — its retried
 * `attach`-by-id streams reconnect to the same shells with scrollback intact.
 */
...
// Hand the resolved nix-shell whitelist to the daemon we spawn: it's a
// separate process, so kolu-pty's module-level whitelist state set just
// above doesn't reach it. The daemon reads this and calls
// `configureNixShellEnv` itself (see `agent/main.ts`). Absent in production
// → the daemon's production safety net applies, same as here.
if (argv.flags.allowNixShellWithEnvWhitelist != null) {
  process.env.KOLU_NIX_SHELL_WHITELIST =
    argv.flags.allowNixShellWithEnvWhitelist;
}
ensureKoluRoot();
...
// --- Local PTY-host daemon (R4c) ---
// Connect to (or spawn) the daemon, reattach the PTYs it still owns from a
// previous kolu-server, then subscribe to its metadata stream — all before
// the HTTP port binds, so reattached terminals are registered by the time
...
try {
  ...
  log.fatal({ err }, "local PTY-host daemon failed to start");
  process.exit(1);
}
