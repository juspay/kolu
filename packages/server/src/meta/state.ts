 ...
 *  position.
 *
 *  `hostId` defaults to `"local"` so the field is always concrete on
 *  fresh metadata; the caller in `terminals.ts` overwrites it for
 *  remote spawns before publishing the metadata snapshot. */
