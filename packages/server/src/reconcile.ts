 ...
 *  local reconcile, and vice versa). Defaults to `LOCAL_LOCATION`: today every
 *  saved record is local and `live` is the local daemon's, so the narrow keeps the
 *  whole session and the partition is unchanged. F-REMOTE reconciles each dialed
 *  host with its own location. */
