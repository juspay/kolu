...
const { router: surfaceRouterFragment, ctx: surfaceCtxBuilt } =
  // The whole surface-app server side in ONE call — `implementSurfaceApp` merges
  // surface-app's buildInfo cell + `surfaceApp.info` probe impls into kolu's own
  // `implementSurface` deps, runs `implementSurface`, AND flows the buildInfo
  // cell's `connect` internally. kolu's build identity has a boot-time-async axis
  // (the pty-host's `system.version` over the in-process link), so that internal
  // connect is load-bearing: it republishes the resolved `{ commit, ptyHost }`
  // through the same fragment once it settles — server-pushed, so a client
  // connected before the pty-host answered fills in its `srv · pty` rail without a
  // reload. kolu passes only its OWN cells/procedures; no hand-spread of the
  // surface-app halves, no hand-written `ctx.cells.buildInfo.set`.
  implementSurfaceApp(
    surface,
    // The build-identity cell's server fragment (skew axis), extended with kolu's
    // pty-host column, PLUS the `surfaceApp.info` restart probe pinned to kolu's
    // boot UUID. `commit` is kolu's single source (`serverCommit` ←
    // `KOLU_COMMIT_HASH`); the pty-host axis is the boot-time-async source — the
    // in-process pty-host reports its identity async (via `system.version`), so it
    // lands as a `Partial<KoluBuildInfo>` patch after the cell is seeded with
    // `{ commit }`. A failed probe leaves `ptyHost` undefined (the fragment
    // swallows it); the rail's column shows `—`.
    surfaceAppServer<KoluBuildInfo>({
      buildInfo: async () => {
      ...
      // Pin it to the existing boot UUID (`serverProcessId`) so the value is
      // stable within a process and changes on restart. Composed, not hand-written.
