/**
 * PR-1 — the terminal lifecycle honors `HostLocation` at CREATE.
 *
 * Two halves of the create seam, exercised pty-host-free (no kaval daemon is
 * booted in the unit env, so a create's spawn RPC rejects on a later microtask —
 * the sync-shadow entry the assertions read is already registered):
 *
 *   - `resolveCreateLocation` — the pure inherit/reject rule the create RPC applies:
 *     a top-level terminal takes the requested location (default local); a
 *     sub-terminal INHERITS its parent's host and REJECTS an explicit child
 *     location that disagrees (BAD_REQUEST).
 *   - `createTerminal` selects the endpoint BY location (a remote location has no
 *     endpoint yet, so it throws — never silently degrades onto the local PTY), and
 *     a created terminal's location rides through to the saved session, so a saved
 *     `location` round-trips create → snapshot → re-create.
 */
...
describe("resolveCreateLocation — inherit from parent, reject a mismatch", () => {
  ...
  it("a sub-terminal whose explicit location DISAGREES with the parent is BAD_REQUEST", () => {
    // The wrong-host child is rejected loudly, never spawned on a different host
    // than its parent (one PTY tree, one kaval).
...
describe("createTerminal — location selects the endpoint and round-trips to disk", () => {
  ...
  it("a local create round-trips its location through to the saved session", () => {
    ...
    // …and that location rides into the saved session record (the persist half of
    // the create→restore round-trip).
    const saved = snapshotSession().terminals.find((t) => t.id === info.id);
    expect(saved?.location).toEqual(LOCAL_LOCATION);

    // Feeding the saved location back into a create — exactly what session restore
    // does (forwarding `t.location`) — re-spawns on the same host.
  ...
  it("a remote location throws (no remote endpoint yet) rather than degrading onto the local PTY", () => {
    // The `{kind:"remote"}` resolver arm must fail loudly until F-REMOTE dials one —
    // a remote terminal silently served by the local endpoint would be a wrong-host
    // bug, not a graceful default (the no-fallbacks stance).
    expect(() =>
