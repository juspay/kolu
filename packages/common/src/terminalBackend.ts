...
export interface TerminalHandle {
   ...
   *  socket-backed handle (R4c daemon, R-2 ssh) reads it over the wire, so
   *  the return widens to a Promise — callers `await` it. */
...
export interface TerminalBackend {
   ...
   *  unregister from the shared registry. Sole termination path. A
   *  socket-backed backend awaits the daemon's kill confirmation before
   *  unregistering (so a failed kill can't orphan the PTY), hence the
   *  Promise. */
  killTerminal(
    id: TerminalId,
  ): TerminalInfo | undefined | Promise<TerminalInfo | undefined>;

  /** Drain and dispose every terminal owned by this backend. Used by
   *  the e2e harness between scenarios. */
  killAllTerminals(): void | Promise<void>;
   ...
   *  them loses and duplicates nothing. A socket-backed backend opens the
   *  attach stream over the wire, so the return widens to a Promise. */
  attach(
    id: TerminalId,
    signal: AbortSignal | undefined,
  ): TerminalAttachment | Promise<TerminalAttachment>;
