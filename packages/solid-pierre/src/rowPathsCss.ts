 ...
 *
 *  A filename may legally contain `"`, `\`, or a raw line terminator — the
 *  filesystem allows every byte but `/` and NUL, and a `-z` listing hands them
 *  through verbatim. Each would otherwise terminate or mangle the selector
 *  string, and the blast radius is wider than the one bad row: callers
 *  concatenate this output with their other rules into a single `replaceSync`
 *  payload, so one unescaped path silently drops every sibling rule in that
 *  sheet.
 *
 *  Backslash goes first (or it would double-escape the escapes we add), then
 *  the quote, then every character CSS Syntax counts as a newline inside a
 *  string — LF, CR, and FF — as numeric escapes. The trailing space in `\\A `
 *  is REQUIRED and not a typo: a CSS numeric escape ends at the first non-hex
 *  character, so `\A` immediately followed by a hex digit in the filename would
 *  swallow it into the code point. */
