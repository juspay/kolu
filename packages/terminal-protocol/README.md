...
| `headlessReplies` | `shouldForwardHeadlessReply` — the server-side forward/drop policy for replies the headless mirror generates. | `@kolu/pty-host` |
| `deviceQueries` | `ANSWERED_DEVICE_QUERIES` / `SILENT_DEVICE_QUERIES` — the matrix as data. | `@kolu/pty-host` contract tests |
| `bracketedPaste` | The `?2004` paste delimiters (`ESC [200~` / `ESC [201~`). | kolu-tui escape scanner · kolu-server paste injection |
| `snapshotReset` | `SNAPSHOT_TTY_RESET` — the reciprocal reset for every mode a replayed `@xterm/addon-serialize` snapshot can switch on. **Audit on every xterm/serialize bump.** | kolu-tui `attach` restore |

What deliberately does *not* live here: the ssh-style `~.` escape state machine (an attach-control concern, `pty-tui/src/escape.ts` — it imports only the paste markers), the OSC 7/2/633 *emission* hooks (`packages/integrations/pty` — coupled to shell-rc mechanics), and the key-input synthesis in the browser (UI-coupled).
