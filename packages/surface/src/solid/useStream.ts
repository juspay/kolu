 ...
 * member ref (`client.streams.gitStatus`'s bound `get`, etc.). The hook applies
 * the framework's retry fence per call so transport drops re-subscribe
 * transparently and the next yield is a fresh full snapshot.
 *
 * When the input changes, the previous subscription tears down (its fiber is
 * interrupted) and a fresh one starts; value resets to `undefined` between input
 * change and first yield.
 *
