 ...
 * subscribed before the add never learns the key exists. (kolu's own client dodged
 * it by sourcing membership from a sibling and reading per-key values, so the bug
 * stayed latent until pulam-web's localhost mirror — R9a — consumed the `keys`
 * stream generically.) The fix tracks the framework's own broadcast set, so the
 * membership snapshot fires on a key's first upsert regardless of the backing.
 *
...
describe("served collection keys-stream — membership for a registry-backed projection", () => {
  it("broadcasts a key added AFTER a consumer subscribed (the R9a membership bug)", async () => {
