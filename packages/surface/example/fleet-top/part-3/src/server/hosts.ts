 ...
 *  stays `warming` and retries, it does not become `failed`. `connected` propagates
 *  the wall-clock offset `makeSession` measures off the reserved `system.clockNow`
 *  at admit (`null` until the first probe lands → `connecting`, so a skewed host's
 *  timestamps are never mis-mapped through a fabricated 0). */
