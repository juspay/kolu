...
A second **framework-reserved procedure**, `surface.system.identity` (`@kolu/surface/identity`), is the identity twin of `live` — same reserved `system` namespace (so reserving `identity` beside `live` can never clobber an app's own `system.*`), same inject-by-`defineSurface` / auto-answer-by-`implementSurface` story. It answers *"who is the far end"* with the server's own identity: a server that knows its build DECLARES the baked triple — contract version, build id (the convergence currency), and a `dev`-vs-`commit` source axis — via `implementSurface(surface, deps, { identity })`; everyone else omits it and the framework answers `anonymous`. This makes identity a UNIVERSAL question every serving process answers — the axis a consumer's `Session.identity()` / a mirror reads to report a bound server's uptime, contract version, and build — generalizing what `@kolu/surface-app`'s `identity.info` did per-app into one framework member. NO NULLS: `SurfaceIdentity` is ONE sum — `disconnected` (no live link, a session between dials) · `anonymous` (connected, no declared build — drishti/odu) · `identified` (connected, build declared — padi) — so the reader must branch and impossible states (identified-but-no-`startedAt`, a commit that might-be-dev-might-be-real) can't be written. `probeSurfaceIdentity(client)` is the one-liner that calls it.
...
It's a **FACT, not a verdict**: a reactive accessor that enrols every **framework** subscription the client creates — each cell (`"<cell>"`), a collection's keys-stream (`"<coll>.keys"`) and each per-key value sub (`"<coll>[<id>]"`) — or, for an opt-in batched `deltas` collection, its single folded sub (`"<coll>.deltas"`), and every stream (`"<stream>"`). A _raw_ stream (one that owns its own loop) is NOT auto-enrolled — it joins the fact through `client.rawStream`, the structural path below, which makes forgetting impossible rather than merely discouraged. Each sub's error flows through its own *self-clearing* reactive `error()`, so the fact **un-latches by construction** — a transient blip disappears from `health()` the instant the stream re-delivers. `live` is no longer just the transport's liveness — it is the full **conjunction**: transport-live ∧ every readiness-cell's predicate (sub errors stay in `subs`, never folded into `live`). The transport leg is the socket signal threaded in (`@kolu/surface-app`'s `connectSurface` / `connectSurfaces` seam mints it with `createLiveSignal` — a watchdog-backed `LiveSignal` that reads `status() === "live"` — so a `down`/`reconnecting` socket reads `live: false` and the gate reads `connecting`, never a confident `ready` over a dead link); only an in-process `directLink` — which has no wire to silently half-open — contributes a constant `true` (a `stdioLink`/`unixSocketLink` is itself a wire link that *can* half-open, so like a websocket it's branded and refused bare, demanding the same watchdog-backed `LiveSignal`).
...
Don't hand-wire that fold for a socket-backed app: reach for **`connectSurfaces({ surfaces, heartbeat?, ...SurfaceSocketOptions })`** (`@kolu/surface-app/solid`) — one seam that opens one socket + one `createLiveSignal` (which wires the half-open watchdog and mints the branded `live`) + `surfaceClients`, and returns `{ ws, echo, clients, status, health: () => surfaceClientsHealth(clients), dispose }`. Because the half-open **watchdog is intrinsic to the seam**, `health().live` reflects a silent half-open (laptop sleep, wifi roam) however the socket was built. The half-open-blind transport leg is not merely discouraged but **unspellable** over a wire: **every wire link (`websocketLink`, `stdioLink`, `unixSocketLink`) marks its clients half-openable, and `surfaceClient`/`surfaceClients` THROW unless handed a watchdog-backed `LiveSignal`** — so a missing `{ live }` AND a bare `() => true` (or an open/close-only `() => socketStatus() === "live"`) are both refused; the brand can be minted *only* by `createLiveSignal` (which wires the watchdog first), so there is no path to a green `live` over a dead wire. Only an in-process `directLink` can't half-open, so any `{ live }` (or none) is honest there; a `stdioLink`/`unixSocketLink` is a wire link, branded and refused bare exactly like a websocket. Over a socket you either build through `connectSurface` / `connectSurfaces` (the watchdog can't be forgotten) or, hand-building the raw seam, mint the `{ live }` with `createLiveSignal` (the example's path) — there is no silent no-heartbeat hand-built path left to spell.
...
surfaceClient<S, Rpc>(surface, transport): SurfaceClient<S, Rpc>
  ...
  // `directLink`. A bare WIRE link — `websocketLink` / `stdioLink` / `unixSocketLink` (any of which
  // can silently half-open) — THROWS: there is no `{ live }` seam to thread a blind accessor through,
  // and the branded handle is unspellable without the watchdog (an un-reflectable WeakSet, no
  // exported stamper). Only an in-process `directLink` can't half-open, so it is passed bare (its
  // transport leg is constant-`true`, honest).
...
- **A per-key collection `get` for a not-yet-existing key HOLDS OPEN — it does not error.** A collection's membership is dynamic by design (the mirror's `initialKeys` reconcile treats it so), so a consumer watching a fixed key may subscribe BEFORE the key is born. The `get` handler subscribes-before-snapshot (like `keys`), yields the current value only if the key is present, then forwards; an absent key yields nothing and holds open until the key's first upsert. This is a **deliberate, tested** semantic: the alternative — throwing "key not found" on the first snapshot — surfaced to a browser as a non-retriable `ORPCError` (`STREAM_RETRY` never retries an `ORPCError`) that KILLED the standing subscription, so a key born after the subscription opened (kolu-server booting with an empty re-serve mirror) never reached the consumer until a full page reload (the gray Kaval chip, [#1681](https://github.com/juspay/kolu/issues/1681)). "Absent" is a recoverable waiting state (`undefined` until the first value), not an error; a key that never appears simply holds open yielding nothing, exactly as a `keys` subscription to an empty collection does.

- **A cell's server-internal `set` can `{ force }` past its `equals` dedup.** `ctx.cells.<key>.set(v)` de-dups a no-op write by `equals`. `set(v, { force: true })` bypasses that for ONE write — used by `reServeSurface`'s rebind epoch: a fresh spawn re-serving a value EQUAL to the pre-drain one must still cross once, or a downstream holder can't tell "rebound and confirmed" from "stale". The re-serve forces the FIRST fold of each spawn and keeps steady-state dedup for the rest.
