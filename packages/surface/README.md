...
- **Procedures are total, not opt-in.** A forwarding stub has no standing cost (it's a typed passthrough to `client.surface.<ns>.<verb>`), so `.procedures` always carries *every* procedure the surface wires — `{}` if it has none. The stubs are bound to the `client` you passed, so a stub is live for that client's lifetime; over a reconnecting transport that hands out a fresh client per spawn, forward through whatever yields the *current* client. Because the stubs are exactly the shape `implementSurface`'s `procedures` deps expects, grafting them into a re-serve makes `serve ∘ mirror ≈ identity` — the location-transparency remote terminals rests on.
