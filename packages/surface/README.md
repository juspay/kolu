...
- **A standing subscription re-subscribes on a clean completion, not only a transport drop.** `ClientRetryPlugin` re-invokes the source on a transport ERROR, but a held-open surface stream can also END cleanly — the SERVER completes it, e.g. a re-serve across a padi rebind (a fresh spawn re-mirrors onto the same downstream router). Left there, the standing subscription would go dead and the post-rebind republished snapshot never arrives (the gray Kaval chip, [#1681](https://github.com/juspay/kolu/issues/1681)). `createSubscription` / `createReactiveSubscription` therefore re-open the source on a clean completion too — rate-bounded (a 1s delay, the reconnect cadence) so a server that pathologically completes in a loop can't spin a republish storm, and terminal on a thrown (non-retriable) error. The retained value + `pending`/`error` are untouched across the re-subscribe, so recovery is a silent refresh in place.

- **The re-serve's rebind epoch forces one republish past the cell equals-gate.** A cell's server-internal `ctx.cells.<key>.set` de-dups by `equals` (a no-op write doesn't republish). But when a re-serve rebinds to a fresh spawn (`reServeSurface`), a cell value that comes back EQUAL to the pre-drain one must still cross once — otherwise a downstream holder can't tell "rebound and confirmed" from "stale". `set(v, { force: true })` bypasses the de-dup for that ONE write; the re-serve forces the FIRST fold of each spawn and keeps steady-state de-duplication for the rest.

- **The consume + reconcile-or-assign branches are shared.** `createSubscription` and `createReactiveSubscription` share BOTH the re-subscribing consume loop (`consumeReSubscribing`) and the store-write branch (`writeWrappedValue`: `reconcile` for objects/arrays for fine-grained reactivity, plain assignment for primitives). Each used to be duplicated with a "keep in sync" comment between the two; now each lives in one place.
