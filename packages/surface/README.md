...
| `terminalListCell` | Live terminal list — drives the dock, canvas tile set, mobile swipe order | `server` | _server-only_ (via `terminal.create` / `kill` mutations) | `inMemoryStore` (terminal registry is canonical) |
...
| **One-shot queries** — request/response, no subscription dimension | `server.info`, `host.list`, `terminal.screenState`, `terminal.screenText`, `terminal.exportTranscriptHtml` | `await client.X.Y(input)` |
