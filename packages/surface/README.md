...
> **Breaking change (R7).** `mirrorRemoteSurface` used to return `Promise<void>` — you would `await mirrorRemoteSurface(...)` to wait for the mirror to settle. It now returns the **plain handle** `{ procedures, done }`. A bare `await mirrorRemoteSurface(...)` therefore **no longer waits** (you'd be awaiting a non-thenable object, which resolves immediately) — the settle is `await mirrorRemoteSurface(...).done`. This is not type-caught when the result is discarded — and a rename wouldn't catch it either, since no TypeScript construct makes `await object` an error — so when migrating a consumer (drishti) **audit every call site**: every old `await mirrorRemoteSurface(...)` becomes `await mirrorRemoteSurface(...).done`. That migration rides the [surface→drishti merge-gate](../../.claude/rules/surface.md) (drishti's CI is the backstop). No back-compat thenable is provided on purpose — the fail-fast rule prefers a deliberate per-site migration over a shim that silently preserves the old await, and the handle's non-thenable contract is pinned in CI by a unit test so the shim can't creep back.
