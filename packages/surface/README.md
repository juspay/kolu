...
|---|---|---|
| `terminalMetadataCollection` | Per-terminal metadata (cwd, git, PR, agent state, foreground process, last-activity timestamp for switcher recency, **right-panel per-terminal state** — activeTab, codeMode, per-mode selected file — and sub-panel state) — each terminal's tile chrome and inspector reads its own key | _server-only_ (the host padi folds the producer's observation stream in `terminalEndpoint/local.ts`: `commitSnapshot` writes the observed half and `updateMemory` the memory half — each gated by its own delta (`authoredFactsEqual`), and the session autosave `terminals:dirty` gated by the strict-superset `restoreRelevantEqual` — so the high-frequency agent-detail / foreground firehose reaches neither the authored collection nor disk; `lastActivityAt` bumps on a live agent-identity change and, throttled to `RECENCY_THROTTLE_MS`, on a same-identity output tick) |
...
- One call mirrors a **cell, a collection, a stream, an event, AND every procedure** together, instead of a consumer hand-stitching `<coll>.keys`+`<coll>.get`, a separate `<cell>.get` read, a `<stream>.get` loop, and a hand-written procedure relay. The per-key collection bridge that *was* `@kolu/surface-remote`'s public `mirrorRemoteCollection` is now the **private** `mirrorCollection` engine inside it — consumers use `mirrorRemoteSurface` only ("you don't sell half a house").
...
defineSurface(spec): Surface<S>
  ...
  //   (core only INVOKES it; the predicate's vocabulary lives in the consumer, e.g. surface-remote)
