 ...
 * `opencode` install writes to under the user's `$HOME`.
 *
 ...
 * Because both endpoints derive the path from the one `$HOME` of the machine
 * the terminal lives on, local and remote are the same code with no geography
 * branch — the whole point of moving the mock into the agent's seat.
 *
