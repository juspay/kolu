 ...
 *
 * `@kolu/padi` is BORN at W1.1 (the padi plan of record, PR #1649): location is
 ...
 * double move. The PACKAGE is born now; the PROCESS at W2.2. W1.1 defines the
 * contract here and serves it COMPLETE in-process (kolu-server imports the
 * node-only assembly in `./assembly`), with ZERO client consumers. Later W1.x
 * PRs physically move each member's backing out of `packages/server` into this
 * package as they migrate the client onto the surface.
 *
 ...
 *      first frame of a fresh stream: `activity` and `terminalAttach`). W1.1
 ...
 * and `zod` — no `node:`/kaval runtime (that lives in `./assembly`). It depends
 * on kolu-common for the terminal schemas (which eventually migrate here); the
 * arrow points `@kolu/padi → kolu-common`, never back.
 */
 ...
 *  from being confused. Lands as a produced state in W1.7; W1.1 reserves it in
 ...
 *  is reserved (W1.7 produces it). Supersedes the client-side reader-join: one
...
// migrate onto (the root `terminal.*` namespace dies across W1.3–W1.7). They
// are intentionally distinct from `kolu-common/contract`'s raw-oRPC schemas —
// most notably `create` DROPS `lastActivityAt` (padi stamps with its own
// clock), so they are not duplicates to fold away.
...
/** `preview.read` — raw bytes of a file under a repo, for the iframe binary
 *  preview (dropping the raw `@kolu/serve-dir` HTTP bypass). The `..`/`%2f`/
 *  symlink 403 guards are re-enforced at the backing. */
...
export const PadiPreviewReadOutputSchema = z.object({
  ...
  /** MIME type derived from the extension (mrmime, via `@kolu/serve-dir`). */
  contentType: z.string(),
  /** The file's mtime in ms — the browser's per-URL cache key. */
  mtimeMs: z.number(),
});

/** `session.restore` — restore the persisted session server-side (padi's boot
 *  reconcile + restore, replacing the client respawn loop in W1.7). `resumeIds`
...
export const padiSurface = defineSurface({
  ...
  procedures: {
    ...
    /** Byte reads — the iframe binary preview. */
    preview: {
      read: {
...
// ── Per-member forwarding policy (W1.1 declares; W2.1's helpers read) ──────
...
// ── The frozen control core (defined W1.1; served for real W2.2) ──────────
 ...
 *  Defined as pure schema shapes in W1.1 (pinned by the contract test); W2.2
 ...
 *  `surface.padi.*`. kolu-common's own `contract` deliberately composes the
 *  padi-LESS `surfaces` (the client consumes that) — so W1.1 has ZERO client
 *  consumers; kolu-server adds the `padi` sibling on top locally. */
