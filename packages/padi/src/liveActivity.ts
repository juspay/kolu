 ...
 * This module is just a CONSUMER of that ONE edge — the SAME edge the `finishGate`
 * effective-finish fold reads — folded through the shared `createActivityTracker`
 * (its short live-dot window) into a live SET the stream publishes whole
 * (snapshot-then-deltas). No byte taps, no per-terminal subscriptions, no resize-mute
 * hack: kaval already excluded the reveal/resize repaint, so a switch to an idle
 * terminal no longer flashes its dot.
 */
