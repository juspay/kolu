...
export const SHARED_ARTIFACTS: readonly SharedArtifact[] = [
  {
    ...
    versionField: "startUnixUs (optional second tab field; pid-first law)",
    versionDisposition: "pid-first-tolerant",
    diskBasenames: ["kaval.pid"],
    diskBasenamePatterns: [],
    why: "Single-instance pid gate both the old daemon and the new supervisor read under the pid-first tolerant-reader law; a foreign shape must not silently no-op recycle.",
  },
  {
    id: "kaval-socket",
  ...
  },
  {
    ...
    diskBasenamePatterns: [],
    why: "Padi's own single-instance gate — same file format as kaval's, same pid-first-tolerant disposition.",
  },
  {
    ...
    diskBasenamePatterns: [],
    why: "The kolu-server ownership claim sits beside padi.pid; old and new supervisors must agree on its pid-first-tolerant disposition.",
  },
  {
    id: "padi-socket",
