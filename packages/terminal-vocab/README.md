...
| `./ports` | the port vocabulary leaf itself (`PortInfo` with its owner's `command` via `portCommand`, `UnclaimedPort`, `PortScope`, `PortFamily`, `foldPorts` / `foldUnclaimedPorts`, `isTcpPort`, …) — Effect Schema only, no sensors. Prefer this entry when you need a symbol that is not re-exported on `./schema` |
