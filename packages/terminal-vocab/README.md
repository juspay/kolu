...
| Entry | What |
| --- | --- |
| `./schema` | the `TerminalSnapshot` / `AgentMemory` / `AgentInfo` / `TerminalId` / `RestoreTarget` / `ProcessRss` schemas plus the `RepoChangePulse` / `FsFileInput` / `FsReadFileTextOutput` fs/git wire schemas `@kolu/padi-client/surface` composes. Re-exports the common port *types* used on the wire (`PortInfo`, `foldPorts`, …) and adds the pieces that ARE domain: `TerminalPorts`' `known`/`unknown` two-way, the host's `HostListeners` (claimed listeners plus an unclaimed half that is its own `known`/`unknown` two-way, with `UNKNOWN_HOST_LISTENERS`), `listenerAt` (what a host reading says about one port: claimed · unclaimed · absent · unknown), `portReach`, and the per-frame comparisons the wire's own records are deduped with (`portsEqual`, `hostListenersEqual`, `gridsEqual`) |
| `./ports` | the port vocabulary leaf itself (`PortInfo` with its owner's `command` via `portCommand`, `PortBind` (the bind a claimed `PortInfo` extends and an unclaimed socket is), `PortScope`, `PortFamily`, `foldPorts` / `foldBinds`, `isTcpPort`, …) — Effect Schema only, no sensors. Prefer this entry when you need a symbol that is not re-exported on `./schema` |
...
| `./duration` | the compact-duration LADDER (`compactDelta` — sec<60 / min<60 / hr<24 / else, with an untrustworthy delta answered `{ kind: "unknown" }` rather than clamped to zero) and the three phrases kolu says with it: `agoPhrase` ("5m ago", "just now" under a minute, empty for never-observed), `compactPhrase` ("45s", "20h") and `dualPhrase` ("2h 20m", "3d 5h" — the dominant tier plus the ladder's next-finer `sub`, taking the one word a caller genuinely disagrees about for an untrustworthy delta). `now` is always a PARAMETER — nothing here reads a clock, because a ticking `now` is ambient app state whose cadence the consuming app owns |
