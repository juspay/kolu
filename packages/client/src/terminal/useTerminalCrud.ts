  ...
  async function handleCreate(
    ...
    const theme =
      ...
      (peerBgs
        ...
        : undefined);
    // The server now returns synchronously even for remote hosts —
    // it registers a placeholder entry with `meta.connecting: true` and
    // finishes the SSH handshake in the background. TerminalContent gates
    // xterm on the connecting flag, so the tile itself shows the
    // "Connecting to <host>…" indicator. No loading toast needed.
