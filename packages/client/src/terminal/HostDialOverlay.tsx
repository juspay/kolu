 ...
 *
 *  No explicit z-index — it's the last child of `Terminal`'s `relative` wrapper,
 *  so DOM order paints it over the xterm without an explicit stacker that could
 *  escape into the resize-handle's stacking context (see `ui/stackLayers.ts`). */

import type { TerminalId } from "kolu-common/surface";
import { type Component, createEffect, createMemo, For, Show } from "solid-js";
import { toneDot, useHostProgress } from "../kaval/useDaemonStatus";
...
/** The screen itself, split out so `useHostProgress` runs only for a resolved
 *  remote host id. Shown only mid-dial (`inLifecycle`); unmounts on connect. */
  ...
  return (
    <Show when={progress().inLifecycle}>
      <div
        ...
        data-host-state={progress().state}
        // z-20 clears xterm's internal canvas layers (z-index up to 11) and the
        // search/scroll FABs (z-10) so the connecting screen actually covers the
        // terminal. Safe because Terminal's wrapper is `isolate`d — this never
        // escapes to the resize handle. Unmounts on connect (the <Show> above).
        class="absolute inset-0 z-20 flex flex-col gap-3 overflow-hidden bg-surface p-4 sm:p-6"
      >
        <div class="flex items-center gap-2">
          <span
            class={`h-2.5 w-2.5 shrink-0 rounded-full ${toneDot[progress().tone]}`}
          />
          <span class="text-sm font-medium text-fg">
            {progress().failed
              ? `Couldn't reach ${props.hostId}`
              : `Connecting to ${props.hostId}…`}
          </span>
          <span class="text-xs text-fg-3">{progress().label}</span>
        </div>
        <div
          ref={logRef}
          ...
          >
            {(line, i) => (
              <div
                ...
                classList={{ "text-fg-2": i() === progress().lines.length - 1 }}
              >
                {line}
              </div>
            )}
