...
/** Sort live terminals by recency, hydrate each into a `DockEntry` with
 *  a precomputed `searchText`, and narrow by AND-token query when one
 *  is supplied. Returns a flat list — for the column / facet / idle-
 *  sub-bucket projections the workspace grid renders, see
 *  `buildDockModel`. Pure; both the recency tie-breaker and the
 *  staleness classifier are caller-injected so the function takes no
 *  implicit clock dependency. */
