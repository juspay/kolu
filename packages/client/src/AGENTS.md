...
- **Socket-backed clients use `connectSurface` / `connectSurfaces`**: They wire the default-on heartbeat / half-open watchdog in, so `health().live` catches a silent half-open (laptop sleep / wifi roam). The half-open-blind path is now **unspellable** over a websocket: `surfaceClient`/`surfaceClients` THROW unless handed a watchdog-backed `LiveSignal`, and that brand is minted ONLY by `createLiveSignal` (`@kolu/surface-app/solid`), which wires the watchdog first — so a bare `{ live: () => true }` (or an open/close-only `() => socketStatus() === "live"`) is refused, not just discouraged. Hand-building `surfaceClient + websocketLink` is fine ONLY when you mint the `{ live }` with `createLiveSignal` (kolu's combined `wire.ts`, the example's raw seam) — never a hand-rolled signal. A `directLink`/`stdioLink` can't half-open, so it needs no `{ live }` at all.
