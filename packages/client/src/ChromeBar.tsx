 ...
 *  global control cluster (settings, command palette) on the right.
 *
...
const ChromeBar: Component<{
  ...
  // doesn't collide with the chrome. Per-tile panels live inside the
  // tile borders now, so there's nothing for the floating chrome to
  // dodge — the right offset goes away with them.
  ...
  return (
    ...
    >
      ...
      {/* Control cluster: settings → ⌘K. Cluster wrapper itself stays
       *  pointer-events-none so the gap-2 spaces pass clicks through;
       *  each button re-enables pointer-events-auto. */}
