...
Three durable analyzers — the minimum set that cracked this case and
will stay useful for the next investigation. Run as:
...
| Script                  | Purpose                                                                                                                                                   |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `count-rn.mjs`          | First check — histogram of Rn/Dl/Qt/xr/yn/debouncedFit. If counts grossly exceed live terminal count, there's retention.                                  |
| `check-yn-disposed.mjs` | Second check — distribution of `yn._store._isDisposed`. Answers "is xterm.dispose running?" If all are `true`, cleanup runs and the leak is post-dispose. |
| `orphan-paths.mjs`      | Full BFS from GC roots to every orphaned `Rn`; prints retainer chain per path signature. This is the analyzer that definitively identifies the leak site. |
