...
- **PR1 — the MCP face**: package scaffold, both transports + the one
  `surfaceClient`, `serveSurfaceAsMcp` + the expose map, the composite
  `wait.*` tools (extracting the shared WaitOutcome scaffold — the third
  consumer), the restart discipline. Its pin: the headless leg of step 9 —
  create → sendInput → wait.outputSettled (the `{seq}` stream, watched not
  rendered) → screen.text → kill, over BOTH transports, plus the restart
  legs. This already exercises most of the graduation path, headlessly.
- **PR2 — the canvas**: renderer, attach loop, `escape.ts` graduation (it is
  attach-scoped, so it rides PR2), create/kill gestures, navigator/rail. Its
  pin: the rendering leg of step 9 — the live canvas over the same path.

**The binary is `kolu`, not a new `kolu-tui` executable** (decision): today's
...
keeps meaning what it means today (the web server). The CODE stays in
`packages/kolu-tui` (location is structure; the app binary imports the tui
package — the arrow points the right way, since kolu-tui imports only
padi/surface libraries and never the reverse). The graduation proof is about
the RUNTIME path — no kolu-server *process* behind `kolu tui`/`kolu mcp` —
not the shipping closure; `p2p-kolu.mdx` already records kolu-tui shipping
...
built: a thin standalone bin target for the tui package if that host profile
ever materializes.</Footnote>

Steps, in build order (PR1 = steps 1–3 + 8; PR2 = the rest):
