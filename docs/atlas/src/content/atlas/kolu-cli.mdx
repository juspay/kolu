...
keeps meaning what it means today (the web server). The CODE lives in
`packages/kolu-cli` (location is structure; the app binary imports the CLI
package — the arrow points the right way, since kolu-cli imports only
...
1. **Scaffold `packages/kolu-cli`** on the sibling pattern (`cleye` arg
