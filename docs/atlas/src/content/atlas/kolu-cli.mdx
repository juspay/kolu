...
- **Boundary verdict, conditioned on the build** (the loop, not a waterfall):
  the CLI client is a **leaf app package** (`packages/kolu-cli`), not electricity —
...
keeps meaning what it means today (the web server). The bin AND the face
code live in `packages/kolu-cli` (location is structure). The dependency
discipline is per-module, because the package is a composition root: the
DISPATCH may import `packages/server` (the `web` arm boots it), but the
tui/mcp FACE modules import only `@kolu/padi`/`@kolu/surface*` — enforced by
an import-guard test (the SR11 negative-property style) asserting the face
modules' import graphs never reach a kolu app package. That module-level
fence is what keeps the graduation proof honest inside a package that also
routes to the server; a separate faces-package stays available if they grow,
but would be a receptacle ahead of its population today. The graduation proof is about
...
1. **The face modules join `packages/kolu-cli`** (the package PR1 created):
   connect/client/canvas/mcp modules beside the dispatch, guarded by the
   import-fence test from day one.
