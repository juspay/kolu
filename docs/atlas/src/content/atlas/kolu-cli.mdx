...
- **PR1 — the CLI restructure** (srid's sequencing): today's flag-only
  `kolu` grows subcommand dispatch — `kolu web` names today's behavior, bare
  `kolu` stays its alias (byte-for-byte: same flags, same boot), `tui`/`mcp`
  reserved with a clear not-yet-shipped error. Small, zero new capability,
  and it makes every later face a pure addition. Pin: bare `kolu` and
  `kolu web` are behaviorally identical (flag matrix test) + the reserved
  subcommands fail with the named message.
- **PR2 — the MCP face** (`kolu mcp`): package scaffold, both transports +
  the one `surfaceClient`, `serveSurfaceAsMcp` + the expose map, the
  composite `wait.*` tools (extracting the shared WaitOutcome scaffold — the
  third consumer), the restart discipline. Its pin: the headless leg of step
  ...
  this merges.
- **PR3 — the canvas** (`kolu tui`): renderer, attach loop, `escape.ts`
  graduation (attach-scoped, so it rides this PR), create/kill gestures,
  navigator/rail. Its pin: the rendering leg of step 9 — the live canvas
  over the same path.
...
Steps, in build order (PR1 = the subcommand dispatch; PR2 = steps 1–3 + 8; PR3 = the rest):
