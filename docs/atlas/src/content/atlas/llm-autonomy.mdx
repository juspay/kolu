...
<D2
  ...
  code={`direction: down
prompt: "User: /be <task>"
interview: "§0 Interview — the ONE sanctioned question"
state: "run-state spine (.do-results.json)\\nstage · pr · ci · evidence · verified\\n— /be does NOT write this today" { style.bold: true; style.fill: "#e6f4ea" }
stages: "§1–§5 stages\\nsetup · implement · PR · gauntlet · ci+evidence"
guards: "Guards — exit codes, not prose" {
  stop: "Stop hook (extend)\\nblock turn-end while active=working\\n+ Done post-conditions: ci-green · evidence · gauntlet-comments · repro-green"
  pre: "PreToolUse hooks (new)\\nblock prod-kolu kill / prod-port bind\\nblock AskUserQuestion after §0"
}
human: "Human (today: the gate)\\n'continue' · 'run ci' · 'DONT KILL PROD' · 'post evidence'" { style.stroke-dash: 4 }
done: "Done — only when every guard passes"
prompt -> interview -> stages
stages -> state: "write at each boundary"
state -> guards: "read"
guards -> stages: "block & auto-resume"
human -> stages: "today: manual re-drive" { style.stroke-dash: 4 }
stages -> done`}
/>
