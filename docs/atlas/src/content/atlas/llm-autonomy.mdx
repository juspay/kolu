...
| 4 <Pill variant="ok">shipped</Pill> | **Codify design philosophy as an always-loaded rule** (fail-fast/no-fallbacks · electricity boundaries · reuse-existing-source) and force §2 to read it | `.apm/instructions/conventions.instructions.md` → `conventions.md`; `be` §2 | S · **high** |
...
<Roadmap>
  <Milestone status="now" label="Round 1 — quick wins">**#4 design-philosophy rule — shipped** (always-loaded `conventions.md` §Design philosophy + `/be` §2 reads it). Remaining: #1 Stop-guard wire-in · #3 every-PR CI + master-sync · #6 CI-triage policy. Then re-run this audit and compare mean autonomy.</Milestone>
