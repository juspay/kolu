...
/** The daemon flavor adds ONLY supervision — the one thing that is daemon-y. */
...
```ts
// ── kolu's whole padi-session vocabulary, after ──
type BoundPadi = DaemonMirrorSession<PadiSurfaceClient> & {
  readonly preservation: { kind: "state-external"; children: "survive" };
  renew(): Promise<void>;   // was drainBoundPadi(); the type says programs survive
};
```
...
```ts
const pool = buildHostRegistry<BoundPadi, WsRpcHandler>({
  initialHosts: [LOCAL_HOST],
  buildEntry: (host) => {
    const session: BoundPadi =
...
Everything above that line is shared and identical: one convergence policy (both feed the kit's `decide()`), one verb set, one identity readout, the same surfaced degraded states. Two transports, one behavior — which is exactly why one alias (`BoundPadi`) can name them both.

### What a user never sees anymore

`RemoteMirrorSession` (renamed; "Remote" was false for the local arm) · `PadiBindingSession` (→ `LocalPadiSession`, symmetric with `RemotePadiSession`) · the role's dead `<C>` generic · `PoolableSession` (never shipped) · `padiStartedAt()/padiSurfaceVersion()/padiBuildCommit()` and their remote twins (six methods → `identity()`) · `drainBoundPadi` (→ `renew()`, with survival in the type) · `evictHostSession` (zero production callers; deleted) · `HostSession` in any registry/fanout type (the class remains only for direct `getHostSession` users).
