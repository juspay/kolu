...
*(The shapes below are the ratified end-state — the debate record in `debates/surface-hosting-simplify/` holds the how-we-got-here; none of it is needed to use this.)*

<Svg svg={after} wide caption="After the series lands — with the implementations shown. MirrorSession has THREE implementations (HostSession + the two padi arms); DaemonMirrorSession has exactly TWO (the padi arms — the only sessions that supervise); BoundPadi is the alias naming their shared type; the remote arm composes a HostSession rather than reimplementing it. No red arrows left." />
...
interface DaemonMirrorSession<Client = SurfaceClientLike> extends MirrorSession<Client> {
...
// fold into the daemon sub-role, nothing padi-specific remains to name).
// Call sites write the instantiation directly:
DaemonMirrorSession<PadiSurfaceClient>
```
...
```ts
const pool = buildHostRegistry<DaemonMirrorSession<PadiSurfaceClient>, WsRpcHandler>({
  initialHosts: [LOCAL_HOST],
  buildEntry: (host) => {
    const session: DaemonMirrorSession<PadiSurfaceClient> =
...
Everything above that line is shared and identical: one convergence policy (both feed the kit's `decide()`), one verb set, one identity readout, the same surfaced degraded states. Two transports, one behavior — both simply `DaemonMirrorSession<PadiSurfaceClient>`; no alias needed (deleted per srid).

### What a user never sees anymore

`RemoteMirrorSession` (renamed; "Remote" was false for the local arm) · `PadiBindingSession` (→ `LocalPadiSession`, symmetric with `RemotePadiSession`) · `BoundPadi` itself (deleted — after renew/preservation joined the daemon sub-role, the alias had nothing left to name) · the role's dead `<C>` generic · `PoolableSession` (never shipped) · `padiStartedAt()/padiSurfaceVersion()/padiBuildCommit()` and their remote twins (six methods → `identity()`) · `drainBoundPadi` (→ `renew()` on the daemon sub-role, with survival in the type) · `evictHostSession` (zero production callers; deleted) · `HostSession` in any registry/fanout type (the class remains only for direct `getHostSession` users).
