...
- **Kaval and "shutdown": two different verbs, two different promises.** *Drain* promises "I exit, my children keep running" — impossible for kaval, whose children ARE the PTYs. What kaval has is *recycle*: kill-and-respawn wrapped in capture→park→restore (records and scrollback come back; running programs don't) — which is exactly why kaval's is human-gated (the nudge, the Restart button) while padi's drain is automatic. The L3 kit types this: `DrainableProbe` vs `PlainProbe` — the false promise doesn't compile.
- **Identity is different — it's universalizable.** Anything could answer `hello` if its contract includes it; the guarantee just has to live in the *contract*, not the transport class. The plan has always named the graduation trigger: the frozen control-core types "live in `@kolu/padi` and graduate to a shared package whenever a second daemon wants them." Make `hello` the standard daemon preamble and `identity()` on the shared session role becomes legitimate — the guarantee exists by contract. **Identity can go universal; drain stays a typed per-daemon capability.**
- **"Second daemon" precisely:** drishti's far end today is an *ephemeral* agent (lives and dies with the link — no gate, no state-root, nothing to drain). The durable daemons are kaval and padi, and only padi is consumed through a bound session (kaval is supervised in-process by padi). So the trigger for a shared `SupervisedSession` is "a second **durable daemon consumed through a bound session**" — not yet real; the identity half, per the previous point, needn't wait for it.
