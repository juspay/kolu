...
*(Build-time refinements the implementing agent surfaced — four roadblocks resolved in code — live in [surface-hosting-roadblocks](surface-hosting-roadblocks.html); they adjust a few specifics below, e.g. identity() sits on `DaemonSession` until universal-hello ships as its own PR.)*
