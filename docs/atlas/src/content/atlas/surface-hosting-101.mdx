...
**Not now, on purpose:** no renames (the table above becomes the canonical explainer, linked from the three docstrings; rename only if confusion keeps recurring) — and no generic drain/identity on the transport (they're sentences to the daemon, not properties of the pipe; the typed can-or-can't versions already live in the L3 kit, and packaging them as a ready-made `SupervisedSession` waits for a second daemon that needs one). Whether the fleet verbs ride the role as optionals or as a declared capability gets settled in the widen's own review, where the real call sites are visible.
