...
<Svg svg={after} wide caption="The hosting side after the simplification series lands: identity is universal on the base role (every server auto-answers hello), the daemon sub-role carries only supervision, BoundPadi is an alias, and the registry demands exactly the one method it calls. No red arrows left." />
