...
`RemoteMirrorSession` (renamed; "Remote" was false for the local arm) · the role's dead `<C>` generic · `PoolableSession` (never shipped) · `padiStartedAt()/padiSurfaceVersion()/padiBuildCommit()` and their remote twins (six methods → `identity()`) · `drainBoundPadi` (→ `renew()`, with survival in the type) · `evictHostSession` (zero production callers; deleted) · `HostSession` in any registry/fanout type (the class remains only for direct `getHostSession` users).
