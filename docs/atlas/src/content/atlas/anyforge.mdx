...
| **0d** | **Route a recognized non-GitHub forge away from gh, honestly.** A new forge-neutral `PrResult` kind `unsupported` (distinct from `absent`: "no adapter for this forge yet" ≠ "this branch has no PR"), plus a `detectForge` host match `codeberg.org → unsupported` backed by a trivial non-gh arm in `FORGE_ADAPTERS`. The decision is made at the knowing endpoint (the remote host) instead of guessed downstream from `gh`'s stderr — so the bug below can no longer be expressed for Codeberg, and the brittle `gh`-stderr allow-list isn't grown. | Codeberg terminals stop emitting `error`-level log noise and the scary "gh: unknown error" popover; they show nothing (no GitHub PR on a non-GitHub repo), like a branch with no PR but for the honest reason. | <Pill variant="done">shipped</Pill> <PrLink pr={1627} /> |
| **1** | `kolu-forgejo` sibling adapter — now **purely additive**: the detection/registry/`remoteUrl` plumbing already exists (0c–0d), so this is **one `PR_REGISTRY` entry + one host match in `detectForge`** (flipping `codeberg.org` from `unsupported` to `forgejo`, and adding `KOLU_FORGEJO_HOSTS`) plus the Forgejo REST resolver (zod-validated, typed errors) and the irreducible compile-time union/popover arms. `kolu-github` and `kolu-git` are untouched. | Full PR pill — number, title, open/merged/closed state, CI checks — on Codeberg and self-hosted Forgejo/Gitea. | <Pill variant="todo">closes #1240</Pill> |
