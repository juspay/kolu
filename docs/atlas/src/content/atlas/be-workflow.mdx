...
- **It's built for Claude Code's `Workflow` tool.** That's the reason `/be`
  exists as its own skill. `/do` is harness-agnostic; `/be` is optimized for
  Claude Code so its review gauntlet can run *dynamic, `Workflow`-backed*
  reviewers — the debate skills <Skill name="be-review" /> drives
  (<Skill name="lens-debate" />, <Skill name="codex-debate" />) fan out as
  multi-agent workflows that `/do`'s flow has no way to express.
