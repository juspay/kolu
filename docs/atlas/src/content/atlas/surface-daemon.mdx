...
<Roadmap>
  ...
  <Milestone status="next" label="S0.5 · the supervisor half is born as its own package (kaval B2)">Revised 2026-06-12 (was: gestate in `server/src/ptyHost/`, extract at S1 — the timing paragraph above records why the deferral fell). B2 births `@kolu/surface-daemon-supervisor` holding endpoint states · driver ops · `waitForPidGone` · the composed restart, parameterized over driver ops and the surface client; kaval-specific reach mechanics stay in `packages/server` (`localDriver.ts`, soul). Not a staleKey root — zero hash surface by construction. Like its daemon sibling, the package carries a README with the mechanism/soul line, the API table, and a usage example (kolu-server's composition, `odu serve`'s projected beside it). B2's recycle-on-every-deploy then soaks the restart race in production with zero sessions at stake. **Exit: a daemon-package edit flips kaval's staleKey while a supervisor-package edit does not — by package boundary, not by a glob.**</Milestone>
