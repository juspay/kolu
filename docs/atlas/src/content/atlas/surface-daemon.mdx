...
// ── pidGate.ts — both sides of one file format ──────────────────────────
export type GateResult =
  | { kind: "acquired"; release: () => void } // we hold it; release at teardown
  | { kind: "held"; pid: number };            // a LIVE process holds it
/** Atomic: write pid to a temp file, link(2) into place;
 *  on EEXIST read the gate, liveness-probe kill(pid,0), steal if stale. */
export function acquirePidGate(gatePath: string): GateResult;
/** The supervisor side — B2's endpoint imports this, from here. */
