...
The createPulam framing this section once carried is **superseded**. The settled architecture ([the awareness design](awareness-derive-store.html), from a 3-agent debate) is **observe vs. remember**: a *memoryless producer* per host emits `HostObservation` events (cache + live, **no** memory); *kolu alone folds* them. The consequence for this model: the three **memory** fields — `lastActivityAt` · `agentSession` · `lastAgentCommand` — **move onto `kolu.authored`**, so the served `awareness` collection everywhere carries only `HostObservation`. The reader-join keeps its shape (`authored ⋈ awareness`); it just takes the memory fields from the authored half. *(The type tables above still show the pre-move shape; the converged types live in the awareness note. The remaining live/memory split is being explored further.)*
</Callout>
...
```text
S1   memoryless producer:  taps → AwarenessObservation events  (HostObservation = cache + live)
