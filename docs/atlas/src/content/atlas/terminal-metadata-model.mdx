...
The createPulam framing this section once carried is **superseded**, and the cutover has **landed**. The settled architecture ([the awareness design](awareness-derive-store.html), from a 3-agent debate) is **observe vs. remember**: a *memoryless producer* per host emits `Observation` events (**no** memory); *kolu alone folds* them. The consequence for this model: kolu's **two** remembered facts — `lastActivityAt` · `lastAgentCommand` — plus the fold-derived discriminated `restoreTarget` **move onto `kolu.authored`** (the old `agentSession` field is gone — the identity now rides the `exact` arm of `restoreTarget`), so the served `awareness` collection everywhere carries only an `Observation`. The reader-join keeps its shape (`authored ⋈ awareness`); it just takes the memory fields from the authored half. *(The type tables above still show the pre-cutover shape; the converged types live in the awareness note.)*
</Callout>
...
```text
S1   memoryless producer:  taps → AwarenessObservation events  (Observation: cwd·git·pr·agent·foreground)
