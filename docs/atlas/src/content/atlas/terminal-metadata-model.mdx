...
<Callout kind="warn" title="Correction (proposed) — the in-process home is the mis-step">
The *kolu-in-process* home of this factory re-couples awareness into kolu — the thing [pulam](pulam.html) exists to decouple. The fix ([R9.0](remote-terminals-finale.html#r90)) makes kolu's awareness backing a **mirror of a local pulam**, so the backing is *always* a pulam mirror (local or remote) and **pulam is the one authority per host**. The reader-join and the injection seam stay; only the backing source changes.
</Callout>
