...
The reader-side **join** documented below (two collections, joined at the reader; no server-side fusion) still stands. But the **observed-half type** shown throughout this note is **pre-cutover**. The [awareness-derive-store cutover](awareness-derive-store.html) (now landed) renamed `AwarenessValue` → **`Observation`**, **deleted** the `agentSession` field (the restore identity is now the fold-derived `resumeAgent` on the authored record), moved kolu's two remembered facts (`lastActivityAt` · `lastAgentCommand`) onto `kolu.authored`, and made `pr` **persisted** (restore-relevant, like `git`) — so the "frozen pr on the sleeping arm" special case described below is **gone**. Read the type tables as history; the converged types live in [the awareness design](awareness-derive-store.html).
</Callout>
...
The createPulam framing this section once carried is **superseded**, and the cutover has **landed**. The settled architecture ([the awareness design](awareness-derive-store.html), from a 3-agent debate) is **observe vs. remember**: a *memoryless producer* per host emits `Observation` events (**no** memory); *kolu alone folds* them. The consequence for this model: kolu's **two** remembered facts — `lastActivityAt` · `lastAgentCommand` — plus the fold-derived `resumeAgent` restore target **move onto `kolu.authored`** (the old `agentSession` field is gone — its identity is now `resumeAgent`), so the served `awareness` collection everywhere carries only an `Observation`. The reader-join keeps its shape (`authored ⋈ awareness`); it just takes the memory fields from the authored half. *(The type tables above still show the pre-cutover shape; the converged types live in the awareness note.)*
</Callout>
