...
```ts
// koluSurface (kolu's own, in-process)
const status = app.streams.gitStatus.use(() => ({ repoPath }));
//    ^ each emission is the full GitStatusOutput; render it.
```
...
```ts
// terminalWorkspaceSurface (the shared surface pulam serves)
const snap = await app.rpc.git.getStatus({ repoPath });        // 1. snapshot
app.streams.subscribeRepoChange.use(() => ({ repoPath }))      // 2. on each pulse…
   .on(() => refetch(app.rpc.git.getStatus({ repoPath })));    //    …re-query.
```
...
- **`terminalWorkspaceSurface`** (the surface `pulam` serves, and the one a remote
  ...
  (`packages/terminal-workspace/src/surface.ts:88-105`: *"re-queries procedures
...
Awareness has **no** such split: it's a `collection`, identical on both surfaces —
