...
across terminals — held against what kolu is. hcom is a genuinely nice design, and
...
memory* and flushes it to the JSONL only **after you answer** — so a log reader
(and a file-based agent) sees `waiting`, never `awaiting_user` (<Issue n={905} />).
...
The comparison is not "kolu is missing a layer." It's "kolu already made the
volatility-boundary call hcom's design fights."
...
- **The terminal is the floor** <Pill>keep</Pill> — `kaval-tui send` types the
  prompt; the screen is read *only* for what files can't carry (the raw prompt
  keystroke and the interactive prompts above). That's why driving an agent is
  *"press Enter,"* not a protocol: it's the one path that works for **any** agent
  at **zero** setup — the same [agent-agnostic bet](./herdr-vs-kolu.html) kolu
  makes everywhere.
