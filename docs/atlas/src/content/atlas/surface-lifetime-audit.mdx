...
Two candidates earn near-term attention if the class bites again:
`daemonMain`'s missing exit (same fix shape as STDIO1 — a natural second
patch in that lineage) and `serveOverUnixSocket.close()` not ending
established connections (an "unenforced" cell, not even homework).
...
the 17 non-framework-owned seams it touches, and each framework fix
(STDIO1 now; `daemonMain` next if ratified) flips rows from homework to
owned. The refuter discipline mattered: two over-claimed variants died in
