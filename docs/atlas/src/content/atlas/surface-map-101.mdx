...
**The proposed primitive — `createKeyedRoot`'s retained sibling** (`@kolu/surface/solid`), then its map-tied form (`@kolu/surface-map/solid`) where the living set is `entries` itself:

```ts
// Layer 1 (solid-generic): N live keyed owners; dispose on RETIREMENT, not on switch.
createRetainedRoots<K, T>(keys: Accessor<readonly K[]>, build: (key: K, ctx: ScopeCtx) => T)
  : { get(key: K): T | undefined }
type ScopeCtx = { isActive: Accessor<boolean> };

// Layer 2 (map-tied): membership drives the key set; "active" stays app policy.
scopedByEntry<K, T>(map: SurfaceMapClient<…>, active: Accessor<K>, build): {
