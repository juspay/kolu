...
- **Ready now:** L1 · L7 · L8 · L10 · L12 (srid mints) · L13 · L26 · L17 (+ video-ext table, was L5b) · L18 · L27 · L20·21 (drishti, merged) · L23 · L24 (W3.1 merged 2026-07-04) — and L4 at a named version
...
**Two additions from post-switch review (srid, 2026-07-06):** (a) the interim shape exports its plumbing as API — consumers write `useX()()` (an accessor returning an accessor; Solid treats that as internal dynamic-source shape, and idiomatic libraries flatten before the consumer) — the context port must flatten hooks to single calls, dissolving `useBindingScopedSub`; (b) `createSharedRoot` hand-rolls `@solid-primitives/rootless`'s `createSingletonRoot` (kolu already depends on four @solid-primitives packages; the conventions say prefer them) — adopt the ref-counted ecosystem primitive, auditing the never-dispose factories first (`getNowTicker`, dock order — anything relying on never-teardown; note the discarded-disposer design is the same shape that caused the useCell toast leak, deliberate here but weaker than ref-counting).

**Where:** `packages/client/src/wire.ts` and every `createSharedRoot` consumer (the switch PR's own diff is the inventory's seed). **Done when:** no transition-tagged code survives; hooks are single-call (no `()()` API); `createSharedRoot` is replaced by (or a documented thin veneer over) `createSingletonRoot`; the switch e2e stays green.
