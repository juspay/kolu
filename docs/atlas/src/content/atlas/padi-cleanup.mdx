...
<Svg svg={birdseye} caption="The ledger at a glance. Top lane: the switch-prep critical path (L3 shipped → L6 in flight → L23 queued → W4 the switch → L11's post-switch sweep). Amber-dashed = gated; pills are milestones that live outside this ledger." />

**Status index** — updated as entries ripen or ship:

- **Ready now:** L1 · L7 · L8 · L10 · L12 (srid mints) · L13 · L14 · L15 · L16 · L17 · L18 · L19 · L20 (drishti) · L21 (drishti) · L23 · L24 (W3.1 merged 2026-07-04) — and L4 at a named version; L5's remaining half (video-ext table)
- **Gated:** L11 on W4 (the switch) · L2 on kolu-tui (future work) · L9, L22, L25 on evidence
- **Shipped:** L6 — padiBinding split (#1698, 2026-07-05); L3 — the convergence kit (#1674, 2026-07-04); L5's fold-half
...
### L25 — extract the browser padi-binding into its own home *(only if a second consumer earns it)*

*Status: gated on evidence (a second consumer of the browser-side binding layer) · Lens: lowy (a maybe-receptacle, population one) · Gate: drishti if it becomes a package touching `@kolu/surface*`; none if it stays a kolu-internal module*

W4's switch PR-2 builds the per-binding client lifecycle in-place in `packages/client` as a cohesive `binding/` module (`bindHost(host) → handle`, components take a handle not a page-global). srid floated a `packages/padi-web-client` package for it (2026-07-05); the verdict was **build in-place first** — it has one consumer today and a package boundary doesn't shrink the Solid teardown/rebuild work, only relocates it (the electricity three-tests: not a domain-agnostic volatility — `@kolu/surface` already owns that — and unproven by a second consumer). This entry is the revisit: if a real second consumer of the browser binding layer appears (a second web frontend — NOT kolu-tui, which is a TUI on `@kolu/padi/dial`), extract the `binding/` module to its own home. Because PR-2 shapes it as a factory-API module from the start, the extraction is pure motion, not a redesign.

**Done when:** either a second consumer exists and the module is extracted (seal/deps green; drishti green if it lands as a shared package), or this entry is deleted with "one consumer, stays a leaf" and a date.
