...
2. **The padi/server re-cut** — *one kolu PR, no drishti gate*. The deep move: reshape `packages/server/src` around its real volatility — **L27**'s `padi/` (or `hosting/`) subdirectory cut lands as the campaign's *opening pure-motion commit* (rename-detection reviewable on its own) — then the substantive home: **L17** (the chrome vocabulary out of `vocab.ts` + the video-ext table homed in `@kolu/padi`). **L13** rides (the seal's test-file flank — the seal is this campaign's own net, so its flank closes here). *(L24 — `padi-tui --host` — is deliberately NOT here: it is a feature, and features never ride a refactor campaign; it ships as its own PR, below.)*
...
<Svg svg={birdseye} wide caption="The ledger at a glance. Top: the three campaigns — complete the primitives (paired drishti + odu; the deep move is the derivation algebra), the padi/server re-cut (opens with L27's pure-motion commit), the client wire/state sweep. L8 rides all three (hygiene never ships standalone). Middle: L24 ships as its own feature PR (a feature never rides a refactor campaign); then the parked/gated lane, each with its gate named. Bottom: shipped." />
...
- **Campaign 2 (the padi/server re-cut):** L27 (opening pure-motion commit) · L17 · L13 rides — ready now
- **Its own feature PR (ready now):** L24 — `padi-tui --host`: new user-visible CLI behavior can't ride a byte-identical refactor campaign
