...
- **Gated:** L11 on W3.2 · L2 on kolu-tui (W5) · L9, L22 on evidence — L3 is IN FLIGHT (the convergence-kit PR, stacked on merged #1671)
...
### L3 — the daemon-lifecycle convergence kit into `@kolu/surface-daemon*` *(upgraded 2026-07-03 — srid's call, and #1670 is the evidence)*

*Status: ready once #1670 lands (extract from two complete instances, never one) · Lens: lowy (one volatility axis, two hand-rolled instances) · Gate: drishti*

kaval and padi each hand-roll the same lifecycle area: **"the running daemon is not the daemon I shipped — detect it, converge it."** The duplication is at least six-fold: baked build ids (`KAVAL_BUILD_ID` / `PADI_BUILD_ID` + staleKey), version handshakes (`system.version` vs `control.hello`), typed skew errors, convergence policies (kaval: supervisor recycle-on-skew; padi: adopt-or-spawn-or-refuse + newest-wins drain + #1670's build-mismatch drain), staleness responses (kaval: the human currency nudge, because recycling kills PTYs; padi: auto-drain, because restarts are cheap), and identity/hello shapes. **#1670 is what duplication cost**: kaval's currency lesson existed, nothing shared it, padi's re-derivation shipped with the hole.
...
*Open question to settle during the extraction (surfaced dogfooding #1671 on zest, 2026-07-03): kaval's `nudge-human` currency keys on its whole hashed closure, which includes `surface-daemon/src` — so a daemon-spine change with no effect on kaval's PTY-survival behavior still nudges "update available", and acting on it costs live PTYs. Decide whether kaval's currency should key on a behavioral slice rather than the full closure. Do NOT change padi here (padi's drain is cheap, over-firing is harmless); this is a kaval-policy question the kit makes visible by putting both daemons' currency side by side.*
