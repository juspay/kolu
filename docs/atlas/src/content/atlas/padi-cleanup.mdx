...
- **Sweep + fix.** Find the same smell repo-wide (prioritize the daemon/session/convergence surfaces: padi, surface-daemon-supervisor, the re-serve). Known seeds the debate surfaced: the L3 convergence kit's `buildId === ""` = "off-nix" (a string sentinel on the control-core-hello / `ConvergenceIdentity` wire — a SEPARATE layer from the surface refactor's readout, which the surface PR deliberately leaves untouched; **L26 owns replacing it** with a match on the identity kind); any `X | null` where `null` answers two different questions (absent-vs-error, disconnected-vs-empty, not-yet-vs-never). Grep seeds: `: string | null`, `=== ""`/`!== ""` as a meaning-bearing check, optional `?:` fields a reader treats as a state, `!`/`as any` around a maybe-absent value. Inventory with an Explore agent (classify each hit: honest single-meaning null = fine · overloaded = fix); propose the sum-type replacements to the coordinator; fix the overloaded ones as discriminated unions. **NOT a blanket anti-null crusade — a single-meaning "absent" is fine; the target is `null` doing two jobs.**
