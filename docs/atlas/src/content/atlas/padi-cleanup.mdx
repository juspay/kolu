...
<Svg svg={birdseye} wide caption="The ledger at a glance. Top: the one remaining campaign — complete the primitives (paired drishti + odu; the deep move is the derivation algebra), with L8's hygiene residue riding it. Middle: L24 shipped as its own feature PR, #1749 (a feature never rides a refactor campaign); then the parked/gated lane, each with its gate named. Bottom: shipped — including campaigns 2 and 3, merged together as #1747." />

**Status index** — updated as entries ripen or ship:

- **Campaign 1 (complete the primitives):** L28 rung 2 · L1 · L20·21 — placed in [the merged plan of record's](surface-runtime-boundary.html) PR sequence (L28 rung 2 → PR 3 · L20·21 → PR 5 · L1 → PR 8); L8's residue rides the campaign's last PR as the tail commit
- **Its own feature PR — SHIPPED:** L24 — `padi-tui --host` (#1749, merged 2026-07-10): new user-visible CLI behavior can't ride a byte-identical refactor campaign, so it shipped standalone
...
*Status: SHIPPED — #1749, merged 2026-07-10 (`--host` flag + `connectPadiTuiViaHost` in `packages/padi-tui/src/main.ts`); it shipped as its own feature PR because new user-visible CLI behavior (`create` lands real terminals on the remote host) cannot ride a byte-identical refactor campaign · Lens: parity (kaval-tui `--host` is the shipped precedent) · Gate: none (kolu-internal)*
...
**Done when:** a load test demonstrates bounded memory under an unpaced sender at every serve site — or the entry closes with the recorded per-consumer-bound decision. *(Cross-link, 2026-07-13: [the merged plan of record](surface-runtime-boundary.html) routes the append-heavy wire concern — terminal bytes, logs — to a named bridge design question; if that design ships a framework-owned subscriber bound, L22 closes with it. The gate is unchanged either way.)*
