...
- **`socketHolder.ts`** — a new leaf next to `waitForPidGone.ts`: `socketHolders(socketPath)
  ...
  on darwin), fail-fast on an unsupported platform.<Footnote>Since OSF4 this leaf holds
  neither the reader nor the fold: the platform dispatch is one `osfacts socket-holders`
  call, the fold from that document to the three-way `SocketHolderReading` lives in
  `osfacts-client` beside the parser, and what remains here is the shape of the ask —
  `ReadSocketHolders`, injected on `EndpointSpec`.</Footnote>
...
- `SocketSquatterForeignError` / `isSocketSquatterForeignError` and `ReadSocketHolders` /
  `SocketHolder` / `SocketHolderReading` — genuinely new exports (the last two re-exported
  from `osfacts-client`, which owns the fold that builds them).
