...
- **First-class *and visually distinct* in every presence surface — from ONE
  ...
  the one place staleness wins over the ☾** (<PrLink pr={1591} />): a long-slept tile
  whose last activity falls *outside* the window routes to `parked` and drops, so the
  window compresses yesterday's dormant terminals too instead of letting them pile up
