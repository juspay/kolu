...
resumes the agent with `resumeAgentCommand`. With the persisted `resumeAgent` identity
...
- **Wake resumes the *exact* conversation that was running — resolved in juspay/kolu#1495.**
  ...
  agent's native session identity (`resumeAgent` = `{ kind, sessionId }`, derived by
  kolu's fold from the live `agent`) and resumes by it (`claude --resume <id>` &c.), falling back to
