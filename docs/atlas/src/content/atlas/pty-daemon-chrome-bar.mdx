...
| **Update pending** | <Pill variant="todo">B3.4</Pill> | `kaval` build ≠ the expected build → amber via the read-site `staleKey !== currentBuildId()` derivation. The nudge #1034 over-fired now fires only here. |
