...
- **Coincidence rendering** — always two explicit columns + the `≡ in-process` tag; verbose/explicit is favoured until the feature stabilizes, no collapse-when-equal. **Revisited once the feature stabilized** (B2/B3.4 shipped): the rail now *does* collapse-when-equal — one readout at rest, fanning out only on divergence — see [chrome-bar-declutter](chrome-bar-declutter.html).
- **Labels** — `srv` / `pty`.
