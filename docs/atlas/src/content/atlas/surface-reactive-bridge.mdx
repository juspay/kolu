...
- **Phase 0 — ships first, and it is [W5's framework slice](padi.html#w5).** `reactor.ts` exporting `derived.cell` + `scan` + `source` with the error wrapper and the stopped-latch→health flip; boot narrowing (derived member: no ctx entry, no write verbs); the engine's deep import lint-banned. Zero runtime surgery — derived cells ride the existing cell connect seam (`server.ts:1120`, `:1490-1494`). First consumer: drishti alerts (worked example 2), paired drishti PR per `.claude/rules/surface.md`. If a leaner W5 slice has already shipped as a plain connect block, it migrates mechanically — the connect block is literally what `derived.cell(scan(...))` lowers to.
