...
3. **The `.fromPoll` fused constructors:** keep as named forms, or downgrade to labeled sugar over `signalFromPoll` + `derived.cell`/`.collection` so the vocabulary stays minimal? *(srid answers after reading [the API section](#the-api-symbol-by-symbol), which marks `.fromPoll` as pending this.)*
