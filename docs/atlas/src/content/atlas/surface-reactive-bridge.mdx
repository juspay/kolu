...
Every new symbol, hello-world sized — one job each. The tag names the migration phase that ships it.

**`derived.cell`** *(phase 0)* — a cell computed from siblings; recomputes when they change, publishes through its own `equals` gate.

```ts
cells: {
  name:     { schema: z.string(), default: "world" },
  greeting: derived.cell(($) => `hello, ${$.name()}!`),
}
```

**`derived.cell.scan`** *(phase 0)* — a cell folded through carried state; passing `store:` makes the state survive restarts — omitting it is a visible choice.

```ts
const clicks = signalFromCallback<void>((emit) => button.onPress(emit));
cells: {
  count: derived.cell.scan({ over: clicks, initial: () => 0, step: (n) => n + 1 }),
}
```

**`signalFromCallback`** *(phase 0)* — adapts any subscribe-style callback into a graph signal.

```ts
const online = signalFromCallback<boolean>(
  (emit) => net.onChange(emit),  // install once; the returned fn uninstalls
  true,                          // initial, until the source speaks
);
```

**`signalFromPoll`** *(phase 2)* — a signal fed by re-reading a source on a tick; no dedup of its own — the target member's `equals` decides.

```ts
const temperature = signalFromPoll({
  read: () => sensor.readCelsius(),          // T+0 seed; a failing FIRST read crashes boot
  install: (tick) => everySeconds(5, tick),  // the caller owns the cadence
});
```

**`derived.collection`** *(phase 2)* — a collection computed as a whole map; the bridge diffs each output against the last by the collection's `equals` and publishes only the changed keys.

```ts
collections: {
  bigFiles: derived.collection(($) =>
    filterValues($.files(), (f) => f.bytes > 1e6)),
}
```

**`derived.collection.fromPoll`** *(phase 2 — pending Q3: may become labeled sugar over `signalFromPoll` + `derived.collection`)* — poll a keyed source; changed keys upsert, vanished keys evict.

```ts
collections: {
  processes: derived.collection.fromPoll({
    read: readProcessTable,   // Promise<ReadonlyMap<pid, Process>>
    install: everySecond,
  }),
}
```

**`reactiveFamily`** *(phase 3)* — a keyed set of sources under one signal: membership diff, per-key last frame, per-key disposal and error isolation.

```ts
const rooms = reactiveFamily<RoomId, RoomState>({
  members: roomIds,                         // a signal of the current key set
  attach: (id, set) => watchRoom(id, set),  // per-key source; disposed on exit
});
// rooms.value: ReadonlyMap<RoomId, RoomState> — the last frame per live key
```

**`signalMap`** *(phase 4)* — a producer-side keyed store that is also a graph source: the app's registry, trackable.

```ts
const registry = signalMap<TerminalId, Terminal>();
registry.set(id, term);   // writers mutate it like a Map…
collections: {
  terminals: derived.collection(() => mapValues(registry, compose)),  // …readers track it
}
```

**`computed`** *(phase 1)* — a private intermediate node: shared work several members read, without itself becoming a wire member.

```ts
const frames = signalFromCallback<Stats>((emit) => statsTap(emit));
...
**`ctx.batch`** *(phase 1)* — group several writes into one graph frame, so derivations recompute once.
...
**`$`, the read face** *(phase 1)* — the typed sibling reader handed to every compute; reading is depending, and a derived sibling is read as its computed, never its mirror.
...
**`Subscription.updated`** *(client — ships with W5's phase 1 regardless)* — the change-iff-fired edge on any cell subscription; fires exactly when the producer's `equals` said "changed".

```ts
const alerts = client.cells.alerts.use();
alerts.sub.updated(({ prev, next }) => {
  if (next.level > prev.level) playRaiseSound();
});
```
...
| `hold` / `holdDyn` | ADD-SHIP | `signalFromCallback` + `derived.cell`: hold a push source's last frame in a cell. |
| `holdIncremental` | EXISTS | the collection store + wrapped upsert/remove publish path (`server.ts:1585-1600`). |
| *poll → keyed snapshot reconcile* | **ADD-SHIP** | `derived.collection.fromPoll` — no single reflex name. The most-repeated hand-roll in both trees. |
...
| `hold`/`holdDyn` | ADD-SHIP | `signalFromCallback` (+ `derived.cell`) and `signalFromPoll` (no isEqual — dedup at the target cell's spec `equals`). |
| `Accumulator` (`accum*`), `foldDyn`/`foldDynMaybe*` | ADD-SHIP | `derived.cell.scan` — `(state, frame) => state` fold into a cell; `accumMaybe`'s "Nothing = no update" = return the previous state reference. Demand: drishti's committed alerts scan (paired-PR campaign); the padi activityFeed adoption is conditional (the activityFeed open question). |
...
| `scanDyn`/`scanDynMaybe`/`mapDynM` | ADD-SHIP | `derived.cell.scan` with a cell upstream. |
