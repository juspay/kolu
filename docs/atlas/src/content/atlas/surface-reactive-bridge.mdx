...
Not carded: the keyed machinery — `reactiveFamily` *(phase 3)* and `signalMap` *(phase 4)* — arrives with its phases, introduced there. Client-side, the one new symbol stays `Subscription.updated` (the change-iff-fired edge; see the worked alerts example).
...
Net public vocabulary: **seven orthogonal symbols** — `source` · `scan` · `computed` (graph), `derived.cell` · `derived.collection` (wire), `$` · `batch` (glue) — plus the phase-3/4 keyed machinery (`reactiveFamily`, `signalMap`); versus the algebra's 7–8 names with 6 queued.
