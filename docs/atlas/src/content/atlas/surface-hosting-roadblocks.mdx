...
**The fix (a):** make the convergence identity null-free too — off-nix becomes a *typed* absence (a `kind`/null), never `""`; `decide.ts` matches that instead of `=== ""`. Behavior-preserving shape swap (the agent confirmed), touching surface-daemon-supervisor + control-core hello + the convergence probe (then padi-local; now `probeDaemonIdentity`).
