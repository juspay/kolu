...
// implementSurface STAMPS system.identity from the server's own baked build info — zero per-server code.
// Every server (daemon or not) now answers surface.system.identity.
```
