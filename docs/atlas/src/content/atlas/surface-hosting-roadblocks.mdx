...
## 6. The convergence kit's `buildId === ""` — fix here, or defer to L26? (build-time; needs your call)
...
- **(b) LEAVE it — recommended.** The sentinel is a *different layer on a different wire*; fixing it means touching the delicate convergence kit (supervisor + control-core hello + `probePadiForConvergence`) — concern-mixing + risk in a session/readout PR. And it's **already owned by L26** (the codebase-wide null/sentinel sweep, which ships *with the lint guard*). Deferring ≠ ignoring — it gets fixed, coordinated, soon. Consistency holds meanwhile: padi always declares its baked identity, so both paths read the SAME underlying value via different members — no incoherence.
- **(a) FIX it here too.** Adapt `decide.ts` to match on the identity kind (`anonymous` = off-nix) instead of `buildId === ""`. Behavior-preserving shape swap — but a medium cross-package change (touches surface-daemon-supervisor + control-core hello + the probe), widening this PR's blast radius into convergence logic.

> **Coordinator's call: (b), defer to L26** — keeps this PR scoped; the sentinel still dies soon, with its lint. **srid: override to (a) if you'd rather kill it now while the agent's hands are in there.** The agent is unblocked either way (the readout layer is unambiguous); flipping later is cheap.
