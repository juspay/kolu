...
```ts
// @kolu/surface
// (final shape — see roadblock 5 for the startedAt/baked split and the BuildCommit sum)
interface SurfaceIdentity {
  startedAt: number;                 // framework-stamped when connected
  baked: BakedIdentity | null;       // the server-declared triple, or null (no declared build)
}
interface BakedIdentity {
  contractVersion: string;
  buildId: string;                   // the build's content hash — convergence CURRENCY (staleKey)
  commit: { kind: "commit"; sha: string } | { kind: "dev" }; // navigable vs dev — a SUM, never `string | null`
}
```
...
interface BakedIdentity {
  contractVersion: string;
  buildId: string;
  commit: BuildCommit;            // a SUM, not `string | null` — dev-vs-real is explicit, reader must branch
}
interface SurfaceIdentity {
