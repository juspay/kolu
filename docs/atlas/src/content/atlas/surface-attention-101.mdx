...
Read the two against each other and the boundary shows itself: the **wire member's provenance** differs (kolu consumes an existing projection; drishti mints W5's one new member), the **writer** differs (a registry fold vs a metrics threshold — two folds that merely *rhyme*, deliberately not unified), the **value shape** differs (bare ids vs labeled items), the **aggregation** differs (a sum vs a count), the **click** differs (switch+focus vs expand). What's identical is everything in between — the honest frames, the change pairs, the watcher, the tag-keyed delivery — which is exactly the slice the framework takes.
