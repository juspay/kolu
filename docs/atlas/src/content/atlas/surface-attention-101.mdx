...
### A derived member is declared, not hand-wired (the derivation algebra)

The third piece generalizes the first two: a **derived** member — a cell whose value is a projection of some other live thing — becomes a *declaration* the framework runs, instead of a `.set()` a human remembers to wire. Today padi's urgency cell is produced by hand: `recomputeUrgency()` is called from inside the publish seam of every write path that might change it, and a prose comment guards the invariant that no seam forgets. The declaration replaces all of it:

```ts
// padi's deps — the fold IS the wiring; every terminals write arms it:
cells.urgency = foldCollection({
  from: "terminals",
  fold: (entries) => ({ awaitingIds: liveAwaitingIds(entries) }),
});  // seeds from fold(readAll()) — current truth, never a default
```

Three guarantees ride every such declaration. **One writer, enforced at boot**: `implementSurface` rejects a derived member whose wire verbs include any mutation (`set`, `patch`, `upsert`, `delete`, even `test__set`) — a derivation is the fact's one writer, and tests reset it by writing its *source*. **Producer-side dedup, in one place**: the fold's output publishes through the member's own spec `equals` gate — the same gate that makes `updated()`'s law true — never through a second, invisible predicate inside the deriver. **The framework's reconnect story**: a derived cell is an ordinary cell on the wire (store snapshot, then changes), so consumers can't tell it was derived.

The algebra has more shapes than this fold — the workhorse for keyed pollers is **`pollCollection`** (poll an external source, diff against the served snapshot through the collection's declared `equals`, upsert changed keys, evict vanished ones — the single most-repeated hand-roll in kolu's and drishti's trees, three production instances in one drishti function alone), and `scanCell` carries state for folds that need memory. The full catalog — the reflex→surface mapping, every shipping signature with wire semantics, what each deletes, and what is deliberately absent — is [the derivation algebra reference](surface-derivation-algebra.html).
...
Phase 1 changes how each is **produced**, not what the wire carries. kolu's urgency becomes the declared `foldCollection` above — the hand-wired recompute at every write seam is deleted; the cell, its `equals`, and its consumers are unchanged. drishti's alerts is born as a declared **`scanCell`**, because hysteresis is *carried state*: "crossed 80 to raise, fell below 70 to clear" is undecidable from the current sample alone — the in/out level plus the recent window rides the scan. `scanCell` can seed its state from a persistent store when survival is wanted; the alerts scan deliberately passes none — a fresh process re-derives its level from fresh samples, and the declaration makes that choice visible.
...
// phase 1 re-produces it as a declaration, hand-wiring deleted:
cells.urgency = foldCollection({
  from: "terminals",
  fold: (entries) => ({ awaitingIds: liveAwaitingIds(entries) }),
});  // equals: urgencyEqual stays on the spec — the one dedup point
...
cells.alerts = scanCell(observableSource(systemTicks), hysteresisStep,
  { level: {}, window: [] });
// hysteresisStep raises at 80, clears below 70; a held level publishes
// nothing (alerts' spec equals), so updated() fires only on raise/clear
...
Read the two against each other and the boundary shows itself: the **wire member's provenance** differs (kolu consumes an existing projection; drishti mints W5's one new member), the **writer** differs (a stateless collection fold vs a stateful hysteresis scan — two declarations that merely *rhyme*, deliberately not unified), the **value shape** differs (bare ids vs labeled items), the **aggregation** differs (a sum vs a count), the **click** differs (switch+focus vs expand). What's identical is everything in between — the honest frames, the change pairs, the watcher, the tag-keyed delivery — which is exactly the slice the framework takes.
