...
<Svg svg={pieces} wide caption="Two phases. Phase 1 completes the primitives: the cell reader gains updated() (change pairs, fired iff the value changes) and mirrors stop fabricating first frames. Phase 2 builds attention on those: one writer per host publishes a level-state cell; watchByEntry raises new item ids by set-diffing updated pairs; notify delivers at the origin's one service worker; each app keeps its own aggregation and click semantics." />
...
Phase 1 is a surface refactor PR of its own, landing with paired kolu, drishti, and odu adoption PRs. Two pieces.
...
Run the failure it causes. kolu-server restarts. A still-open page reconnects and reads the attention cell: an empty items list **nobody asserted** — the mirror needed something to show, so it showed the default. Then the authority's real value lands: the same two terminals that have needed you for an hour. Any hand-diff sees "empty → two items" and fires **duplicate OS notifications** for old news. The consumer did nothing wrong; the wire handed it a fabrication and called it truth.
...
**Attention is level state**: *what needs you now* — a plain value, in an ordinary per-entry cell, written by the **one endpoint per host that can actually know it**. For kolu that is padi's awaiting fold. For drishti it is the server, because the server holds the metrics history — and threshold plus hysteresis need history, so they live there, never in the browser.

Why a cell and not an event member? Check the contracts table: an event is an occurrence with **no snapshot** — fired while the browser is closed, or while the link is flapping, it is simply *gone*. Attention must survive exactly those windows: the whole feature is "hear about the host you are *not* watching". A cell replays current truth on every (re)connect, and phase 1's `updated` tells you honestly whether that replay changed anything. Level state plus honest change detection is the entire wire story.
...
drishti (the paired consumer — its adoption lands paired):
...
Read the two against each other and the boundary shows itself: the **writer** differs (an awaiting fold vs a metrics threshold — two folds that merely *rhyme*, deliberately not unified), the **value shape** differs, the **aggregation** differs (a sum vs a count), the **click** differs (switch+focus vs expand). What's identical is everything in between — the honest frames, the change pairs, the watcher, the tag-keyed delivery — which is exactly the slice the framework takes.
