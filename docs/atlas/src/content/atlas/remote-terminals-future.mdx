...
**F7 · Honest clocks (the deferred W4 piece 5)** — the named fast-follow. The offset half now ships: `makeSession` measures the RTT-compensated offset off the framework-reserved `system.clockNow` per bind and carries it on the session's `connected` state, folded into `EntryStatus.connected` (PR3 — graduated off the removed padi-specific `control.core.clockNow`). What remains is translating host-stamped times through that offset in the client's formatters. *Prerequisite hygiene for any cross-host feature that displays time; W5 was shaped to not need it. Size: small.*
