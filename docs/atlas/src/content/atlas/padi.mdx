...
### W4 — the switch *(its own phase — promoted out of W3, srid 2026-07-05: the largest change since W2.2; READY once L6 + L23 merge)*
...
1. **The server holds a pool, not a choice.** Instead of picking one padi at boot, kolu-server keeps a warm connection per machine, opened on demand. Everything W3.1 built (provisioning, reconnecting, converge-safely) is already per-connection — the pool just holds several. *(This is why L6 + L23 gate the phase: they make the layer being multiplied clean and single-policy first.)*
