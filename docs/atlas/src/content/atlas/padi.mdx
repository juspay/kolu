...
### W6 — the honest connect *(client-only; the data already flows — a small, high-value UX phase)*
...
**The work (client-only, no daemon change; lands as its own follow-up PR after surface-map merges — srid-phased):** the connect overlay reads the active entry's state from `entries` and renders honestly —
- **`copying`** → a distinct message naming the host and the act ("Provisioning padi on `zest`… first connect builds it, this can take a few minutes"), with a **live tail** of the last N `progressLines` (dimmed monospace) and an **elapsed timer** so long-but-progressing never reads as hung;
- **`connecting`** → the brief post-copy handshake, distinct from copying;
- **`failed`** → the real `lastError` + the `progressLines` tail (the actual "why"), with `network` ("`zest` unreachable — retrying") read differently from `remote` ("`zest` rejected the connection");
- the **fast path stays calm** — a warm reconnect (no copy) shows `connected` immediately with no alarming build UI; the heavy provisioning treatment appears only when `copying` actually fires.
...
- **Done:** connect to a fresh remote host (cold Nix build) → the overlay names the provisioning phase, streams live build lines, and shows elapsed — never a mute "Connecting…"; a warm reconnect connects fast with no build UI; a genuine failure shows the real error tail, and unreachable reads differently from rejected.
