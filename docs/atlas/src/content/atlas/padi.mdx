...
**Known remote-mode gaps — exactly two, both loud, both W3.2-owned:** (1) **file preview** — a 501 under a remote binding. Root cause: `padiSurface.preview.read` (range-capable, forwards remotely) has existed since W2, but the route shares the *implementation function* as a direct local-disk read instead of dialing the procedure — invisible on one machine, wrong-host on two, hence the fail-closed guard. (2) **the "Running daemons" list** — **CLOSED (#1686, merged 2026-07-05)**: the list now shows the connected machine's daemons (padi reports its own host via the `hostInventory` member, contract 1.2), with each group labeled by machine. The W3 overview's "full peer… nothing silently wrong-host" promise completes when (1) closes too. Standing permanently: **the ssh-user caveat** — the remote padi runs as the SSH user; 0700 sockets make the SSH identity the daemon owner.
