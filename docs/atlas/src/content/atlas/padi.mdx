...
- **PR "pick your host" — the switch takes effect on reload. ≈ a #1675-sized PR, low-to-medium risk (additive; no lifetime assumptions change).** Server: the warm pool (several `BoundPadi`, created on demand, replacing the single boot-time choice — the layer L6+L23 just cleaned); each browser connection declares its host (consuming the per-connection router machinery W2.1 built); clock offset measured per bind (`clock.now`, consumed at last); `preferences.recentHosts`; the inherited `BoundPadi.readPreview` fold-in (deferred from #1685). Client: the ChromeBar picker (ssh-config + free text + recents); loud connecting/degraded/incompatible states; picking stores the tab's choice and **reloads the tab** (~a second against a warm pool). Ships as a complete, honest multi-host kolu — laptop on `zest`, phone on `sincereintent`, recents shared. **The misroute bug is impossible by construction here**: one binding per page-lifetime — there is never a second host mid-session to misroute to. *Done: an e2e proves two tabs on two hosts stay independent; pick → reload → the remote canvas; recents persist across devices; create refused while warming; local single-host mode byte-identical.*
