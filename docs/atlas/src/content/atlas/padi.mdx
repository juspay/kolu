...
- **Upstream padi-tui's wait-until-predicate helper into `@kolu/surface`** — gated on [kolu-tui](kolu-tui.html) shipping (the second consumer that proves the boundary; the helper carries a ledger tag comment in `packages/padi-tui/src/`). If kolu-tui never ships, delete this item with that reason — never upstream on one consumer.
