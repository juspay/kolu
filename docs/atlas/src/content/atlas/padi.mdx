...
// @kolu/surface — the derivation algebra: a derived member is DECLARED
// (foldCollection · scanCell · pollCollection · …), one writer boot-enforced
// (mutation verbs incl. test__set rejected), dedup solely at the member's
// spec equals, the framework's own reconnect story. Full catalog + wire
// semantics: surface-derivation-algebra.html.
...
```ts
// padi — no new member (cells.urgency exists: packages/padi/src/surface.ts);
// phase 1 re-produces it as a declaration, the hand-wiring deleted:
cells.urgency = foldCollection({
  from: "terminals",
  fold: (entries) => ({ awaitingIds: liveAwaitingIds(entries) }),
});  // cell, equals (urgencyEqual), and consumers unchanged on the wire
...
```ts
// server — the one writer, declared: hysteresis is carried state (no store —
// the alert level must not survive restarts; a fresh process re-derives it)
...
Two live edges. **K1, consciously excluded:** the owner retains only *cheap, client-owned* state (focus, MRU, attention, camera, the restore latch) — the **wire subscriptions** (`terminals`, saved session) stay active-host-only, and retaining those for *instant* switch-back is a deliberate future improvement, not this phase: the owner holds state, never sockets. **Field acceptance still open:** srid confirms the live camera symptom gone on a **real deploy** (the timing a unit pin structurally can't see) before this phase closes.
