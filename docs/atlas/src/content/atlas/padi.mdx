...
**The defect, precisely (corrected 2026-07-12 after the incident forensics + a live kill-9 — the seedling's exit-flush guess was falsified):** the facts are **not** written at exit — padi already journals `restoreTarget` on every change (the always-on autosave). The real defect: an unclean kaval death makes each terminal's agent sensor emit an authoritative *agent-ended* — because it can no longer **observe** the terminal through the dead pty-host — and the fold turns that into `restoreTarget: none`, which the always-on autosave journals **over** the good `exact` targets ~0.5s later, before any restore. On 2026-07-12 the false *ended* fired at 09:38:39.923, **~3ms before** the endpoint was even marked `degraded`, so no freeze/park-on-degraded guard could win that race — the emit itself is the defect. **The cure:** give *cannot observe* its own spelling. A resolved-null while the terminal's pty-host taps have **failed** (an unclean death severs the socket) is ignorance — the sensor emits `unknown` and the fold **keeps** the last agent and its resume id; a resolved-null while the taps are **live** is a genuine end and still **clears** the target, so restore never resurrects a dead agent. The resume id survives an unclean death by construction.

**Shipped** <PrLink pr={1784} />. The grounding overturned the seedling's two guesses: the resume facts live in **padi, not kaval** (kaval persists no session file at all), and they were **already** journaled on change — so the "move exit-flush to change-journaling" framing was moot. The fix lives entirely in `@kolu/padi`: the agent sensor's **observability gate** (an unobservable terminal's resolved-null emits `unknown`, not an authoritative end) plus a restore-respawn **seed** so restore re-persists the surviving `exact`, never `none`. Witnessed on a real kill-9 — `restoreTarget` stayed `exact` across the kill (100/100 polls) and restore brought the agent back on its exact prior session (same `sessionId` + transcript). Independent of W10: that lives in kolu-server's host pool; this lives in the per-host padi.
