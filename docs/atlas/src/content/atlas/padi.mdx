...
export const ROADMAP = [
  ...
  { d: 0, g: "○", c: "prog", l: "W12 · restore survives an unclean kaval death — agent-resume facts journaled on change", m: "ratified 2026-07-12 · next up", h: "#w12" },
...
<Phase id="W12" name="restore survives an unclean kaval death — agent-resume facts journaled on change, not flushed at exit" status="next" />
...
1. **W12 — restore survives an unclean kaval death** is next: [W10](#w10) (<PrLink pr={1772} />) shipped 2026-07-12 in the ratified conf-store shape, clearing the numbered ladder. W12 is independent of it in code (it lives in kaval, not the host pool) and is still a seedling — it needs its own interview and grounding before build.
2. **The cleanup ledger runs in parallel** — as a few substantial refactor campaigns, not one-PR-per-entry; the campaign plan lives in [the ledger note](padi-cleanup.html).
3. **Parked, not forgotten:** W3.4 (remote e2e parity in CI) waits on the mock-agent burn-down (<PrLink pr={1689} />); W4's honest-clocks piece 5 is a named fast-follow; W7's field-acceptance leg (srid confirms the camera symptom gone on a real deploy) stays open until observed; W5's drishti pair ([srid/drishti#93](https://github.com/srid/drishti/pull/93) — the `alerts` cell) is still in review.
