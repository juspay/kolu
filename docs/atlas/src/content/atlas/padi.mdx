...
**In simple words:** today kolu marries one machine when the server starts — everything you see comes from that one machine until you restart it. After this phase, kolu keeps connections to several machines warm at once, and **each browser tab picks which machine it's looking at** — switched instantly from a picker in the top bar, like switching tmux sessions. Your laptop tab can sit on `zest` while your phone sits on `sincereintent`, against the same kolu, neither disturbing the other.
...
4. **The picker, honestly.** A host list in the ChromeBar (your `~/.ssh/config`, free typing, and recents), loud connecting/degraded/incompatible states while a binding warms, creating a terminal refused until it's ready — never a frozen canvas pretending.
5. **Honest clocks.** Timestamps are stamped on each host's own clock; the binder measures the offset at each bind (the control core's `clock.now`, RTT-halved — served since W2.2, consumed at last) so "3m ago" is true no matter which machine said it.

**Decided (2026-07-05, srid):** `KOLU_PADI_HOST` sets the **default host only** — the picker switches freely away from it (W3.4's CI boots through it, and CI never opens a picker). **Recently seen servers** live server-side in the existing preferences store — `preferences.recentHosts` (`PreferencesSchema` in `packages/common/src/surface.ts`, persisted by `packages/server/src/preferences.ts` into `~/.config/kolu/config.json`) — so every device shares one list; each tab's "which host am I on" and per-host view posture (zoom, panels) live in browser localStorage, namespaced per host, deliberately per-view.

**TWO PRs (decided 2026-07-05, srid) — the dividing line is one question: does switching reload the page?** Each is end-to-end and fully consumed per this note's law; the second upgrades the first, never completes it.

- **PR "pick your host" — the switch takes effect on reload. ≈ a #1675-sized PR, low-to-medium risk (additive; no lifetime assumptions change).** Server: the warm pool — `buildHostRegistry` holding one `PadiSession` (`DaemonSession<PadiSurfaceClient>`, `padiSession.ts` — #1705 deleted the old `BoundPadi` name) per machine, created on demand via `buildEntry` (local arm for the local host, remote arm otherwise), each entry's `reServeSurface` router dispatched by the connection's declared host; **no `controls`** (kolu needs no fleet verbs — so `pool.reconnect` doesn't even typecheck, per the registry's design). Each browser connection declares its host (the per-connection router machinery W2.1 built); clock offset measured per bind (`clock.now`, consumed at last); `preferences.recentHosts`; and the preview route reads through the bound session — killing its `remoteHost` fork (deferred from #1685). Client: the host picker **lives under Debug → command palette for now, not the ChromeBar** (ssh-config + free text + recents) — **decided 2026-07-05 (srid): the palette item and `KOLU_PADI_HOST` are BOTH undocumented, and together the ONLY way to reach remote terminals until the feature stabilizes** (no discoverable surface = dogfood-only, no support burden; the ChromeBar picker graduates when it's stable). Loud connecting/degraded/incompatible states; picking stores the tab's choice and **reloads the tab** (~a second against a warm pool). Ships as a complete, honest multi-host kolu — laptop on `zest`, phone on `sincereintent`, recents shared. **The misroute bug is impossible by construction here**: one binding per page-lifetime — there is never a second host mid-session to misroute to. *Done: an e2e proves two tabs on two hosts stay independent; pick → reload → the remote canvas; recents persist across devices; create refused while warming; local single-host mode byte-identical.*
- **PR "switch without reload" — the electricity PR (decided 2026-07-05, srid): the risky machinery lands in the FRAMEWORK, kolu consumes it.** The client's real mud is one buried assumption — *the wire lives exactly as long as the page* — and that client-lifetime volatility already has a receptacle: `@kolu/surface-app`, which today assumes ONE static client handed to a provider at startup. This PR grows it the **ratified accessor shape** (the three-AI debate settled it, S5 — *smaller* than the "dynamic client scope" first planned here): `SurfaceAppProvider` takes `controlPlane: Accessor<ControlPlane>` instead of a static object — today's callers just write `() => surfaceApp` — and every subscription keys off the accessor, so a swap tears down and re-subscribes as a FRAMEWORK guarantee, tested once, where the gray-chip class of bug goes to die. **Drishti is the real second consumer** (a fleet view is exactly "N live mirrored clients whose set changes"), so the drishti ship-gate is not a tax here — it IS the second-consumer proof, the W2.1 precedent. Kolu's side shrinks to what is genuinely kolu: a thin `binding/` module (which host · picker wiring · recents · localStorage) that produces clients and hands them to the scope — components stop being ALLOWED to assume a global wire, because the wire arrives through scope; that is the de-entangling, structural not cosmetic. The mid-session misroute guard lands co-located as before: binding-scoped clients whose ids cannot cross — a stale in-flight call from the old host is rejected, not misrouted. **First implementation step, named: verify `surface-app`'s current provider shape can grow this WITHOUT a breaking change for its existing consumers (kolu today, drishti's static uses) — if it can't, STOP and bring the break to the coordinator before building.** *Done: the live-switch e2e — local → remote → local with tiles live on both ends, a stale in-flight call rejected, the switch never disturbing another device; paired drishti PR CI-green at the final kolu HEAD; reverting this PR alone leaves a working multi-host kolu (the reload path).* The ledger's L11 sweep follows it.

*Considered — a `packages/padi-web-client` package (srid, 2026-07-05): rejected — an app module in a receptacle costume, population one. But the QUESTION behind it ("is there an electricity that de-entangles the client?") had a real answer, adopted above: the volatility is CLIENT LIFETIME, its receptacle already exists (`@kolu/surface-app`) and was one axis short (static client → dynamic scope). PR-2 is that electricity landing in its true home, with drishti as the proving second consumer.*
