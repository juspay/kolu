...
> **In review: <PrLink pr={1764} />.** Ships **both** halves of the symptom — the per-host wire subscriptions (the canvas) and the Code-tab `createPolledQuery` per-host scoping (the Code tab). No server/padi changes; xterm/WebGL stay active-host-only.
