...
export const ROADMAP = [
  ...
  { d: 0, g: "▶", c: "prog", l: "W9 · instant host switch-back — retained per-host wire subscriptions (W7's K1)", m: "#1764 · in review", h: "#w9" },
...
<Phase id="W9" name="instant host switch-back — retained per-host wire subscriptions (completing W7's K1)" status="next" needs={["W7"]} links={[{ label: "#1764", href: "https://github.com/juspay/kolu/pull/1764" }]} />
...
### W9 — instant host switch-back *(in review — completing W7's K1)*

> **In review: <PrLink pr={1764} />.** Ships **both** halves of the symptom — the per-host wire subscriptions in the retained `scopedByEntry` owner (the canvas) and the Code-tab reads retained per host in a **parallel** owner `hostCodeTab` (the Code tab). No server/padi changes; xterm/WebGL stay active-host-only.
...
**The fix is W7's own cure applied to its one exclusion (K1):** the per-host wire subscriptions — terminal keys, `terminals`, saved session, daemon status, activity feed — move into the retained `scopedByEntry` per-host owner (`createHostWire`): created lazily on first activation, retained across switch-away, disposed only on membership exit. The Code tab's own reads — git status, file list, diff, and file content — get the **same** ownership, but in a **parallel** per-host owner (`hostCodeTab`), not a member of the wire owner: those reads key on view-selection state (`useTerminalStore` / `useRightPanel`) that sits *downstream* of the wire owner in the import graph, so folding it in would close a `biome noImportCycles` cycle — the graph is telling the truth about layering. Same primitive, same `padiMap`+`activeHost` inputs, so the two owners' lifetimes are parallel by construction. Switch-back — **and** a `canvasMode` round-trip that unmounts the Code tab — then have no resubscribe at all, so no pending window *can* exist. (Keep-last-value or holding the mode would leave the defect expressible per readout — enumeration, the bug-shape W7 was built to kill; the component-scoped constructor that expressed it is retired outright.)
...
**Same class, same cure — shipped in the same PR (<PrLink pr={1764} />):** `createPolledQuery` also blanked the Code-tab queries on a *genuine* host change; W9 scopes the query per host by **ownership**, the same shape as the wire subs — `perHostPolledQuery` builds one `createPolledQuery` per host inside the `scopedByEntry` owner and gates it on `ctx.isActive`, so each host's query is **paused** while backgrounded (value held, pulse torn down — no background polling), **resumed** from its held value on switch-BACK with no blank, and **disposed** when the host leaves the pool. No cache, no LRU, no key enumeration — the defect class is unrepresentable, not merely bounded. Folding it in here kills the *whole* symptom (dock, RightPanel, **and** Code tab) at once. (<PrLink pr={1758} /> already fixed the *spurious* blank — a fresh-reference-but-equal-value input no longer blanks, pinned by tests — and W9 builds on that value-keyed contract, never undoes it: the active-path blank behavior is unchanged.)
