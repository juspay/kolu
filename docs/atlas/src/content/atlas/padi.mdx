...
**Cross-host dock** (B-lite: foreign hosts' rows, urgency-ranked, click = switch + focus — the host axis is already in the contract) is the first escalation if switching proves insufficient. **kolu-tui** — the graduation proof, now a ratified plan of its own: [kolu-tui](kolu-tui.html) owns the criterion (the named padiSurface path incl. the `terminalAttach` `{seq}` stream, both transports, or it doesn't count). **Hybrid canvas** (model B) only if one-window side-by-side proves a recurring demand — it stays an aggregation layer over N bindings, zero daemon changes.
...
| `kolu-tui` | **planned** — [kolu-tui](kolu-tui.html), new package | a new consumer app, not an extraction — it exists to prove padiSurface serves a second frontend (test ③'s proof); the plan note owns scope + criterion |
