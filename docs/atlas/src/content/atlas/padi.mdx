...
- **Unify the wait-until-predicate scaffold shared by padi-tui and kaval-tui** — the duplication is REAL today (padi-tui `read.ts:awaitAgentState` and kaval-tui `wait.ts:awaitOutputCondition` share a near-verbatim ~40-line scaffold: the 5-arm `WaitOutcome` union, AbortSignal chaining, first-writer-wins settle — held in lockstep by JSDoc cross-references), but this item's earlier form was doubly wrong: the "ledger tag comment in `packages/padi-tui/src/`" does not exist (grep confirms), and the kolu-tui gate was misdrawn — kolu-tui consumes neither helper, so its shipping proves nothing here. Real gate: a third consumer of the *scaffold*, or lockstep drift actually biting. **That gate is now met on paper**: [kolu-tui](kolu-cli.html)'s MCP face plans composite `wait.*` tools built on the same scaffold — its PR extracts the shared leaf instead of hand-rolling copy #3.
...
**Cross-host dock** (B-lite: foreign hosts' rows, urgency-ranked, click = switch + focus — the host axis is already in the contract) is the first escalation if switching proves insufficient. **kolu-tui** — the graduation proof, now a ratified plan of its own: [kolu-tui](kolu-cli.html) owns the criterion (the named padiSurface path incl. the `terminalAttach` `{seq}` stream, both transports, or it doesn't count). **Hybrid canvas** (model B) only if one-window side-by-side proves a recurring demand — it stays an aggregation layer over N bindings, zero daemon changes.
...
| `kolu-tui` | **planned** — [kolu-tui](kolu-cli.html), new package | a new consumer app, not an extraction — it exists to prove padiSurface serves a second frontend (test ③'s proof); the plan note owns scope + criterion |
