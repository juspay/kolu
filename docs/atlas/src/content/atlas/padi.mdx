...
<Phase id="W3" name="remote — the ssh/Nix binding, the full-peer gaps, CI parity" status="todo" needs={["W2"]} />
<Phase id="W4" name="the switch — host picker, per-view bindings, warm pool" status="todo" needs={["W3"]} />
...
### W4 — the switch *(its own phase — promoted out of W3; the largest change since W2.2. READY NOW — every gate merged 2026-07-05: L6 <PrLink pr={1698} /> · L23 <PrLink pr={1700} /> · the surface-hosting simplification <PrLink pr={1705} />, which reshaped this phase's vocabulary — see the [ratified ledger](surface-hosting-101.html))*
...
### W7 — per-host state by ownership, not enumeration *(the class fix for the "forgotten field" bug family; ratified 2026-07-08; its own post-merge PR)*
...
**Status — IMPLEMENTED (2026-07-08):** shipped on kolu branch `w7-per-host-ownership` + its [`srid/drishti#91`](https://github.com/srid/drishti/pull/91) framework-gate pair. `scopedByEntry` lives in `@kolu/surface-map` (over `@solid-primitives/keyed`'s `keyArray`, plus the `codec` now exported on `SurfaceMapClient`); kolu's `hostScope/hostScopes` dissolves the `HostView` record, `ensureHost`, the camera-swap seam (`useCanvasCameraSwap`, **deleted**), `useCanvasViewport`'s three module-scope signals, and `useSessionRestore`'s hand-rolled latch `Map` into one per-host owner. The facades (`useViewState`, `useCanvasViewport`, `useSessionRestore`) became windows onto `activeScope()`; the ~35 store consumers are untouched. The boundary guard (`canvas/canvasBoundaryGuard.test.ts`) is the residual fence, and the comment-only `HOST-SCOPING:` convention retired into it.
...
### W8 — remote terminals, documented honestly *(a docs phase: the ssh-session environment gap; seeded by a live srid incident 2026-07-08)*
...
### W6 — the honest connect *(overlay + the connection type made perfect; ~a day, drishti-gated)*
...
type SessionState<Prov extends string = never> = {
  log: readonly { source: "local" | "remote"; line: string }[];   // last 20, never reset
} & (
...
// browser cell: z.discriminatedUnion("phase", …) mirroring the SAME arms
// (up arms carry only log; disconnected carries error+cause both; failed carries cause:"remote" only)
```

DELETED by this reshape: `remoteProgressLines` (a stored duplicate — now `log.filter(source === "remote")`), the `[local] `/`[remote] ` in-band string prefixes (provenance becomes a field), and the `ProvisioningPhase`/`UpConnectionState`/`LocalConnectionState`/`FailureCause`/`SessionStateCommon` name-chain (one type + one connector vocabulary + one zod const replace all seven names). The `copying`→`copying|building` split is the connector updating its own declared vocabulary — zero framework change, and `nixCopy.ts` already runs the two steps as separate commands. **Wire-schema change ⇒ the drishti gate applies** (drishti reads the cell at `App.tsx:690`; paired PR, green at final kolu HEAD).
