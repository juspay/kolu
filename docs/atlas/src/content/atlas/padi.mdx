...
**Open decision at dispatch (srid rules), constrained by W3.4:** the fate of `KOLU_PADI_HOST` once the picker exists. "Dies entirely" is off the table — W3.4's remote CI lane boots kolu-server with the knob (a headless run can't click a picker), so it survives as the deterministic boot-time binding. What's actually open: its semantics under the picker — a default-host **seed** views may switch away from, or a **pinned** binding the picker can't override.
