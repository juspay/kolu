...
**Known remote-mode gaps — exactly two, both loud, both W3.2-owned:** (1) **file preview** — a 501 under a remote binding. Root cause: `padiSurface.preview.read` (range-capable, forwards remotely) has existed since W2, but the route shares the *implementation function* as a direct local-disk read instead of dialing the procedure — invisible on one machine, wrong-host on two, hence the fail-closed guard. (2) **the "Running daemons" list** — it only scans the machine kolu-server runs on, even when connected to another machine (it says so in the dialog, so it can't mislead — but it also can't show you a leaked daemon on the machine you're actually using). The W3 overview's "full peer… nothing silently wrong-host" promise completes when W3.2 closes both. Standing permanently: **the ssh-user caveat** — the remote padi runs as the SSH user; 0700 sockets make the SSH identity the daemon owner.
...
- **PR "preview" — self-sufficient TODAY, needs no switch.** Clicking a file while connected to a remote machine currently shows "preview unavailable" (a deliberate refusal, so kolu never shows you the wrong machine's file). This PR makes preview actually work remotely: the route asks padi for the file's bytes over the existing `preview.read` procedure (fetched in chunks, so big files don't blow memory) instead of reading the local disk. Works with today's `KOLU_PADI_HOST` setup — no picker needed.
- **PR "inventory" — self-sufficient TODAY, needs no switch.** The Kaval dialog's "Running daemons" list currently only scans YOUR machine, even when kolu is connected to another one. This PR makes it also list what's running on the connected machine — your padi and kaval there, plus anything leaked there — with each list labeled by which machine it describes. Also works with today's setup — no picker needed.
