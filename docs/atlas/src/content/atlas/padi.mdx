...
export const ROADMAP = [
  ...
  { d: 0, g: "○", c: "prog", l: "W9 · instant host switch-back — retained per-host wire subscriptions (W7's K1)", m: "next — W5 ✓", h: "#w9" },
  { d: 0, g: "○", c: "prog", l: "W11 · one verb per fact — user-remove vs system-retire in the pool", m: "ratified 2026-07-12 · blocks W10", h: "#w11" },
  { d: 0, g: "▶", c: "prog", l: "W10 · hosts survive a restart — pool membership persisted beside its one writer", m: "#1772 in gauntlet · BLOCKED on W11", h: "#w10" },
  { d: 0, g: "○", c: "prog", l: "W12 · restore survives an unclean kaval death — agent-resume facts journaled on change", m: "ratified 2026-07-12 · independent", h: "#w12" },
  { d: 0, g: "▶", c: "prog", l: "Consolidation — the cleanup ledger (unnumbered · parallel · ongoing)", m: "→ the ledger note", h: "padi-cleanup.html" },
...
<Phase id="W9" name="instant host switch-back — retained per-host wire subscriptions (completing W7's K1)" status="todo" needs={["W7"]} />
<Phase id="W11" name="one verb per fact — user-remove vs system-retire in surface-remote's pool" status="next" needs={["W6"]} blocks={["W10"]} />
<Phase id="W10" name="hosts survive a restart — pool membership persisted beside its one writer" status="todo" needs={["W6", "W11"]} links={[{ label: "#1772 (draft, in gauntlet)", href: "https://github.com/juspay/kolu/pull/1772" }]} />
<Phase id="W12" name="restore survives an unclean kaval death — agent-resume facts journaled on change, not flushed at exit" status="todo" />
...
1. **W9 — instant host switch-back** is the next feature phase: W5 shipped (<PrLink pr={1759} />, 2026-07-11), and a 2026-07-11 root-cause pinned why every host switch still rebuilds the whole canvas — the cure is completing W7's K1 (below).
2. **W11 — one verb per fact** lands first as its own small PR (ratified 2026-07-12): W10's gauntlet found that persisting removals would let an internal fault path silently forget a user-added host — the verbs split before W10 may merge.
3. **W10 — hosts survive a restart**: the implementation (<PrLink pr={1772} />) is complete and in the gauntlet, but **BLOCKED on W11** — it stays draft, rebases on W11, then ships.
4. **The cleanup ledger runs in parallel** — as a few substantial refactor campaigns, not one-PR-per-entry; the campaign plan lives in [the ledger note](padi-cleanup.html).
5. **Parked, not forgotten:** W3.4 (remote e2e parity in CI) waits on the mock-agent burn-down (<PrLink pr={1689} />); W4's honest-clocks piece 5 is a named fast-follow; W7's field-acceptance leg (srid confirms the camera symptom gone on a real deploy) stays open until observed; W5's drishti pair ([srid/drishti#93](https://github.com/srid/drishti/pull/93) — the `alerts` cell) is still in review.
...
### W9 — instant host switch-back *(next up — completing W7's K1)*

**In simple words:** switching hosts is supposed to feel like switching tmux sessions — instant, the canvas already there. Today every switch-back rebuilds the whole canvas instead: the dock empties and refills, the terminal goes black, the Code tab drops to "Loading…" — about a second of churn for state the client held moments ago.

**The cause, in one line:** every per-host wire readout keys on `activeHost` with dispose-on-key-change, so switch-back reopens each readout from *pending* — pending data makes `canvasMode` leave `workspace`, and the `<Switch>` arm unmounts Dock, tiles, and RightPanel wholesale.<Footnote>The chain, frame-grounded against a screen recording (root-caused 2026-07-11): every per-host wire readout keys on `activeHost` via `useEntry`/`createKeyedRoot` (`packages/client/src/wire.ts`, `useTerminalMetadata.ts`), so switch-back reopens the terminal-keys stream, the metadata collection, daemon status, and session cells from pending; pending data makes `canvasModeResolver` leave `workspace`, so `App.tsx`'s `<Switch>` unmounts the entire workspace arm, which remounts from zero as records trickle back — the Dock refills incrementally, the terminal goes black while xterm content refetches, and the Code tab drops to "Loading…". Transport stays warm; the ~1s is purely logical resubscribe + refetch, and it is distinct from the reprojectClock identity churn #1714 introduced — that flicker is fixed by <PrLink pr={1758} />, which does not fix the host-switch rebuild.</Footnote>

**The fix is W7's own cure applied to its one exclusion (K1):** the per-host wire subscriptions — terminal keys, `terminals`, saved session, daemon status, activity feed — move into the retained `scopedByEntry` per-host owner: created lazily on first activation, retained across switch-away, disposed only on membership exit. Switch-back then has no resubscribe at all, so no pending window *can* exist. (Keep-last-value or holding the mode would leave the defect expressible per readout — enumeration, the bug-shape W7 was built to kill.)

**Deliberately excluded:** xterm/WebGL stay active-host-only — a sub-second re-attach paint remains, mitigable later via a per-host screen snapshot. No server or padi changes.

**Same class, same cure, as a follow-on:** `createPolledQuery` still blanks the Code-tab queries on a *genuine* host change — scoping the query per host removes it. (<PrLink pr={1758} /> already fixed the *spurious* blank — a fresh-reference-but-equal-value input no longer blanks, pinned by tests — and W9 builds on that value-keyed contract, never undoes it.)

<a id="w10"></a>

### W10 — hosts survive a restart *(implementation <PrLink pr={1772} /> in gauntlet — BLOCKED on [W11](#w11))*

**Status (2026-07-12):** the implementation (<PrLink pr={1772} />) is complete and in the review gauntlet, but **BLOCKED on [W11](#w11)** — the gauntlet found that persisting removals would make <Issue n={1708} />'s pump-death auto-retire durably forget a user-added host; #1772 stays draft and rebases on W11 before shipping.

**In simple words:** the hosts you added through the selector strip come back after a kolu restart. Today a restart forgets everything but the local default, and you re-add your fleet by hand. After W10, every remembered host reappears in the strip and reconnects through the normal connect pipeline; a host that's gone shows its honest failed chip with the cause rather than silently vanishing. The only way a host leaves the strip is your explicit remove.<Footnote>Sequenced after W9 by number only — there is no code dependency between them: W10 is server-side, W9 is client-side. W9 is already numbered and in flight, so W10 takes the next slot.</Footnote>

**Membership is a server fact, so its memory lives beside its one writer.** kolu-server's pool is the sole authority for who's in the map (P3) — persistence sits next to that authority, never in the browser, where localStorage would fork the one list into per-device copies (every client must see the same list). The persisted artifact is a value replaced whole, not a place edited (P1): a zod-schemed `{version, hosts}` JSON under `KOLU_STATE_DIR` (the existing state root — reuse the source of truth), written atomically (tmp + rename) inside the addHost/removeHost handlers themselves — no shutdown flush, no periodic sync; the file is always the last accepted membership change. It holds only the encoded host keys the pool already speaks — no connection state, no metadata (live facts the sessions re-derive) — and the unremovable local default is excluded: it's seeded in code before the server listens, so persisting it would mint a second authority for "local always exists".

**Boot re-enters through the front door:** after the pool is built, each persisted host goes through the ordinary `pool.add` and flows the W6 connect pipeline — warming → probing → connected, or failed with its honest cause. No lazy-dial mode: surface-remote already retries network faults forever, and a lazy/eager toggle would be a knob. **Fail fast:** a file that exists but fails the schema *crashes the boot* with the path in the message (delete it to recover) — never "start with empty hosts", which silently eats the user's fleet (caught-error-must-not-collapse-to-empty).

**Deliberately not:** no framework change — persistence policy is app-level; if drishti ever wants the same shape, graduate it then (prove-then-extract). No Nix/declarative host-config second path, no TTL or staleness pruning, and the client persists nothing.

**One PR:** a new load/validate/atomic-save module in `packages/server` (schema `{version: 1, hosts: string[]}`), two call-site touches where the handlers live (grep `addHost:` in `packages/server/src/index.ts`, ~line 486 on master), and a boot loop after pool construction. Tests: round-trip add/remove/restart; a corrupt file crashes with the path in the message; the local default never enters the file; remove updates the file. Synergy with W9, not dependency: restart + re-add mints fresh per-host client worlds, composing with W9's retained subscriptions.

<a id="w11"></a>

### W11 — one verb per fact: user-remove vs system-retire *(blocks W10 — ratified 2026-07-12)*

**In simple words:** removing a host should mean the *user* removed it — never the system giving up on a bad connection. Today one internal fault path — <Issue n={1708} />'s pump-death auto-retire — calls the same `pool.remove` the user's ✕ button does; once W10 persists removals, a rare internal guest fault would silently shrink the remembered fleet across restarts.<Footnote>Silent fleet shrink is exactly the data-loss class W10's crash-on-corrupt doctrine exists to prevent — persistence must never make a system fault look like a user decision.</Footnote> W11 splits the verbs so the system can retire a faulted session without touching what the user asked kolu to remember.

**The root defect: surface-remote's pool speaks ONE verb for TWO facts** — "the user removed this host" vs "the system retired a faulted session". Per the two-constructors-not-a-mode-flag doctrine, the pool gains two verbs (P4: two facts, two spellings): the persisting user-remove and a non-persisting internal retire, with the persist hook firing only on user-intent mutations — the one authority for "the remembered fleet" is the user. kolu wires #1708's pump-death path to the internal retire. Framework change → the drishti gate check applies (pair only if drishti consumes the touched API).

**Sequencing:** W11 lands first as its own small PR; W10's implementation PR (<PrLink pr={1772} />, in gauntlet, otherwise complete) stays draft, rebases on W11, then ships. Honestly noted: the wart is pre-existing on master — #1708 already un-hosts the live pool on a pump death today; W10 only extends it across restarts. W11 fixes the class for both.

<a id="w12"></a>

### W12 — restore survives an unclean kaval death *(ratified 2026-07-12 — independent of W10/W11)*

**In simple words:** today, if kaval dies cleanly your agents come back after restore — but if it is killed or crashes, you get your terminals back *empty*: the agents are gone, and you restart each one by hand.<Footnote>The 2026-07-12 production incident: prod's kaval was killed un-gracefully; session restore brought back the terminal list, but every agent came back as a bare shell and srid restored them by hand. On zest, a *graceful* kaval restart restores agents fine — the two outcomes differ only in how kaval died.</Footnote> After W12 the two cases look the same: restore brings the agents back either way. The only thing an unclean death may still lose is the last few moments of screen output.

**The defect is the class W10 just fixed for host membership:** the per-terminal agent-resume facts — whatever a graceful shutdown persists today that lets restore respawn the agent — are written at *exit* time, and a crash is precisely the case where the end never runs (SIGKILL, crash, OOM all skip it). The cure is the same: move the facts from exit-time flush to change-time journaling — the persisted record is a value replaced whole on every change (P1), owned by kaval beside its existing state — so the file is always current no matter how the process dies. The graceful-shutdown path stops being load-bearing; it can remain as a final no-op write.

**Seedling — this phase needs its own interview and grounding before build.** The first implementation step is to ground what "agent-resume facts" concretely *are* in kaval's current code — what the graceful path persists today — rather than guessing the field list here. Independent of W10 and W11: those live in kolu-server's host pool, this lives in kaval.

<a id="cleanup"></a>

### Consolidation — the cleanup ledger *(unnumbered — a parallel, ongoing track; scope lives in [its own note](padi-cleanup.html))*

W2 and W3 ship at pace (a deliberate call); the ledger pays the pace's debts — refactors, decomplecting, framework-level electricity upstreaming, deletions. Nothing user-visible ships here; that is the point. **The full ledger — each entry a self-sufficient, lens-framed brief an implementing agent can execute under coordinator guidance — lives in [padi — the consolidation ledger](padi-cleanup.html)**, appraised and appended as each W phase lands (a shipped phase's "deferred / acceptable-for-scope" review items land there explicitly; an entry that never ripens is deleted with a reason). The index, for orientation:

- **Shipped:** campaigns 2 + 3 in one PR (<PrLink pr={1747} /> — L27 server re-cut · L17 vocab homes · L13 seal flank · L11 wire sweep · L18 HydrationPhase) · L3 the convergence kit (<PrLink pr={1674} />) · L5 both halves (ProcessRss in W2.2; video-ext via L17 in #1747) · L6 `padiBinding` split (<PrLink pr={1698} />) · L7 `terminal-workspace` re-cut (<PrLink pr={1734} />) · L14 memory via the mirror (<PrLink pr={1699} />) · L15+L16 write-seam/boot invariants (<PrLink pr={1703} />) · L23 remote arm on the kit (<PrLink pr={1700} />) · L26 the `no-overloaded-null` guardrail (<PrLink pr={1744} />)
- **The one remaining campaign** (the plan lives in the ledger note): complete the primitives — its deep move, bridge phase 0, shipped inside W5 (<PrLink pr={1759} />); the residue is L28 rung 2 · L1 · L20·21, with L8's hygiene sweep as its tail commit. L24 (`padi-tui --host`) ships as its own feature PR — a feature never rides a refactor campaign.
...
Still live from the ship: **honest clocks (piece 5) is a named fast-follow** — the serving half (the control core's `clock.now`) has existed since W1, but the consuming half was never built anywhere, so remote hosts' ages mislabel until it lands (a pre-existing W3.1-era gap, not a switch regression). The per-host-state bugs the deploy surfaced (focus, splits, camera) got instance fixes in-PR; their class fix became [W7](#w7). The ledger's L11 sweep (the client's interim wire-shape scaffolding) landed with cleanup campaign 3 (<PrLink pr={1747} />).
...
Two live edges: **K1's completion is [W9](#w9)** — the per-host wire subscriptions the owner deliberately did not retain, now the next feature phase. **Field acceptance still open:** srid confirms the live camera symptom gone on a **real deploy** (the timing a unit pin structurally can't see) before this phase closes.
