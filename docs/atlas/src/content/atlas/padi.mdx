...
- **Shipped:** L3 the convergence kit (<PrLink pr={1674} />) · L5 (in W2.2) · L6 `padiBinding` split (<PrLink pr={1698} />) · L7 `terminal-workspace` re-cut (<PrLink pr={1734} />) · L14 memory via the mirror (<PrLink pr={1699} />) · L15+L16 write-seam/boot invariants (<PrLink pr={1703} />) · L23 remote arm on the kit (<PrLink pr={1700} />) · L26 the `no-overloaded-null` guardrail (<PrLink pr={1744} />)
- **The campaigns** (few substantial refactors — the plan lives in the ledger note): ① complete the primitives — W5 phase-1, absorbing L28 rung 2 · L1 · L20·21 under its one drishti pairing; ② the padi/server re-cut — L27 (opening pure-motion commit) · L17, with L13 riding; ③ the client wire/state sweep — L11 · L18. L8 (the doc sweep) rides every campaign — hygiene never ships standalone. L24 (`padi-tui --host`) ships as its own feature PR — a feature never rides a refactor campaign.
