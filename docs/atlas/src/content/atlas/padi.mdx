...
- **PR "switch without reload" — client-only, zero server delta. ≈ 1k lines, but the risk profile of W1's rewiring stage — every line in the sensitive place.** The client's create-once-per-page wiring becomes per-binding: tear down every live subscription, rebuild against the new host in place, no reload (the biggest client rewiring since W1 — and why this waited for #1687: rebuilt subscriptions behave sanely from their first moment). The mid-session misroute risk *first exists* in this PR, so its structural guard lands co-located: **binding-scoped clients whose ids cannot cross** — a stale in-flight call from the old host is rejected, not misrouted (the #1685 `readPreview` rule: the guard ships with the change that opens the seam). *Done: the live-switch e2e — local → remote → local with tiles live on both ends, a stale in-flight call rejected, the switch never disturbing another device. Reverting this PR alone leaves a working multi-host kolu (the reload path).* The ledger's L11 sweep follows it.

  *Considered — a `packages/padi-web-client` package (srid, 2026-07-05): appealing because a package boundary would make "reach for a page-global" a COMPILE error (the illegal-states principle at the package level — the gray chip was a subscription-lifetime bug). Verdict: **not as part of the switch, probably not a package at all yet.** It doesn't shrink the risky work (the Solid teardown/rebuild exists either way — a boundary relocates it, not reduces it); it fails the electricity test (a padi-specific BROWSER client is an app module, not a domain-agnostic receptacle — `@kolu/surface` already owns that volatility) with a population of ONE consumer (`packages/client`); and wrapping an extraction around PR-2 undoes the single-concern reviewability the split exists for. Instead: build the per-binding lifecycle in-place as a cohesive `binding/` module with a factory API (`bindHost(host) → handle` — components take a handle, never a global), which buys most of the enforcement without a workspace package and makes a later extraction pure motion. Logged as a ledger candidate; the switch's implementing agent makes the final leaf-vs-package call WITH the rewiring in front of it (the verdict is contingent on what the code looks like), not blind here.*
