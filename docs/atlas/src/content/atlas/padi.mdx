...
export const ROADMAP = [
  ...
  { d: 0, g: "○", c: "prog", l: "W4 · the switch — host picker · per-view bindings · warm pool — its OWN phase (largest since W2.2)", m: "READY after L6+L23 merge", h: "#w4" },
...
**Decided (2026-07-05, srid):** `KOLU_PADI_HOST` sets the **default host only** — the picker switches freely away from it (W3.4's CI boots through it, and CI never opens a picker). **Recently seen servers** live server-side in the existing preferences store — `preferences.recentHosts` (`PreferencesSchema` in `packages/common/src/surface.ts`, persisted by `packages/server/src/preferences.ts` into `~/.config/kolu/config.json`) — so every device shares one list; each tab's "which host am I on" and per-host view posture (zoom, panels) live in browser localStorage, namespaced per host, deliberately per-view. **One end-to-end PR** — the server and client halves are commit stages inside it, per this note's law; the inherited `BoundPadi.readPreview` fold-in (deferred from #1685) rides along so "a remote binding reads the local disk" becomes unspellable the moment bindings go per-view.

**The safety property, built in rather than checked:** a click meant for the old machine must never land on the new one ("kill terminal 42" arriving at the wrong host). Requests are born bound to their machine — binding-scoped clients whose ids cannot cross — so the misroute cannot be written, rather than being caught by sprinkled checks.

- **Done:** an e2e switches local → remote → local with tiles live on both ends; a stale in-flight call is **rejected, not misrouted**; two browser views parked on different hosts stay independent; the switch never disturbs another device; picker + recents + loud warming states all exercised by the e2e.
