...
// derived.cell.scan + signalFromCallback (the engine hidden behind it,
// lint-banned elsewhere); a derived member is wire-read-only by
// construction (no ctx entry, no write verbs); rides the existing cell
// connect seam. Full phases + laws: surface-reactive-bridge.html.
...
cells.alerts = derived.cell.scan({
  over: metrics,
  initial: () => noAlerts,   // no store — the alert level must not survive restarts
