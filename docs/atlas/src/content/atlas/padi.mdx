...
**Known remote-mode gaps — exactly two, both loud, both W3.2-owned:** (1) **file preview** — a 501 under a remote binding. Root cause: `padiSurface.preview.read` (range-capable, forwards remotely) has existed since W2, but the route shares the *implementation function* as a direct local-disk read instead of dialing the procedure — invisible on one machine, wrong-host on two, hence the fail-closed guard. (2) **the "Running daemons" inventory** — a local-machine scan only, labeled "this machine, not the bound host"; the bound host's other daemons are invisible. The W3 overview's "full peer… nothing silently wrong-host" promise completes when W3.2 closes both. Standing permanently: **the ssh-user caveat** — the remote padi runs as the SSH user; 0700 sockets make the SSH identity the daemon owner.
...
6. **The two W3.1 gaps** — preview through `preview.read` (ranges looped for streaming; the 501 dies); the inventory's remote arm (the bound host's daemons beside the labeled local scan).
...
- **PR "the switch" — ONE end-to-end PR, the phase's heart.** Server pool + per-connection wire-scope binding + clock offset at bind + the client per-binding rebuild + ChromeBar picker + per-binding loud states + per-binding localStorage/last-binding + the misroute-unrepresentable structure + the switch e2e (local → remote → local, stale in-flight call rejected). The server and client halves are **commit stages inside it** (the W1/W2.2 pattern — only the final state must be green); neither ships alone because neither is consumed alone.
- **PR "preview" — self-sufficient TODAY, needs no switch.** The iframe route dials `padiSurface.preview.read` (ranges looped for streaming) instead of the local-disk read; the 501 dies, e2e-pinned. Complete user-visible value against the existing `KOLU_PADI_HOST` binding.
- **PR "inventory" — self-sufficient TODAY, needs no switch.** The daemon inventory's remote arm: the bound host's daemons listed beside the labeled local scan. Same: complete against the knob, no picker required.
