...
**Same class, same cure — shipped in the same PR (<PrLink pr={1764} />):** `createPolledQuery` also blanked the Code-tab queries on a *genuine* host change; W9 scopes the query per host — an opt-in per-`(input, host)` value cache the Code-tab repo/file queries use, so a switch-BACK adopts the held result (no blank) while the pulse refreshes. Folding it in here kills the *whole* symptom (dock, RightPanel, **and** Code tab) at once. (<PrLink pr={1758} /> already fixed the *spurious* blank — a fresh-reference-but-equal-value input no longer blanks, pinned by tests — and W9 builds on that value-keyed contract, never undoes it: the cache is off by default, so that pinned behavior is unchanged.)
