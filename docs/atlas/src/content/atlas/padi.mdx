...
**In simple words:** today, if an agent on another host needs you, kolu mostly can't tell you — an OS notification escapes the app, but the unread badges live per host and only the host you're currently looking at renders them. After this phase, kolu watches every connected host and rolls the needs-you signal up in one place: each host's chip in the switcher shows a count, the app icon badge sums all hosts, and clicking the OS notification switches to the right host and focuses the terminal that wants you.
...
**The ratified architecture (2026-07-10).**<Footnote>Two grounded reviews: a 12-agent /architecture-first-principles + /perfection-review fan-out (3 ground readers over kolu, drishti, and the framework seams · 3 independent designs · 6 adversarial refuters — the level layer survived every adversary; browser-side frame-diff edge detection was refuted four ways: unordered streams, default-materialization double-fire after restarts, per-window dedup memory, and clear→re-raise misses inside a link flap), then an independent /hickey pass that cut the surface-map braid (attention has nothing to do with maps) and found the hydration lie, re-judged with the fact that drishti's adoption is committed and lands paired.</Footnote> Attention is **level state** ("what needs you now"), written once per host at the endpoint that can know it (padi's awaiting fold; drishti's server, which holds the metrics history — threshold + hysteresis live there), carried as an ordinary per-entry cell. Nothing attention-specific enters `@kolu/surface-map`; the framework's pieces are generic. Three framework packages each gain a piece — `@kolu/surface-remote` (the hydration fact), `@kolu/surface-map` (the kernel, the pure fold, the watcher), `@kolu/surface-app` (delivery) — taught end-to-end in [the attention primer](surface-attention-101.html):
...
// @kolu/surface-map — generic, attention-agnostic:
//  · the membership kernel factored OUT of scopedByEntry (entries fold · codec identity ·
//    keyArray per-key root · dispose on exit) — the lazy scopedByEntry and the new eager
//    watcher both call it; the kernel is never re-derived
...
What stays app-side, deliberately: the domain folds and value shapes (two folds that merely rhyme are not unified), every aggregation, the pixels, the click semantics. One semantic the step fold's property tests must pin before implementation: a clear→re-raise of the same item id entirely inside a connectivity gap (the app-side fallback if level-only proves insufficient is an occurrence counter in the app's value).
