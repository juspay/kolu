...
export const ROADMAP = [
  ...
  { d: 0, g: "○", c: "prog", l: "W5 · cross-host attention — urgency fan-in · OS badge · notification deep-link", m: "next — W4 ✓", h: "#w5" },
...
**As of 2026-07-09, in order:**

1. **W5 — cross-host attention** is the next feature phase: everything it needs is shipped — the warm pool and per-host entries (W4), and now W6 (<PrLink pr={1730} /> + its drishti pair [srid/drishti#92](https://github.com/srid/drishti/pull/92), both merged 2026-07-09) — and the need is user-visible (the 2026-07-08 field note below).
...
3. **Parked, not forgotten:** W3.4 (remote e2e parity in CI) waits on the mock-agent burn-down (<PrLink pr={1689} />); W4's honest-clocks piece 5 is a named fast-follow; W7's field-acceptance leg (srid confirms the camera symptom gone on a real deploy) stays open until observed.
...
### W5 — cross-host attention *(next up — everything it needs shipped with W4)*

**In simple words:** today, if an agent on another host needs you, kolu mostly can't tell you — the host strip's chips already show each host's awaiting count (the urgency cell ships today), but nothing escapes the app for a host you're not looking at. After this phase, kolu rolls the needs-you signal up where it can reach you: the app icon badge sums all hosts, an OS notification fires for a background host, and clicking it switches to the right host and focuses the terminal that wants you.
...
**The ratified architecture (2026-07-10).**<Footnote>Two grounded reviews: a 12-agent /architecture-first-principles + /perfection-review fan-out (3 ground readers over kolu, drishti, and the framework seams · 3 independent designs · 6 adversarial refuters — the level layer survived every adversary; browser-side frame-diff edge detection was refuted four ways: unordered streams, default-materialization double-fire after restarts, per-window dedup memory, and clear→re-raise misses inside a link flap), then an independent /hickey pass that cut the surface-map braid (attention has nothing to do with maps) and found the hydration lie, re-judged with the fact that drishti's adoption is committed and lands paired.</Footnote> Attention is **level state** ("what needs you now"), written once per host at the endpoint that can know it, carried as an ordinary per-entry cell — an event member is the wrong carrier (fired while the browser is closed or the link is flapping it is simply *gone*; a cell replays current truth on every (re)connect). **kolu mints no wire member**: the cell already exists on master — `cells.urgency` (`packages/padi/src/surface.ts`; `PadiUrgencySchema = { awaitingIds: TerminalId[] }`, padi's registry fold the sole writer, read-only on the client, no recency and no count field — the count is derived as `awaitingIds.length` at every read site, as the host strip's chips already do), the deliberately tiny per-host projection W7's K1 ruling keeps hot while full wire subscriptions stay active-host-only. **drishti's `alerts` cell is W5's one new wire member** (its server holds the metrics history — threshold + hysteresis live there — and no member carries those facts today). It lands as **two PRs**: **phase 1**, W5's framework slice, is **phase 0 of the [reactive bridge](surface-reactive-bridge.html)** — `reactor.ts` exporting `derived.cell` + `scan` + `source`, riding existing seams (zero runtime surgery), drishti alerts the first consumer, paired PR — plus `updated()` change pairs and mirrors-never-fabricate, which ship regardless; **phase 2** is W5 itself on top. The full bridge phases and the campaign-1 hand-roll deletions are **separate, sequenced after** — [the bridge note](surface-reactive-bridge.html) holds them. Four framework packages gain a piece — `@kolu/surface` (the completed Dynamic + bridge phase 0), `@kolu/surface-remote` (a mirror never fabricates a value), `@kolu/surface-map` (the eager watcher), `@kolu/surface-app` (delivery) — taught end-to-end in [the attention primer](surface-attention-101.html):

```ts
// ── PHASE 1 · the surface refactor PR ───────────────────────────────────────

// @kolu/surface — the cell reader completes the Dynamic:
...
// @kolu/surface-remote — a mirror never fabricates a value: a mirrored cell
// serves NO frame until the authority's first real one, so the reader's
// "undefined until the first frame" is true end-to-end; the declared default
// belongs to the one writer, never to mirrors.

// @kolu/surface — bridge phase 0: reactor.ts exports derived.cell +
// scan + source (the engine hidden behind it, lint-banned elsewhere);
// a derived member is wire-read-only by construction (no ctx entry,
// no write verbs); rides the existing cell connect seam. Full phases
// + laws: surface-reactive-bridge.html.

// ── PHASE 2 · W5 — attention on top ─────────────────────────────────────────

// @kolu/surface-map — an eager per-member watcher on scopedByEntry's membership
// kernel (entries fold · codec identity · keyArray roots · dispose-on-exit —
// one kernel, two policies: lazy scopedByEntry for client state, eager watcher
// reading the wire). Raise detection is a pure set-diff over updated's
// {prev, next} pairs — no hand-held prev, no frame classification:
function watchByEntry<A>(client, cell, items, onRaise): {
  get(key): { kind: "live" | "stale"; value: A } | undefined;  // chips read this
};  // items(a) ⇒ stable ids — the app's one obligation;
...
kolu (phase 2 — this phase; nothing minted, the member exists on master):
...
// sequenced after W5; cell, equals, and consumers are unchanged on the wire.
...
drishti (the paired consumer — the one new wire member):
...
What stays app-side, deliberately: the domain folds and value shapes (two folds that merely rhyme are not unified), every aggregation, the pixels, the click semantics.
...
- *Field note (2026-07-08, srid on the W4 deploy):* the asymmetry is now user-visible — OS notifications escape the app (cross-host reach), but unread marks live in each host's `hostScope` owner (W7), whose only render surface is the active host's dock, so a background host's attention is invisible. W5's app-escaping rollup (badge + notification off the urgency cell the chips already read) is the missing reach. One wiring fact for the implementer: `useTerminalAlerts`'s transition watcher rides the ACTIVE host's swapped store — the urgency-projection member (this phase's wire design) is what makes background hosts observable, not the client-side watcher.
...
- **The one remaining campaign** (the plan lives in the ledger note): complete the primitives — W5 phase-1, absorbing L28 rung 2 · L1 · L20·21 under its one drishti pairing, with L8's hygiene residue as its tail commit. L24 (`padi-tui --host`) ships as its own feature PR — a feature never rides a refactor campaign.
...
Two live edges. **K1, consciously excluded — now the slotted work item after [W5](#w5):** the owner retains only *cheap, client-owned* state (focus, MRU, attention, camera, the restore latch) while the **wire subscriptions** stay active-host-only, and a 2026-07-11 root-cause showed that deferral is exactly why every host switch rebuilds the whole canvas — the dispose-on-key-change resubscribe reopens the per-host readouts from *pending*, `canvasMode` leaves `workspace`, and the `<Switch>` arm unmounts Dock, tiles, and RightPanel wholesale.<Footnote>The chain, frame-grounded against a screen recording: every per-host wire readout keys on `activeHost` via `useEntry`/`createKeyedRoot` (`packages/client/src/wire.ts`, `useTerminalMetadata.ts`), so switch-back reopens the terminal-keys stream, the metadata collection, daemon status, and session cells from pending; pending data makes `canvasModeResolver` leave `workspace`, so `App.tsx`'s `<Switch>` unmounts the entire workspace arm, which remounts from zero as records trickle back — the Dock refills incrementally, the terminal goes black while xterm content refetches, and the Code tab drops to "Loading…". Transport stays warm; the ~1s is purely logical resubscribe + refetch, and it is distinct from the #1714 reprojectClock identity flicker (#1753 does not fix it). Same class, same cure as a follow-on: `createPolledQuery` blanks and pends the Code-tab queries on host change — scope the query per host. No server or padi changes.</Footnote> The fix is W7's own cure applied to its exclusion: the per-host wire subscriptions (terminal keys, `terminals`, saved session, daemon status, activity feed) move into the retained `scopedByEntry` owner — created lazily on first activation, retained across switch-away, disposed only on membership exit — so switch-back has no resubscribe at all and no pending window *can* exist (keep-last-value or holding the mode would leave the defect expressible per readout — enumeration, the bug-shape W7 was built to kill). Deliberately still active-host-only: xterm/WebGL — a sub-second re-attach paint remains, mitigable later via a per-host screen snapshot. **Field acceptance still open:** srid confirms the live camera symptom gone on a **real deploy** (the timing a unit pin structurally can't see) before this phase closes.
