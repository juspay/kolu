...
**The 3-PR split (proposed for srid's ruling, 2026-07-04):**
- **PR 1 — the server half.** The binding pool (`padiSession` singular → pool keyed by host, on-demand, warm); per-connection binding declared in the wire scope; clock-offset measured at each bind; `KOLU_PADI_HOST` re-read as the default-host seed (decided above); per-binding connecting/degraded/skew on the shell surface. Self-sufficiency: the pool's size-1 default binding replaces today's singular path (consumed immediately by the existing flow); the per-connection scope is pinned by tests — the W2.1 precedent (framework first, frozen before the big consumer PR churns).
- **PR 2 — the client half + the picker (the phase's heart).** Wire singletons → per-binding lifecycle (tear down + rebuild the `padi` client and every subscription on switch); ChromeBar host picker (ssh-config sourced + free text); per-binding localStorage namespace; persisted last-binding per view; create refused while that binding warms; the **misroute-unrepresentable structure** (binding-scoped clients); the switch e2e (local → remote → local, stale in-flight call rejected).
- **PR 3 — full-peer completion.** File preview through `preview.read` (ranges looped for streaming; the 501 dies, e2e-pinned); the daemon inventory's remote arm (bound host's daemons beside the labeled local scan).

**Dispatch (decided, srid 2026-07-04):** after the SURF PR (the #1681 subscription-recovery + cell equals-gate fixes) merges — W3.2's client refactor builds on the fixed subscription behavior rather than churning against it. W3.4 may still be in flight (harness overlap is mild).
