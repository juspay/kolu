...
```ts
// padi — no new member, no .set: cells.urgency already exists
// (packages/padi/src/surface.ts — PadiUrgencySchema { awaitingIds: TerminalId[] },
//  padi's registry fold the sole writer; read-only on the client)
