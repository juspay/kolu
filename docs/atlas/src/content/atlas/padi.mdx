...
export const ROADMAP = [
  ...
  { d: 0, g: "▶", c: "prog", l: "W10 · hosts survive a restart — membership as a settings field in the conf store", m: "#1772 · reworked, in flight", h: "#w10" },
...
<Phase id="W10" name="hosts survive a restart — membership as a settings field in the conf store" status="next" needs={["W6"]} links={[{ label: "#1772 (being reworked)", href: "https://github.com/juspay/kolu/pull/1772" }]} />
...
1. **W10 — hosts survive a restart** is the finishing phase: W9 (<PrLink pr={1764} />) and W11 (<PrLink pr={1775} />) both shipped, so W10 is unblocked — <PrLink pr={1772} /> is being reworked to the ratified conf-store shape (below), and needs codex + code-police on the delta, full two-platform CI, and restart-persistence evidence before merge.
2. **W12 — restore survives an unclean kaval death** comes next after W10 — independent of it in code (it lives in kaval, not the host pool), but sequenced after so one phase finishes at a time.
3. **The cleanup ledger runs in parallel** — as a few substantial refactor campaigns, not one-PR-per-entry; the campaign plan lives in [the ledger note](padi-cleanup.html).
...
### W10 — hosts survive a restart *(implementation <PrLink pr={1772} /> — being reworked to this shape)*

**In simple words:** the hosts you added through the selector strip come back after a kolu restart. Today a restart forgets everything but the local default, and you re-add your fleet by hand. After W10, every remembered host reappears in the strip and reconnects through the normal connect pipeline; a host that's gone shows its honest failed chip with the cause rather than silently vanishing. The only way a host leaves the strip is your explicit remove.

**Hosts are settings** (ratified 2026-07-12). The remembered fleet is one field in `PersistedStateSchema` — a value in the conf store (`config.json`) beside every other preference — riding the same store, the same migrations ladder, and the same corruption behavior: conf **throws** on a corrupt file (`clearInvalidConfig` is false; it never resets to defaults), which is already the fail-fast doctrine, inherited rather than re-implemented. The pool's transactional persist-before-commit hook — firing only on user-intent mutations, [W11](#w11)'s split — writes through `store.set`; the unremovable local default never enters the field (it's seeded in code, and persisting it would mint a second authority for "local always exists"); an env seed (`KOLU_PADI_HOST`) stays declarative provenance, never captured into persistence; and validation is loud — a persisted value naming local or carrying duplicates is rejected, not repaired.<Footnote>The original design persisted membership as its own zod-schemed `hosts.json` with hand-rolled atomic writes (tmp + rename) and its own crash-on-corrupt rule. It was overturned by srid's 2026-07-12 ruling when its justifications failed fact-checking: conf does not reset-to-defaults on corruption (it throws — the same behavior the separate file was built to guarantee), and no concrete incident distinguished the two designs. Hosts are no different from settings, so they ride the settings store.</Footnote>

**Boot re-enters through the front door:** each persisted host goes through the ordinary `pool.add` and flows the W6 connect pipeline — warming → probing → connected, or failed with its honest cause. No lazy-dial mode: surface-remote already retries network faults forever, and a lazy/eager toggle would be a knob.

**Status:** unblocked — [W11](#w11) is in master. <PrLink pr={1772} /> is being reworked to the conf-store shape; before merge it needs codex + code-police on the delta, full two-platform CI, and restart-persistence evidence.

<a id="w12"></a>

### W12 — restore survives an unclean kaval death *(ratified 2026-07-12 — independent of W10)*
...
**The defect is the class W10 fixes for host membership:** the per-terminal agent-resume facts — whatever a graceful shutdown persists today that lets restore respawn the agent — are written at *exit* time, and a crash is precisely the case where the end never runs (SIGKILL, crash, OOM all skip it). The cure is the same: move the facts from exit-time flush to change-time journaling — the persisted record is a value replaced whole on every change (P1), owned by kaval beside its existing state — so the file is always current no matter how the process dies. The graceful-shutdown path stops being load-bearing; it can remain as a final no-op write.

**Seedling — this phase needs its own interview and grounding before build.** The first implementation step is to ground what "agent-resume facts" concretely *are* in kaval's current code — what the graceful path persists today — rather than guessing the field list here. Independent of W10: that lives in kolu-server's host pool, this lives in kaval.
