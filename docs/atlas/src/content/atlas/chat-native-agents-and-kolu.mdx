...
| Surface | Half | Reuses |
| --- | --- | --- |
| the board | out · read-only | padi's agent sensors — which agent is working / asking you / just finished (`padi-tui wait`'s buckets), rendered straight into the thread with no coordinator hop |
| attention | out · read-only | per-terminal urgency through the notify seam — the bot pings the thread instead of the desktop |
...
`pesu` is a small XS bot that binds one XS thread to the coordinator's terminal: it
renders the board and the coordinator's reports into the thread, and relays the thread's
messages — attributed — as the coordinator's input. It rides XS's own identity, presence,
and login, and needs no auth in kolu: just the connection.

**The milestone is concrete: run kolu's own development from a thread.** kolu is already
built this way — a coordinator driving a fleet of worker agents through the exact loop
the bot would mirror — so the bot is a renderer of a proven pattern, not new coordination
machinery. What pesu proves is the *chat half*: XS identity mapped to attribution, a
thread bound durably to one terminal, and the stream rendered where the team already is.
