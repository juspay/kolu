...
<Roadmap>
  <Milestone status="next" label="R1 · the serve/run split">`odu serve` owns `.ci/odu.sock`; `run` becomes a surface procedure; the CLI keeps its exact UX by ensuring a server before invoking it. The MCP face's `run` tool becomes a thin projection like its siblings. **Exit: `odu attach` connects with nothing running** and shows a (possibly empty) runs list.</Milestone>
  <Milestone status="next" label="R2 · run identity">The `runs` cell; `(repo, sha, seq)` minted by the server and serialized into the per-SHA trail; `nodes` / `nodeLog` / `rerun` gain optional run scope with latest-run defaults so every shipped face is unmodified-correct. **Exit: a run started by the CLI and one by MCP land in one list with stable ids** — the ids odu-web Phase 1 puts in ledger rows and `target_url`s.</Milestone>
  <Milestone status="next" label="R3 · lifecycle, crash semantics, pricing">The three lifecycle modes made real: foreground default, the home-manager unit (graduation roadmap item), idle-timeout auto-serve as the no-daemon fallback. Server death marks in-flight runs errored in the trail and restarts clean. The pricing note written against the warm pu-box pool: what stays warm, what spawns, and why the coordinator is never the expensive half. **Exit: kill the server mid-run — the trail shows errored, restart serves history, nothing wedges.**</Milestone>
</Roadmap>
