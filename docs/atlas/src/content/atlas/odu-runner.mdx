...
*[odu-web](./odu-web.html) opens with "Phase 0 · prerequisites in the runner" and points back at [odu](./odu.html)'s backlog. This note is that phase given its own work order: what the runner must become before a service can sit on top of it — and why each piece is the runner's job rather than the service's.* <Pill variant="warn">proposed</Pill>

<p>Status: <Pill variant="warn">proposed</Pill> · maturity <Pill variant="todo">seedling</Pill> · consumer: [odu-web](./odu-web.html) Phase 1 (the ledger needs run identity) and Phase 2 (the live observer needs idle attach) · the items themselves have sat on [odu](./odu.html)'s Phase 2 backlog since the MCP face shipped</p>
...
<Roadmap>
  <Milestone status="next" label="R2 · run identity — no kaval dependency, can land first">The `runs` cell; `(repo, sha, seq)` minted by the server and serialized into the per-SHA trail; `nodes` / `nodeLog` / `rerun` gain optional run scope with latest-run defaults so every shipped face is unmodified-correct. Independent of the daemon spine — the identity work applies to today's run-scoped coordinator unchanged. **Exit: a run started by the CLI and one by MCP land in one list with stable ids** — the ids odu-web Phase 1 puts in ledger rows and `target_url`s.</Milestone>
