...
description: A standalone Rust tool — one versioned snapshot of processes and sockets, scoped to the ask and honest about what it couldn't read. The binary, its V2 facets, the socket-holders verb, kolu's whole adoption, drishti's migration, and the tool's graduation to its own repo have all shipped.
parents: [feature]
status: accepted
maturity: budding
updated: 2026-08-01
...
OSF5 its own [repo](https://github.com/juspay/osfacts) — not a kolu module. One static Rust binary that answers *what does
...
half), described under [what shipped last](#what-shipped-last); the shipped
work is compacted to its durable lessons.<Footnote>The candidate measurements come from
...
README](https://github.com/juspay/osfacts/blob/main/README.md)'s
opening:
...
- **Facets** so a consumer pays only for what it reads: `--procs`, `--ports`,
  ...
  set; the machine, not a process) — and each owns its own `E` facet
  vocabulary, so a consumer cannot match one verb's blindness against
...
- **One home for the facet vocabulary.** A facet name is a `Facet` enum variant
  ...
  change (2 → 18 across V2), so adding one must be a single edit that fails
  loudly if only half of it lands, not a typo that reaches a consumer as a
...
what keeps the contract's meaning identical on both.<Footnote>The pid set a
...
consumer could act on.</Footnote>
...
sweep confirmed the list is exhaustive.<Footnote>The sweep covered
...
| `socketHolder` (in `surface-daemon-supervisor`) | which pids hold a unix socket **path**   | **shipped** — the `socket-holders` verb; the `/proc/net/unix` parse, the fd readlinks, and the darwin `lsof` shell-out all deleted (<PrLink pr={2071} />) |
...
**Kolu's own adoption closed in three PRs.** <PrLink pr={2058} /> took daemon
...
<PrLink pr={2071} /> took the last hand-rolled reader — the daemon
...
consumer bakes the binary into. <PrLink pr={2067} /> took memory: one osfacts
...
| **OSF4 — socket-holders**        | the second verb, its three-way answer, and the deletion of `socketHolder`'s `/proc/net/unix` parse, fd readlinks, and darwin `lsof` shell-out — **shipped in <PrLink pr={2071} />** | OSF1              |
...
| **OSF5 — drishti & extraction**  | drishti's migration relands whole on a fail-fast baked path — **shipped** ([#122](https://github.com/srid/drishti/pull/122), [#123](https://github.com/srid/drishti/issues/123), [#127](https://github.com/srid/drishti/pull/127)). The extraction is **shipped** too ([detail](#what-shipped-last))<Footnote>The directory moved wholesale: `osfacts/` already carried its own README, `default.nix`, a zero-input subflake, and `client-ts/` (`osfacts-client` — no `@kolu` scope, Node builtins only) precisely so extraction would be a move, not a redesign. The subflake became *the* flake and the contract did not change.</Footnote> | OSF3 · OSF6 · OSF7 |

Every phase has shipped, OSF5's extraction half last, now that both external
adopters are on the contract and the verb set is complete. Each phase is
...
[juspay/osfacts](https://github.com/juspay/osfacts)
([#1](https://github.com/juspay/osfacts/pull/1)). It was built for this from
...
**What the move did not take.** `osfacts/client-ts` stayed in kolu. A pnpm
workspace member must be a path inside the repo, and `packages/padi`,
`packages/kaval`, and the surface examples import `osfacts-client` by name — so
the client cannot come from a pin until it is consumable another way (published
package, or a pnpm git-subdirectory dependency). `facets.json` stayed with it,
because the client's tests pin the TS facet unions to that file; a nix check
(`osfactsFacetsInSync`) fails the build if that copy drifts from the pinned
repo's, so there are two files but still one source of truth. Both of kolu's
osfacts CI leaves left with the tool — the live-oracle lane because its script
did, and the hermetic one because it had nothing left to assert that `ci::nix`
does not already build — so all four `ci::osfacts…@…` contexts must be dropped
