...
repo's, so there are two files but still one source of truth. Kolu's live-oracle
CI leaf left with the tool, which is why the `ci::osfacts-live@…` contexts must
be dropped from branch protection.
