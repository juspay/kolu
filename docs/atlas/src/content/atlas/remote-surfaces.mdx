...
7. **The one-socket ops gate (debate-demanded, blocking)**: before `?host=` sockets are deleted, the PR measures and documents N-host behavior on the shared socket — reconnect re-subscribe cost on socket drop (today `STREAM_RETRY` is infinite; N hosts × M subs must not storm), and whether one stalled host's frames delay another's (head-of-line). Budget in the PR description; if the measurements refuse the design, per-host sockets return as an implementation detail UNDER the same map API (the notion is transport-agnostic — that is the point of it).

**The odu PR — the third consumer, mechanical by design.** Analyzed at odu master (`b0386d3`): odu imports exactly four symbols from the renamed package — `makeSession`/`sshConnector` (`coordinator/lane.ts`, its per-host CI lane sessions) and `isLocalHost`/`resolveSystem` (`coordinator/run.ts`) — and none of the registry/re-serve machinery, so it is NOT a map consumer (its lanes are ephemeral per-run sessions, not a served fleet). Its paired PR is therefore import-path updates + the pin bump only; the naming sweep deliberately leaves odu's four symbols untouched (they were never legacy vocabulary). Three consumers, three paired PRs: kolu (the work), drishti (substantive adoption), odu (mechanical rename).
