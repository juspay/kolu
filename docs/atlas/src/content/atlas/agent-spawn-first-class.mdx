...
**PR 2 — stop the detection loss** — *not yet dispatched* (small; `kaval`).
The remember-the-argv bookkeeping: seed `lastCommand` (and the initial title)
...
a command-rooted terminal's `lastCommand` equals its argv at birth. This PR
