...
Three PRs, each independently shippable, ordered by harm:

**PR 1 — stop the data loss** (`kaval-tui` + `kolu-pty` + `padi` +
...
**PR 1.5 — close the last spawn seam** (`@kolu/surface-remote`, immediate
...
**PR 2 — stop the detection loss** (small; `kaval`).
...
**PR 3 — the face verb** (rides the kolu-tui build-out, not before).
