...
| --- | --- | --- | --- |
| `@kolu/solid-markdown` <Pill variant="todo">new</Pill> | marked + GFM + `safeHref` + md→Solid styling. Volatile: the spec, the sanitizer. | `<Markdown markdown variant="inline"\|"document" />` | The ~250 LOC token-walk core inside `notes/NotesMarkdown.tsx`. |
...
| --- | --- | --- | --- |
| **1 · Extract `@kolu/solid-markdown`** <Pill variant="ok">shipped</Pill> <PrLink pr={1079} /> | Lift the token-walk core out of `NotesMarkdown.tsx` (257→30 LOC) into a package with `inline`/`compact`/`document` variants; migrate the notes surface. Behavior-preserving. | ↓ | Low — mechanical, one call-site. |
...
| File | Change |
| --- | --- |
| `notes/NotesMarkdown.tsx` | <Pill variant="ok">−~250 LOC</Pill> A thin consumer of `@kolu/solid-markdown`. |
