...
- **WebGL context-loss recovery and deterministic release.** The addon is
  ...
  `clearTextureAtlas` on the per-tile show, sequenced post-layout — the per-tile twin of
  `refitOnTabVisible`'s whole-tab clear (which never covered the `display:none`
  toggle). `clearTextureAtlas` re-warms on the live context without routing through
  `_refreshCharAtlas`, so it can't hit xterm's zero-dims atlas-skip.</Footnote>
