...
- **WebGL context-loss recovery and deterministic release.** The addon is
  ...
  A mid-2026 corrupted-glyph investigation confirmed this boundary is sound and
  correctly library-owned.<Footnote>A corrupted-glyph symptom — box-drawing worst,
  ...
  loss+restore even though the addon's handler runs). The real mechanism behind the
  reported symptom remains under investigation, on a real-GPU venue.</Footnote>
