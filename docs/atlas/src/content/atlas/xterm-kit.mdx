...
proof. WebGL context-loss recovery and owner-correct async dispose keep their
existing unit coverage (`webgl.ts`'s self-heal, the lifecycle's owner-capture);
they are not re-proven by a new e2e scenario here.
