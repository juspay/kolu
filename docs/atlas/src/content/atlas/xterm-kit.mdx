...
- **The touch divisor was a hand-rolled parallel of a fact xterm already owns —
  ...
  the user visually tapped, at normal scale AND under zoom, proven by a
  standard-suite browser e2e (the `@touch-desktop` tap-under-zoom scenario). No
  user-observed regression surfaced, so it landed as the
