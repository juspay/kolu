...
1), the `<Xterm>` wrapper + touch-divisor fix in the e2e-gated PR 2 fast-follow.
...
A SolidJS app renders a live terminal in one component (the `<Xterm>` wrapper is
**PR 2 target API** — it does not ship in PR 1, which graduates the primitives it
will compose):
...
The first three below are **PR 2 target API** (the `<Xterm>` component and its
lifecycle/WebGL primitives); the rest ship in PR 1:

```tsx
import {
  Xterm,                // PR 2 — the component: lifecycle + addons + WebGL + fit, in JSX form
  createXtermLifecycle, // PR 2 — the primitive under it, for a non-JSX composition
  attachWebGL,          // PR 2 — single-owner WebglAddon lifetime + context-loss recovery
...
- **Touch intent and the soft-keyboard focus rules join as owned mechanism —
  ...
  the contenteditable input via `softKeyboardInput.ts`, the
  ...
  prop.
- **The touch divisor is a hand-rolled parallel of a fact xterm already owns —
  the follow-up deletes it; this PR keeps its fence.** One invariant —
  pointer→cell under a CSS transform — is computed today by **two separate
  divisors**: xterm's hit-test path (its font-metric cell mapping, the authority
  that actually places the glyphs, corrected on the *input* by
  `unscaleEventPoint`), and the tap path's `fileRefAtPoint`, which derives its
  OWN cell size from the post-transform rect (`rect.width / cols`). These agree
  only when xterm's internal cell equals `rect.width / cols` — which nothing
  guarantees: font-metric rounding, sub-pixel, or screen padding can make a tap
  near a cell boundary resolve to a *different* cell under zoom. So the
  rect-derived divisor is a hand-rolled parallel of the existing source of
  truth, and collapsing the two is a **behavior change, not a move**. The
  `/solid`-component follow-up (PR 2 below) deletes the rect divisor and routes
  the tap through xterm's mapping; its done-criterion is *correctness*, not
  equivalence to today — a tap resolves to the cell the user visually tapped, at
  normal scale AND under zoom, proven by standard-suite browser e2e (it may well
  be a bug fix: today's taps can already mis-target under zoom where the cells
  diverge). Until that PR both divisors stay, and the mirrored *"keep both in
  step"* warnings at both sites<Footnote><Cite file="packages/client/src/terminal/Terminal.tsx" />
  and <Cite file="packages/xterm-kit/src/internals.ts" /> carry the paired
  warning — the load-bearing fence while two divisors coexist.</Footnote> are
  the fence that keeps them from drifting.
...
Its gate is that standard-suite e2e proving three things on both platforms:
tap-under-zoom correctness, WebGL context-loss recovery, and owner-correct async
dispose — the properties a typecheck cannot vouch for.
...
`<Xterm>`, the single-authority divisor is proven correct under zoom, and the
webgl/dispose e2e is green on both platforms.
