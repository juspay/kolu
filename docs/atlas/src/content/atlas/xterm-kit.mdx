...
- **WebGL context-loss *and restore* recovery, plus deterministic release.** The
  addon is single-owner, self-heals on `webglcontextlost`, explicitly fires
  `WEBGL_lose_context.loseContext()` on unload — without that, detached canvases
  hold GPU contexts until GC and Chrome's per-tab context budget evicts *live*
  ones — and recovers from an eviction+restore by rebuilding the renderer so glyphs
  re-rasterize into a fresh atlas.<Footnote>Chrome allows ~16 WebGL contexts per
  tab; rapid focus changes created contexts faster than GC released them,
  flickering every tile (#575, #591, the #1399 budget). A *separate*, longstanding
  gap surfaced later: when Chrome evicts a live context and later restores it, the
  glyph-atlas GPU texture dies with the context while xterm keeps drawing to its
  now-stale coordinates — **botched glyphs** that clear only on re-rasterization
  (selecting text, or a reload). The kit now opts the canvas into restore
  (`preventDefault` on loss, as the WebGL spec requires) and does a FULL renderer
  re-init on `webglcontextrestored`; `clearTextureAtlas()` alone can't fix it — the
  atlas texture *object* itself died with the context. This gap was **not**
  introduced by the graduation: it was reproduced deterministically, traced to the
  xterm/addon layer (commit-independent), and confirmed by field history — Linux
  always showed it sporadically, macOS clustered under load, with no Chrome change —
  so the behavior-neutral claims for <PrLink pr={1795} /> / <PrLink pr={1808} />
  hold. The restore recovery + its regression pin land in <PrLink pr={1824} />.
  Mechanism in the package
  (<Cite file="packages/xterm-kit/src/solid/webgl.ts" />); the *policy* of which
  panes deserve a context stays the consumer's.</Footnote>
...
import {
  ...
  attachWebGL,          // single-owner WebglAddon lifetime + context-loss/restore recovery (PR 2)
...
- **`attachWebGL(terminal, should: Accessor<boolean>)`** exists because the
  ...
  context loss, full renderer re-init on context *restore* (rebuilding the atlas
  the eviction destroyed), explicit `loseContext()` on unload. The gate is an
  accessor by type — budget *policy* (kolu's recency budget, a preference toggle,
  always-on) stays whatever the consumer computes.
