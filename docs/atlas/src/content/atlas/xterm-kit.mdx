...
- **WebGL context-loss recovery and deterministic release.** The addon is
  ...
  A mid-2026 corrupted-glyph investigation exonerated this boundary and traced the
  real cause to a per-tile visible-transition gap, now fixed.<Footnote>A corrupted-glyph symptom — box-drawing worst,
  clearing on selection or reload — reported under a busy split + maximized
  workspace was investigated here. **Verdict: not a #1795/#1808 regression** —
  diff-falsified (no dropped atlas-clear, logic-identical context-loss path, the
  `@xterm/*` versions `pnpm.overrides`-pinned repo-wide so no skew) and confirmed
  by field history (Linux always sporadic, macOS clustered under load, no Chrome
  change); those PRs' behavior-neutral claims hold. **And the pinned
  `@xterm/addon-webgl@0.20.0-beta.224` already owns context loss AND restore
  in-renderer** — byte-verified against the installed lib: `webglcontextlost` →
  `preventDefault()` + a 3s timeout that fires `onContextLoss`; `webglcontextrestored`
  → `removeTerminalFromCache()` + `_initializeWebGLState()` + redraw (a full renderer
  rebuild against the live context, the atlas included). So a *kit-level* restore
  re-init would DUPLICATE that library boundary, not fill a gap — a first attempt
  that added exactly that was reverted after the byte-check (reproduce-on-software-GL
  had produced a false positive: SwiftShader doesn't visually recover from a synthetic
  loss+restore even though the addon's handler runs). The real mechanism, then found
  on a real-GPU venue: a tile returning from `display:none` (a switch-to-tile in
  split/maximized) kept a STALE glyph atlas — while hidden it missed the resize/DPR
  events that rebuild it, and on show a same-geometry `fit()` is a cols/rows no-op that
  fires no `handleResize`, so every glyph drew from a wrong slot (a window-resize
  cleared it, an un-hide did not). Fixed in `<Xterm>`'s visible effect: force
  `clearTextureAtlas` on the per-tile show — the per-tile twin of `refitOnTabVisible`'s
  whole-tab clear (which never covered the `display:none` toggle). `clearTextureAtlas`
  re-warms on the live context without routing through `_refreshCharAtlas`, so it can't
  hit xterm's zero-dims atlas-skip. The rebuild MUST run at the tile's real size, though:
  a single post-layout rAF can itself fire before the `display:none → visible` reflow has
  flushed, where `clearTextureAtlas` at 0×0 is a no-op and the whole tile stays stale
  until streaming heals it top-down (<PrLink pr={1824} /> shipped the single-rAF form and
  fixed the common case; the intermittent 0×0 tail needed a follow-up). So the effect now
  RETRIES per frame until the tile measures `offsetWidth/Height > 0`, then rebuilds —
  bounded (~30 frames) so a genuinely 0-sized/off-screen tile can't spin.</Footnote>
...
```ts
import {
  // mirror anchoring (the kaval half) + reattach discrimination — runtime-neutral
