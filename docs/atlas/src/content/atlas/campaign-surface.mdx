...
| **the attention flow — a blocking ask can't exist without a listener** | padi delivers `[kolu] Worker terminal <id> (<intent>) …` into the SUPERVISOR terminal's mailbox (the `parentId` edge names it; kaval's serialized input is the transport), on the attention EDGE. Only agent-terminal supervisors are notified, never a human shell. Plus standing named subscriptions (`watch_*`) for a supervisor with no terminal. | **shipped — [#2135](https://github.com/juspay/kolu/pull/2135)** |
...
**Shipped in [#2135](https://github.com/juspay/kolu/pull/2135), and it needed
less than this note assumed.** The conjunction was already computed — padi's
[effective finish (EF2)](effective-finish.html) is exactly *agent-state
`waiting` ∧ PTY output quiet*, and it is what the `urgency` cell's
`finishedIds` has always meant. So the delivery reads that shipped fact rather
than re-conjoining two wait tools. One correction to the design above: `asking`
(`awaiting_user`) is delivered **ungated**, because an agent that declares it is
blocked is definitionally not mid-output; the quiet window only ever guarded
turn-END, which is where the subagent flap actually lives. Kolu's shipped
`attentionClass` partition already drew that line, and this rides it.
...
browser's own alert engine, so a human at the canvas and a supervising agent in
a PTY fire on ONE definition rather than two that agree by luck.

A departure (`kind: "gone"`) rides the same channel: a supervisor blocked on a
worker that no longer exists is the same failure as one blocked on a worker
nobody reported.
