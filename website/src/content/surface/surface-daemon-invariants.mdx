...
| Teardown ordering | Shutdown is always **stop accepting + disconnect established peers (the listener `close()`) → release gate → return `DaemonExit` → release the wrapper's resources → exit** — `daemonProcessMain` runs the exit strictly after everything inside its `run()` promise, so the exit cannot preempt a release stage, and a shutting-down daemon no longer leaves still-connected clients being served by a "closed" socket. |
