...
| Honest absence | `readBakedIdentity` returns empty strings off-nix rather than inventing an identity. The build id and commit are one fact: both must be present or both absent; a half-pair crashes at daemon boot. |
