...
| The disposition is the contract-skew policy | An undecodable wire is the limit case of a skew — the one whose version cannot be read — so each daemon's declared skew policy decides. `recycle` (kaval): SIGTERM the verified gate holder and spawn fresh. `refuse` / `drain-newer-else-refuse` (padi): **refuse** — a drain verb on an unspeakable wire is unreachable, so the ordered policy degenerates to its other arm; the survivor is left standing and degraded, never killed, and the `unconverged` anomaly carries the `unspeakable-protocol` cause with the `socketPath`, `gatePath` and verified `pid` as data. |
