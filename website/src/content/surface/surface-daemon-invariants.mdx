...
| Frozen within an epoch | The control-core fragment never versions *inside* a protocol epoch. The Effect-4 framing change (oRPC base64+newline → Effect RPC ndjson) was a declared flag day: a previous-epoch peer is undecodable at its first frame and is observed as `unspeakable-protocol`, never as an incompatible version. `CONTROL_CORE_VERSION` versions the payload, which did not move. |
