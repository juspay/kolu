...
an app can fold into the member's `liveWhen` so a frozen derivation reads
unhealthy rather than green over stale data. It never heals; recovery is a
restart. Streams and events deliberately do **not** ride the graph — a signal
conflates same-batch frames, and a stream must see every one.
...
The accessor these hooks return is a `Subscription<T>` (`@kolu/surface/solid`) —
a SolidJS `Accessor<T | undefined>` with `.pending()`, `.error()`, `.complete()`
(latches once the stream ends normally; the value is then frozen), and
`.updated(handler)`. `.updated()` is the change half of an FRP `Dynamic`:
