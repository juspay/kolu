...
hosts once at admit; `probeSurfaceClockNow(client)` performs the raw round-trip and
`measureSurfaceClockOffset(client)` returns the RTT-compensated offset
(`round(remoteEpochMs − (sentMs + rtt/2))`, sampling local time at the round-trip
midpoint so the one-way latency does not bias the result).
