...
suppressed write never pokes. **Declare an upstream before a downstream:** a
`derived.cell` that reads a sibling `derived.cell` via `$` must be declared after
it (the walk builds nodes in declaration order, exactly as an app orders its own
`computed`s); reading a not-yet-declared derived sibling crashes the boot walk.
