...
export const surface = defineSurface({
  ...
  procedures: { proc: { kill: { input: Pid, output: Killed } } },
});
```
...
Every surface also carries one framework-reserved procedure, `surface.system.live`,
injected by `defineSurface` and auto-answered by the server — no app declares or
implements it (see [Liveness](#liveness-and-health)).
...
```ts
import { implementSurface, inMemoryStore, inMemoryChannelByName } from "@kolu/surface/server";
