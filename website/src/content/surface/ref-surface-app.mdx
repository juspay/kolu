...
| `onClick(handler)` | subscribe to clicks; the handler receives the clicked notification's **validated** `data`. Covers the live path (the worker retries `postMessage` under an ACK handshake so a still-LOADING window that has not installed its listener yet does not drop the click) and the cold-start URL handoff (no window open → opens one carrying the payload in `NOTIFICATION_DATA_PARAM`, read once at startup; a loading window that never acks is navigated to the same URL as a durable fallback). Returns an unsubscribe fn |
