...
```ts
let conn = await connectSurfaces({ surfaces: {}, core: { surface: core, name: "olai" } });
...
the replacement is dialled *first* and this connection released only once it is
up, so a transient dial failure leaves the working wire alone and rejects —
...
that must survive a roster change, and a serve site reads the group and handlers
per accepted connection anyway. The asymmetry is the honest one, not an
oversight.
