...
| `connectionEpoch` | *(multi only)* how many usable **connections** this wire has established — 0 before the first, monotonic for the connection's life — as a Solid accessor, so a `createResource` can key on it. See [below](#what-a-roster-move-does-not-hide) |
