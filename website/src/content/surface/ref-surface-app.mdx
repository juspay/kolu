...
that must survive a roster change, and `serveSurfaceSocket` reads the group and
handlers per accepted connection. (The two *listener* doors snapshot at bind; see
the caution on that page.) The asymmetry is the honest one, not an oversight.
