...
`clients`, `core`, `transport`, `readout`, `health`. Read them off the returned
connection; a `connectSurfaceMap(map, conn.transport)` built over the old handle
fails loudly on its next call, never silently. A **second** `redial` (or one
after `dispose`) is refused: it would dial a third wire while the caller still
