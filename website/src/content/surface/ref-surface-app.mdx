...
Closing the scope **drains before it drops**: every connection's serving stack is
closed and awaited first — that is what releases its RPC fibers and every
in-flight subscription it opened — and only then are the remaining raw sockets
terminated and the server closed. So when the finalizer resolves, nothing this
listener started is still running.
...
| `SurfaceAppConnection` | `/serve` | one accepted connection's `{ request, url }` — what a per-connection `services` layer is built from |
...
| `hostAuthority(host, port)` | `.` | `host:port` a URL parser accepts — the ONE place an IPv6 literal is re-bracketed |
...
| `Disconnected` | `connection` | silent — the same connection hung up |
...
| `ServingFailed` | `cause`, `url` | loud `console.error` — `done` MUST be observed |

`Connected`/`Disconnected` are the pair a live-connection count is built from
(the package's own example counts its `serverStats.connections` on them), and
they carry the same `SurfaceAppConnection` object both times, so a consumer can
pair them by identity. `services` stays a separate option: it returns a `Layer`,
which is a different kind of thing from narration.
...
today. kolu's does: an `https.Server` when TLS is configured, a `middleware`
passthrough to `NodeHttpServer.makeHandler`, and a shell layer mounted only when
a built dist exists. drishti's per-host `?host=` routing picks WHICH runtime
serves a socket, which the accept seam deliberately leaves at the call site.
...
| `SurfaceAppLayerOptions` | `/server` | that layer's options (`clientDist`, `manifest`, `serviceWorker`, the freshness paths) — `ServeSurfaceAppOptions` **extends** it, so the shell half is named once |
