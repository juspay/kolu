...
| `onClick(handler)` | subscribe to clicks; the handler receives the clicked notification's **validated** `data`. Covers both the live postMessage path and the cold-start URL handoff (a click with no window open opens one carrying the payload in `NOTIFICATION_DATA_PARAM`, read once at startup). Returns an unsubscribe fn |
