...
whole teardown on the enclosing `Scope`.
...
<Aside type="caution" title="The inbound frame cap is not a knob">
  ...
  inbound leg, so if they disagree the tighter one silently governs.
  `serveSurfaceApp` reads the cap from `RPC_MAX_FRAME_BYTES`
  (`@kolu/surface/frame-limit`) and offers no option to lower it. An app that
  hand-rolled `maxPayload: 8 * 1024 * 1024` got a 10 MiB frame — one the framework
  promises to carry, and that `exceedsFrameLimit` and every margin derived from it
  size against — killed at the raw `ws` layer instead of on the handled path.
</Aside>
...
| `SURFACE_WS_PATH` | `.` | `/rpc/ws`, the one path a surface speaks on |

Optional hooks are observational and default **loud**: `onSocketError` (a
transport error on an accepted socket), `onDisallowedOrigin` (a refused
cross-site upgrade), `onServingFailed` (this connection's serving stack faulted).
`onStaleTab` is silent by default — a server that restarted while a tab was open
is ordinary, not a fault — but every real consumer wires it to a log line.

The sections below are the pieces `serveSurfaceApp` composes; reach for them
directly when an app owns its own listener (kolu's multi-host dispatch, drishti's
per-host routing).
