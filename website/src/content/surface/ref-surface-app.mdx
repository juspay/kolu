...
| Fact | Shape | Notes |
| --- | --- | --- |
| `upgradeHeaders` | `ReadonlyArray<string>` | the allowlist. **Empty by default** — a header nobody named is not available at all |
| `connection.headers` | `Readonly<Record<string, string>>` | the named headers this upgrade carried. Matched case-insensitively; keyed by the spelling in `upgradeHeaders` |
...
<Aside type="caution" title="An allowlist, because the alternative leaks">
  ...
  make. A name outside HTTP's field-name grammar takes the **bind** down (it
  throws) rather than reading, forever, as a header the proxy never sends.
</Aside>
