...
- **`clients` is the same map**, mutated: an arriving sibling appears on it, a
  ...
  members the new one may not have.
...
| `redial` | *(multi only)* async: take a NEW sibling roster **in place** — a new wire is dialled over this connection's own options and adopted, the root does not move, and every handle above keeps its identity. See [above](#a-roster-change-follows-in-place--connredialsurfaces). Returns this same connection, retyped |
