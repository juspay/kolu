...
| `escapeRe(value)` | `.` | everything a RegExp treats as syntax, escaped — what `chunkPattern` builds its own patterns with, exported so a caller assembling a matcher beside them does not grow a second character class |
