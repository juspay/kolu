...
generation it was built over until the client [redials](#a-roster-change-is-a-redial--connredialsurfaces).
...
### A roster change is a redial — `conn.redial(surfaces)`

A wire whose **sibling set** changes cannot be updated in place, and the reason
is the transport rather than this seam. Effect RPC resolves a call's
payload/success/error **schemas** by looking its tag up in the `RpcGroup` its
client was built over, and that group is fixed at the dial (`openWireLink` does
`RpcClient.make(group, …)` once, over a protocol whose fibers live in the link's
own scope). A sibling that joins the roster brings tags that group never minted,
so no client built before it can dispatch them — and the far end has the same
constraint, because each accepted socket builds its own `RpcServer` over the
group it was handed at accept. **A roster change is a new wire at both ends.**
So the door says so, instead of offering an `update` that would quietly rebuild
everything under a name that promises it did not.

```ts
let conn = await connectSurfaces({
  surfaces: {},
  core: { surface: core, name: "olai" },
  retired: reloadForUpdate,          // …and the rest, as above
});
// …the served roster moved:
conn = await conn.redial({ kolu: koluSurface });
```
...
the replacement is dialled *first* and this connection released only once that
dial has resolved, so a failing dial leaves the working wire alone and rejects —
rather than the obvious dispose-then-dial, which leaves the caller with nothing.
("Resolved" is the seam's own await, not an open socket: `connectSurfaces` hands
back a connection whose wire may still be connecting, exactly as a first dial
does. What is guaranteed is that a dial which *throws* costs you nothing.)

A `dispose()` that lands while a `redial` is in flight is terminal and wins: the
replacement is released and the redial rejects, rather than handing back a wire
...
`connectSurfaces` again.

Everything the superseded connection handed out is dead once `redial` resolves —
`clients`, `core`, `transport`, `readout`, `health` — and it **says so**. A
superseded (or disposed) connection's `readout` reads `retired` and its `health`
reads not-live, rather than freezing on whatever the disposed memo last computed:
on the common path — the roster moved, the wire was fine — that frozen value is
`live`, so an indicator still bound to the old accessor would paint green over a
closed wire. A `connectSurfaceMap(map, conn.transport)` built over the old handle
fails loudly on its next call, never silently. Reading everything off the
returned connection is still the right habit; it is no longer what stands between
the page and a lie. A **second** `redial` (or one after `dispose`) is refused: it
would dial a third wire while the caller still believes it holds one.
...
`{ link, clients, core, transport, readout, health, redial, dispose }`.
...
| `health` | *(multi only)* `surfaceClientsHealth` over every sibling — and, when a `core` was passed, the root under its `name` — their subs AND-reduced with the shared transport leg |
| `redial` | *(multi only)* async: take a NEW sibling roster by dialling a new wire over this connection's own options, keeping the root — see [above](#a-roster-change-is-a-redial--connredialsurfaces). Returns the replacement; this connection is released once it is up |
