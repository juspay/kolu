...
| `chunkPattern(module)` | `.` | that template read backwards — what the split chunk for `module` is CALLED, anchored to the whole filename, for a caller that must name the file BEFORE the build has run. A RegExp, because callers hand it to a route matcher and print it in a diagnosis |
| `chunkUrlPattern(module, assetPrefix?)` | `.` | the same rule as a REQUEST under the hashed prefix, taken through `assertAssetPrefix` so a prefix the build refuses is not one a matcher accepts |
