...
| `SurfaceClientBuildResult` | `/bun` | `{ jsHref, assetHrefs, assets }` — the hashed URLs your **placeholders** were rewritten to (the JS entry, and one per extra asset), plus an `AssetReport` per **compressible** asset in the hashed dir (sourcemaps and already-compressed media are skipped, and absent from the report). The shell also carries a `modulepreload` link per static chunk; those come from the build graph, not from a placeholder, and are not reported |
...
<Aside type="note" title="The shell preloads the entry's STATIC chunks, and only those">
  ...
  and an entry that split into nothing gets no tags at all.
</Aside>
