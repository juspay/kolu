...
| `UpgradeHeadersRefused` | `error`, `url` | loud `console.error` — a live `upgradeHeaders` named something unreadable. The socket is **served anyway**, with no named headers, and the fault is reported at **each** such accept ([why](#a-live-allowlist-read-at-each-accept)) |
