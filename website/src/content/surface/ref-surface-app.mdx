...
const url = yield* serveSurfaceApp({
  group: () => runtime.group,       // or a value — a value is the generation at the call
  handlers: () => runtime.handlers, // read as a pair, at each accept
...
`group`, `handlers` and `expose` are `LiveServed<T>` — a value **or** a thunk
(a getter on the options object is the same live read). A value is the
generation that was current when the option was written; a thunk is re-read at
each accept, always as a pair, and `restrictHandlers` is memoized by generation
identity so an unchanged roster costs nothing. A socket accepted after a
...
Existing callers that pass values do not change.
...
| Export | Subpath | Role |
| --- | --- | --- |
| `serveSurfaceApp(opts)` | `/serve` | the whole listener — shell, socket, gates, heartbeat, bind, teardown. `group` / `handlers` / `expose` are `LiveServed` |
| `LiveServed<T>` | `/serve` | `T \| (() => T)` — a value is the generation at the call; a thunk is re-read at each accept |
...
| `SocketError` | `error`, `url` | loud `console.error` — a transport error, or a live generation `restrictHandlers` refused |
...
that must survive a roster change. `serveSurfaceApp` and `serveSurfaceSocket`
both read the group and handlers per accepted connection; `serveOverUnixSocket`
still snapshots at bind (see the caution on that page). The remaining
asymmetry is the honest one, not an oversight.
