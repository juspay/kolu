...
`cause` (`"network"` unreachable / `"remote"` refused); `failed` is terminal with `cause`
pinned to the `"remote"` literal (a network fault never gives up, so `failed` + `network` is
unrepresentable). There is no separate `progressLines` / `remoteProgressLines` — provenance
is the `source` field.
