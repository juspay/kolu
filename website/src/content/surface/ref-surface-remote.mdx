...
returns the host's `{ session, handler }`. `remove(host)` and `retire(host)` run the
**same** teardown (evict sockets, drop membership, destroy the session), differing
only in persistence: `remove` is the **user's** departure — it calls the `persist`
hook first — while `retire` is an **internal** shed (a dead session the pool sheds on
its own) that does **not** persist, so a membership store re-seeds the host next boot.
A pool with no `persist` hook makes the two identical. `serveHostMap` takes `{ linkFor, causeFor? }`;
