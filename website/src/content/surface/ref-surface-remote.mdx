...
`serveHostMap` takes `{ linkFor, causeFor? }`; `linkFor(host, session)` is built once
per host and evicted on **either** removal or retirement (both drop live membership).
Together they are the server half of a host map — see
