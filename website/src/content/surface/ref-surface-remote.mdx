...
- **The pre-connected liveness backstop (#1908 R8b).** While a campaign is coming up, if NO progress line arrives and NO phase advances for `MakeSessionOptions.preConnectedLivenessMs` (default 20min), the session cycles the attempt (abort + redial), narrated. It is liveness (a chatty build resets it every line, never capped) and guards the seam the per-child policies can't — a wedge in a non-helper await like `resolveDrvPath`.
