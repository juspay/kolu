...
| `resolveAgentDrv(host, flakeRef, packageName, { signal, onProgress })` | `.` | probe the host system and lazily evaluate that one package's `.drvPath` inside the owning dial's cancellation/progress path; deterministic Nix errors are terminal, while transport and liveness failures remain retryable |
...
closure, `resolveDrvPath: ({ signal, localProgress }) => Promise<string>` resolves
the agent `.drv` for the *target* arch inside the owning dial's cancellation and
progress lifetime, and **`localEnv: Record<string, string>` is the REQUIRED env for a
