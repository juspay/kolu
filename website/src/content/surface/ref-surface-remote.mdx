...
destroy rather than repair** — a CI lane that has been compiling for twenty
minutes should ride out a multi-minute network blip, not be killed and restarted
(`{ intervalS: 30, countMax: 10 }` buys five minutes). The cost is symmetric: a
genuinely dead host keeps that dial parked for the same window before the loop
retries. Out-of-range values throw at `sshConnector(...)` construction rather
than being clamped (`assertSshKeepalive`, ceiling `MAX_SSH_KEEPALIVE_TOLERANCE_S`
= one hour) — the same fail-fast rule as `makeSession`'s `liveness`, the
heartbeat half of the same question. The policy reaches **every** ssh the dial
spawns: the arch probe, the warm validity check, the GC-root pin, the closure
ship, the `NIX_SSHOPTS` that the remote-store `nix build` forks its own ssh with,
and the agent command itself.
