...
`serveHostMap` takes `{ linkFor, failureOf }`; `linkFor(host, session)` is built
...
`connection` from the **same** `SessionState` frame in one projection (its injected
`connectionOf`), so a "dot connected, word connecting" split has no construction path.
...
| Export | Subpath | Role |
| --- | --- | --- |
| `projectConnection(s)` | `/connection` | the pure, browser-safe leaf a consumer applies to a `SessionState` frame → `ConnectionInfo` (a provable identity: `ConnectionInfo` **is** `SessionState<SshProv>`). The one named projection both the server (`serveHostMap`'s `connectionOf`) and a client use — never a second server-side subscription |
