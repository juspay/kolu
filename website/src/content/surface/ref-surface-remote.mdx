...
and surfaces the failure on the session's diagnostic `onLog(line, severity)` sink —
classified `error` for a genuine failure (the wedged-clock deadline, a transport fault) and
`info` for the expected-absent case (an agent dial whose client carries no reserved
`system.clockNow`). `onLog`'s `severity` is optional and defaults to `info`, so a one-argument
sink stays valid. `disconnected` adds `error` +
