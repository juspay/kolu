...
`keepalive?: { intervalS, countMax }` is the dial's ssh **dead-peer policy** —
...
killed and restarted (`{ intervalS: 30, countMax: 10 }` buys five minutes). The
cost is symmetric: a genuinely dead host keeps that dial parked for the same
window before the loop retries. Out-of-range values throw at `sshConnector(...)`
construction rather than being clamped (`assertSshKeepalive`, ceiling
`MAX_SSH_KEEPALIVE_TOLERANCE_S` = one hour). The policy reaches **every** ssh the
dial spawns: the arch probe, the cache prefetch, the warm validity check, the
GC-root pin, the closure ship, the `NIX_SSHOPTS` that the remote-store `nix
build` forks its own ssh with, and the agent command itself. `dialAgentOnce`
takes the same option and forwards it.
...
<Aside type="caution" title="Raising this alone does not protect a CONNECTED link">
  ...
  **both**, and the heartbeat's own ceilings bound what is reachable there
  ...
  ```ts
  makeSession({
    connectOnce: sshConnector({ …, keepalive: { intervalS: 30, countMax: 10 } }),
