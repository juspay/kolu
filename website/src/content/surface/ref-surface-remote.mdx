...
- **State progression.** `probing → provisioning → connecting → connected`; a warm hit skips `provisioning`. On drop, `disconnected → probing → …` at exponential backoff capped at 60s.
