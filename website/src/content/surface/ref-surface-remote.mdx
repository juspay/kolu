...
- **`currentState()` is the synchronous twin of `onState`.** It returns the current published `SessionState` frame — the point-read companion to the snapshot-then-delta subscription, reading the same cell `onState` publishes. It always returns the *freshest* frame: `onState` delivery is microtask-deferred and one synchronous frame can drive two transitions (e.g. `disconnected` → give-up `failed`), so a listener delivered one frame may read a *later* one here. Honest liveness is `currentState().phase === "connected"`.
