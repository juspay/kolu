...
| `dialAgentOnce<C>(opts)` | `.` | one-shot CLI dial: validate the baked source-flake ref, probe the host system, resolve only `packages.<system>.<package ?? binary>.drvPath`, pin, probe, and return `{ client, dispose }` with the link already proven live. `package` defaults to `binary`; set it when the closure a host receives is deliberately larger than the one daemon exec'd from it (kolu ships `padi-agent`, runs `padi`), and keep every dial path to a given host naming the same attr — two paths naming different attrs give one host two different closures |
