...
terminal `failed` state that yields `null` fails loud (`UnclassifiedHostSessionError` on the
unknown-host race). There is no `"other"` fallback cause. Together they are the server half
of a host map — see
