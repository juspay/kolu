...
| `LogEntry` / `LogEntrySchema` | `/connection` | one `{ source, line }` log-tail entry — a type alias of `@kolu/surface-map`'s `EvidenceLine` (the same concept, so the same type), with the zod validator annotated `ZodType<LogEntry>` |
