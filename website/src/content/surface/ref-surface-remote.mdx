...
through** — the drop ends the downstream stream with the **named retryable
transport end** `RelayTransportLostError` (an `ORPCError` carrying the
`SURFACE_RELAY_TRANSPORT_LOST` code, the one code the shared retry fence retries)
so the client re-subscribes end-to-end and a snapshot only ever leads a fresh
stream. A raw re-throw would cross oRPC sanitized to a non-retriable error and
strand the downstream — this named end is what makes fail-through actually
recover. Holding open a byte stream is a compile error.
