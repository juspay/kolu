...
| `SURFACE_AGENT_FLAKE_REF_ENV` | `.` | canonical name of the wrapper-baked exact agent source; `dialAgentOnce` validates its value eagerly, while long-lived consumers validate it at their own typed failure boundary |
| `resolveAgentDrv(host, flakeRef, packageName, { signal, onProgress })` | `.` | probe the host system and lazily resolve one package as `{ kind: "flake-installable", drvPath, installable }` inside the owning dial's cancellation/progress path; the installable lets provisioning evaluate, temporarily root, and copy in one Nix operation, so local GC cannot break the handoff |
...
`localEnv`. Its `agentFlakeRef` is the exact, minimal source flake baked by the
Nix wrapper; the package name is the same `binary` value, so the executable and
derivation cannot drift into two hand-maintained mappings.
