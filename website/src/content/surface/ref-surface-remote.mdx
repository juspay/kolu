...
<Aside type="caution" title="This bounds ssh transport death — it is not a survival budget">
  ...
  | 2 | **`makeSession`'s heartbeat watchdog** — the `system.live` round-trip | a `connected` link | ≈25s at its defaults | Yes, `MakeSessionOptions.liveness` — but per (1) it *never gets a vote*, because the lower deadline always wins |
  | 3 | **The provisioning progress-liveness budget** — `PROVISION_STEP_SILENCE_BASE_MS`, which group-kills the child | `provisioning` (the build/copy) | **120s without child output**, doubling per expiry, `PROVISION_STEP_MAX_EXPIRIES` = 4 | Not by this option. ssh keepalives are protocol traffic and produce **no child stdout**, so they never reset it |
  ...
  multi-minute blip — the link is gone in 5–10s — and raising it past 120s is
  inert for a provisioning build, whose child bound is (3). What it prevents is
  the opposite failure: a 30s verdict tearing down a dial whose peer was merely
  slow to answer a probe, and an unbounded park on a transport that is genuinely
  gone. `MAX_SSH_KEEPALIVE_TOLERANCE_S` (one hour) is therefore a ceiling on *how
  long a dead ssh may go unnoticed*, not a promise that anything survives that
  long.
</Aside>
