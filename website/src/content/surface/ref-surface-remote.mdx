...
sum, keyed on `phase`: the up arms (`copying` / `building` — the ssh connector's
provisioning phases — plus `connecting` / `connected`) carry only a provenance-tagged
`log` tail (`{ source: "local" | "remote"; line }[]`); `disconnected` adds
`error` + `cause` (`"network"` unreachable / `"remote"` refused); `failed` is
terminal with `cause` pinned to the `"remote"` literal (a network fault never gives
up, so `failed` + `network` is unrepresentable). There is no separate
`progressLines` / `remoteProgressLines` — provenance is the `source` field.
