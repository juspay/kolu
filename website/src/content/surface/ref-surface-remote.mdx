...
`connected` state, which `serveHostMap` reads directly (offset-at-hello — the entry reads
`connecting` until the first probe stamps it).
...
once the probe lands — `null` means *not yet measured*, so `serveHostMap` reads it as still
`connecting` (a `0` placeholder would be a lie on a skewed host). This is where the
`clockOffset` a keyed `SurfaceMap` folds into `EntryStatus.connected` originates — it rides
the session's own state, no longer an injected `offsetOf`. `disconnected` adds `error` +
