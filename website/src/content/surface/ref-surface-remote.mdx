...
- **State progression.** `probing → provisioning → connecting → connected`; a resolver-cache hit followed by a target warm hit skips `provisioning`. An uncached exact-source evaluation enters `provisioning` even if the later target check is warm, because that evaluation has the same minutes-long lifetime contract as a cold provision. On drop, `disconnected → probing → …` at exponential backoff capped at 60s.
