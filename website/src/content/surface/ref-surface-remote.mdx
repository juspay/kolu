...
| `AgentDerivation` | `.` | closed provisioning-source union: `{ kind: "drv-path", drvPath }` when the caller owns a direct store path, or `{ kind: "flake-installable", drvPath, installable }` when Nix must recreate and root an evaluated derivation through copy/build |
...
`{ kind: "drv-path", drvPath }`; `resolveAgentDrv` returns the flake-backed arm
