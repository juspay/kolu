...
`dispose()` tears down only its own session; it is what `kaval-tui --host` and
`pulam-tui --host` use — and it carries the same required `localEnv`.
...
<Aside type="caution" title="localEnv is REQUIRED — the localhost arm never inherits ambient env">
  ...
  `dialAgentOnce` both) — omitting it is a **compile** error, not a review catch — so a
  locally-hosted agent can never silently inherit the caller's `process.env` (identity
  vars, secrets, a parent agent's session markers). surface-remote is **policy-free**:
