...
- **A session never holds the process alive by itself.** Its internal timers (reconnect backoff, connect watchdog, clock probes) are `unref()`'d, so a pinned-but-abandoned session cannot keep an otherwise-finished process running — the process exits and the session dies with it. A consumer whose sole purpose is waiting on a session must hold the process by its own means (a server socket, stdin, a live transport; every live dial's child/socket is itself a hold). Consequently **a parked `ClientCursor.next()` / onState-derived wait is *not* a process hold**: across a reconnect gap it is settled only by the backoff firing, and if nothing else holds the loop it goes silently unsettled as the process exits. The one deliberately ref'd timer is the admit-handshake timeout — it must fire even as the last handle standing, because it rejects a `pin()` a caller is awaiting.
