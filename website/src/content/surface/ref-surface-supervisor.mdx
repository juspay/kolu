...
| `probeDaemonIdentity({ capability, drainCeilingMs? })(socketPath)` | endpoint probe for the frozen fragment; full `ConvergenceProbe`, `null` only for honest `ECONNREFUSED`/`ENOENT`, all other dial/hello failures—including a control-core version other than frozen `1.0`, and an undecodable first frame (`UnspeakableProtocolError`)—throw |
...
| `UnconvergedCause` | `budget-exhausted \| drain-not-taken \| adopt-bind-failed \| identity-unverifiable \| probe-failed \| unspeakable-protocol` — evidence as data under `unconverged` |
| `UnspeakableProtocolError` · `isUnspeakableProtocolError(err)` | the TRANSPORT fact: the peer's FIRST frame could not be decoded. Raised by `probeDaemonIdentity`'s dial at the decode itself — never by the hello deadline, a close, or a member error. Carries `socketPath` and a bounded, JSON-quoted `frame` excerpt. Uncorroborated it is an ordinary `probe-failed`. Brand-checked, realm-robust |
| `UnspeakablePeerError` · `isUnspeakablePeerError(err)` | the CORROBORATED verdict — the same fact plus the two attestations the endpoint adds: the gate at this rendezvous is OURS and its `pid` passed the holder identity law. Only this becomes the `unspeakable-protocol` observation |
...
window may catch only that structured route-missing failure and project it as an
...
Effect RPC failure it was replaced by.)

### The third observation — `unspeakable-protocol`

A daemon from a **previous protocol epoch** cannot be asked its version at all:
version negotiation happens inside the protocol that was replaced, so its first
frame is undecodable. Convergence therefore has a third narrowly-typed peer
observation beside "identity" and "absent", and it is raised under exactly two
attestations — **an explicit first-frame decode failure** from a peer whose
**gate file is ours** and whose **pid we verified**. Nothing else earns it:
an uncorroborated decode failure stays `probe-failed`, and a foreign
socket-squatter keeps the untouched `SocketSquatterForeignError` path, so the
observation can never put a SIGTERM near a process we have not proven is ours.

Classification happens **at the decode**, not by waiting out the frozen hello's
30 s deadline — which is also what keeps `awaitHelloGone` responsive inside a
drain ceiling when the peer will never answer.

The disposition is the **contract-skew policy**, because an undecodable wire is
the limit case of a contract skew (the one whose version cannot be read):

| policy | disposition |
| --- | --- |
| `recycle` (kaval) | SIGTERM the verified gate holder and spawn fresh — its children do not survive a wire break, as at any contract skew |
| `refuse` · `drain-newer-else-refuse` (padi) | **refuse**: a drain verb on an unspeakable wire is unreachable, so the ordered policy degenerates to its other arm. The survivor is left standing + degraded, never killed, and the `unconverged` anomaly carries the `unspeakable-protocol` cause plus an operator-facing `detail` that says to stop that daemon out of band |
