...
flows through one internal fold (`foldObserved`). That fold always runs
`decide(policy, identity)` (never a hand-copied compat/match subset), owns
connection lifecycle so non-adopt returns release any held connection, preserves
three-valued observations (`identity | absent | failed`), and reports
`unconverged.running: null` when the running identity is honestly unknown (never
fabricates `expected`). A characterization probe throw is `probe-failed` with the
true message — never catch-to-null into `identity-unverifiable`.
...
| `BindResult` · `BoundResidentCharacterization` | private-bind result; adopted residents carry three-valued characterization (`characterized \| absent \| failed \| uncorrelated`) |
