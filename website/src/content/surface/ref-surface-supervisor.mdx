...
(the supervisor tears down from paths that cannot await). Two more —
`readProcessIdentity` and `readSocketHolders` — stay Promise-shaped because they
are satisfied by **`osfacts-client`**, a package outside this dependency graph;
the supervisor lifts each at its one call site.
