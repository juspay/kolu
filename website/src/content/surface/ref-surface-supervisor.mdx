...
| `osfactsSocketHolders(bin)` → `ReadSocketHolders` | the `readSocketHolders` inject, bound to an already-resolved osfacts binary path (`bakedOsFactsBin(<your env var>)` — this package never learns which var a consumer bakes). Calls `osfacts socket-holders <path> --procs`; the platform split, the parse, and the honesty policy live in that binary, not here. The identity source the gate-less-squatter recovery uses when the gate no longer names the holder. A leaf beside `waitForPidGone` — pid lifecycle via the OS |
| `SocketHolderReading` · `foldSocketHoldersReading(reading)` | the **three-way** answer, never one possibly-empty list: `{ kind: "holders", holders }` (at least one named `{ pid, command }`), `{ kind: "none" }` (**proven** nothing holds it — only linux's world-readable `/proc/net/unix` can prove this), `{ kind: "unattributed", detail }` (something may hold it and the OS would not say what — a bound-but-unreadable holder, or a search that could not complete). Only `none` is evidence of freedom, and the endpoint still re-probes before spawning. `foldSocketHoldersReading` is that fold over an `osfacts-client` `SocketHoldersReading`, exported for consumers that spawn the binary themselves |
