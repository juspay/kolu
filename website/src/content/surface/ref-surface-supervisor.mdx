...
| Export | Role |
| --- | --- |
| `createEndpoint({ home, readProcessIdentity, policy, probe, connect, … })` | returns an **unforgeable** public `Endpoint` (boot trio lives in a package WeakMap, not on the object); `home` is `DaemonHomePaths`; `readProcessIdentity` may be async so osfacts does not block the serving loop (prefer `processIdentityFromEnvAsync`) |
