...
`createEndpoint({ home, policy, probe, connect: (socketPath) => …, … })` is the
...
`home` (the same spine primitive `daemonMain` takes); `connect` receives the
...
| Export | Role |
| --- | --- |
| `createEndpoint({ home, policy, probe, connect, … })` | returns an **unforgeable** public `Endpoint` (boot trio lives in a package WeakMap, not on the object); `home` is `DaemonHomePaths` |
