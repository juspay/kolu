...
| Export | Role |
| --- | --- |
| `createEndpoint({ home, readProcessIdentity, policy, probe, connect, … })` | returns an **unforgeable** public `Endpoint` (boot trio lives in a package WeakMap, not on the object); `home` is `DaemonHomePaths`; `readProcessIdentity` is the same sync inject as the daemon half (`ReadProcessIdentity`) for two-field gate identity |
