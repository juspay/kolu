...
`{ keySchema, entry, contract, entriesSpec, codec, failureSchema }`; `entriesSpec` is the
membership collection's wire spec, always string-keyed.
...
- `resolve` returns `EntrySession` (`{ link, state }`) or `EntryFault` (`{ failure }`, a schema-valid domain value); `members()` and `has()` answer from one consistent snapshot. An unclassifiable failed state throws the named `UnclassifiedEntryFailureError` rather than falling back to a framework `"other"` cause.
