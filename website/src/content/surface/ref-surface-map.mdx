...
- **`failure.reason` is a human string**, never parsed for control flow; `kind` is the outer discriminant; `failure` is a sibling field on the same `failed` arm, not a second `kind`.
