...
- **`items(value)`** extracts **stable** ids from the cell value
  (`v => v.awaitingIds`). Stability is the app's one obligation — it is what makes
  "same item, not a new one" decidable.
...
- **`get(key)`** is a point read — `{ kind: "live" | "stale"; value }` — or
  **`undefined`** when the key is not a member OR has no frame yet (a mirror stays
  ...
  the host is up and connected; `stale` holds the last value under a down link so
  a chip DIMS rather than lies.
