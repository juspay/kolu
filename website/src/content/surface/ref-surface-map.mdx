...
- **`UnobservableEntry` — the client-only arm, minted by the liveness floor.** `{ kind: "unobservable"; membershipId: MembershipId; published: "warming" | "connected" }`. It means "our link to the publisher is dead", and it exists so that a consumer which **times** an entry (a deadline, an escalation, a "failed to start" verdict) cannot mistake our own outage for a real, self-healing campaign — a conflation that certified a healthy daemon dead in [kolu#2129](https://github.com/juspay/kolu/pull/2129). It is an arm rather than a flag on `warming` deliberately: a flag can be ignored, an arm is a compile error at every `.exhaustive()`. It carries **no** `clockOffset` and **no** `connection`, so nothing green is paintable and no frozen live word can keep narrating; `membershipId` rides through (the floor is about liveness, not identity). There is **no `failed` inhabitant of `published`** — a post-mortem does not go stale like a live claim, so `failed` passes the floor untouched. It has **no wire schema**: it is a projection of the consumer's own transport, so `entryStatusSchema` has no arm to encode it into and a floored value can never be republished.
...
<Aside type="caution" title="No status renders green over a dead transport">
  A server-published claim is only as trustworthy as your link to the publisher.
  With that link dead (`live === false`), both live arms floor to
  `{ kind: "unobservable", membershipId, published }` — applied to both
  `entry(key).state()` and the `entries` projection. `live` is a required argument
  so the fold cannot forget to floor.
</Aside>
