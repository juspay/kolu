...
| answer | means | is it evidence? |
| --- | --- | --- |
| `{"present":true,…}` | the item's current value | — |
| `{"present":false,"why":"absent"}` | membership says it is not a member | yes — the item is gone |
| `{"present":false,"why":"deadline"}` | the read ran out of time | **no** — a fact about the read, not the item |

Both are exit **0**, because in both the read genuinely happened. A caller
reaping on absence must act on `"absent"` alone: a quiet producer, or a
collection with no key set to resolve against, produces `"deadline"` while the
item is perfectly alive.
...
Effect.catchCause((cause) => {
  if (Cause.hasInterruptsOnly(cause)) return Effect.failCause(cause)  // Ctrl-C → 130
...
The **cause**, not the failure, and the interrupt passed through — both matter.
A *defect* is not a failure, so `Effect.catch` never sees one, and the runtime
then reports it itself through the default logger, which writes to **stdout**:
a log line in the middle of the data channel. That case is not exotic — the
server's own per-request refusal crosses the wire as a defect whenever the
[serving face](#two-gates-as-with-mcp) withholds a member this face's map
offers. Pair it with `runMain`'s `disableErrorReporting: true`, since the line
is already written and Effect's would be a second, differently-worded copy of
it.
