...
| Option | Meaning |
| --- | --- |
| `core?` | the unprefixed root — `{ surface, expose }`. Its verbs and readers sit at the TOP of the mounted tree. Its `expose` is the default-deny allowlist, the same [`ExposeMap`](/surface/ref-surface#per-face-expose-kolusurfaceexpose) every other face takes, checked against **its own** `spec` at build |
| `surfaces?` | the siblings, keyed — `{ [key]: { surface, expose, verbs?, annotate? } }`. Each is mounted **behind its key as the first argv word**, and each `expose` is checked against that sibling's own spec. There is no top-level `expose`: one map per surface |
| `verbs?` | hand-authored [`SurfaceVerb`](/surface/ref-surface#the-projection-vocabulary-kolusurfaceverbs)s at the BUNDLE ROOT, by name — the same record `serveSurfaceAsMcp` takes as its top-level `tools` |
| `endpoint` | the transport seam: `{ flags?, resolve }` — see below |
| `annotate?` | CLI-only ergonomics by verb name, for the core's and the bundle root's verbs: `{ positional?, render? }`. A sibling's live on the sibling |
...
This face takes the same **rooted bundle** the MCP face does, and composes it the
way argv already composes: a sibling's whole projection is mounted under a
subcommand named by its key.
...
`list` stays at the **top** and describes the whole bundle — one authoritative
"what can I address" per face. Each row's `name` is what a caller *types*, in
both arms: `outlines ops_run` for a verb, and the full address for a readable
member — `outlines get nodes`, plus a `keys` / `watch` row wherever the
collection declares them, since those are different addresses. The sibling is
repeated as a `surface` field and the member key as a `member` field, so a script
can group by either without re-splitting a string this face composed.

A sibling's own `verbs` are mounted behind its word like everything else it
contributes, so `outlines read` is here what `outlines_read` is on the MCP
face — one composition, spelled once per face in the separator that face has.
Only the bundle-root `verbs` sit bare at the top.

A **verb receives the client of the thing it is declared on**: a sibling's
`verbs` are handed `clients[key]`, and the bundle-root `verbs` are handed the
whole `{ core, clients }` bundle.

Four refusals, all at BUILD:

- a bundle with neither a core nor a sibling — that is not a bundle;
- a sibling that exposes nothing and declares no verbs — it would mount an argv word a user can only ever be refused at;
- a sibling key that would be mounted beside a reader command or a root verb of the same name, which the parser would answer with whichever it met first;
- everything the single-surface projection already refuses, per scope.

A dialled bundle that carries **no client** for a sibling is exit `3`, not a
usage error: the command line was right, and what is missing is on the far side —
the same arm a dead socket lands on.
...
`client` is a [`RootedSurfaceClients`](/surface/ref-surface#the-face-and-its-retry-fence-kolusurfaceclient)
bundle — `{ core?, clients? }`, each leg a
[`SurfaceClientCallable`](/surface/ref-surface#the-face-and-its-retry-fence-kolusurfaceclient),
...
a deadline, so a key that is not there answers instead of hanging on a
subscription that will never yield. Both bounds are always armed, and they answer
...
- **stdout is data.** One JSON value for a one-shot read; one **compact** JSON
  ...
  *The one exception is not this package's:* a refusal from the CLI **library**
  ...
  reason on stderr before `runEdge` maps it to exit 2. This face's own usage
  errors never do that — they are one line on stderr and an empty stdout — so a
  script that captures stdout should branch on the exit code before reading it.
...
- **A hung-up reader is a complete run.** `watch nodes | head -1` closes the
  ...
  so it exits 0. Every other write failure is reported. The predicate this arm
  decides on is exported as `isConsumerHangup` (below), for a host whose own
  writes sit outside this projection and answer the same question of a sink
  failure.
- An `annotate.render` renders a verb's output as text **for a human**, applied
  unless the caller passed `--json`. **The flag is the only thing that decides.**
  It used to be the descriptor — prose to a terminal, data through a pipe — so
  one command answered with two different things depending on what it happened to
  be attached to, and neither could be asked for on purpose.
...
[serving face](#two-gates-as-with-mcp) withholds a member this face's map
...
| `SurfaceCliOptions` · `SurfaceCliCore` · `SurfaceCliSibling` · `EndpointSeam` · `ResolvedEndpoint` · `SurfaceCliConnection` · `VerbAnnotation` · `InputProjection` · `Assembled` · `ProjectedCommand` · `RunEdgeReport` | types |

The framework's rules stay the framework's: `messageOf` is
[`@kolu/surface/errors`](/surface/ref-surface#the-tagged-error-vocabulary-kolusurfaceerrors)'s
and `SurfaceClientCallable` — the shape each leg of the bundle `resolve`'s `open`
returns has — is
[`@kolu/surface/client`](/surface/ref-surface#the-face-and-its-retry-fence-kolusurfaceclient)'s.
One concept, one import path; a face that published the framework's rules under
its own name would be a second place to look for them. The one exception is
`RootedSurfaceClients`, re-exported here for the same reason the MCP face
re-exports it: `SurfaceCliConnection` below carries it, and a host writing an
`endpoint.open` has to be able to import the name it names.
`SurfaceCliConnection` is
