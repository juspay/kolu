...
"what can I address" per face. Each row's `name` is what a caller *types*
(`outlines ops_run`), with the sibling repeated as a `surface` field so a script
can group by it without re-splitting a string.
