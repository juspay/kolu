...
The `failed` arm is a **two-field record**: the reason, and the output that evidences
it. Neither half is optional, because a card that names a cause without showing what
the machine actually printed is exactly the card nobody can act on. The pairing is also
...
<Aside type="note" title="The domain owns its failures; the framework owns the discriminant and one co-product">
  ...
  `PadiEntryFailure` is a validated `z.discriminatedUnion` over a structural `cause`
