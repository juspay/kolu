...
<Aside type="note" title="The domain owns its failures; the framework owns the discriminant">
  ...
  domain's failure taxonomy, so it carries the discriminant and nothing more; a domain
  instantiates `EntryStatus<PadiEntryFailure>` at its own map, where `PadiEntryFailure`
  is a validated `z.discriminatedUnion` over a structural `cause` (each arm carrying its
  human `reason`, the `contract-skew-refused` arm additionally a typed `running`/`expected`
  version pair). The `failed` arm carries a schema-valid domain value, not a loose object.
  Domain vocabulary stays in the domain — the dependency arrow points *out*.
</Aside>
