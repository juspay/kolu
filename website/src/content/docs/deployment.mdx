...
On a **headless** NixOS box, also set `users.users.<you>.linger = true` in the
system config so the user systemd — and this service — starts at boot without
a login.

## Ubuntu (and other non-NixOS Linux)

Same module, applied with standalone home-manager. Install Nix with flakes
first ([NixOS Asia guide](https://nixos.asia/en/install)), then a flake whose
`homeConfigurations` output imports the module:
...
Linger keeps the user systemd running after SSH disconnects — without it the
service dies when you log out, which is the opposite of a headless box. Then
open [http://127.0.0.1:7681](http://127.0.0.1:7681). Address, origins, and
logs below apply unchanged. The [From a Mac](/from-a-mac) tutorial walks this
as the first step of the Mac-window setup.
...
host trusts it — your ssh user is in the host's `trusted-users`, or the copied
binaries carry a signature the host trusts (the cache's, when this machine
downloaded them rather than building them). When it doesn't, kolu shows Nix's
refusal in the connect log and the host realises the agent itself; the fix is
`trusted-users` on that machine, or the same `nix.conf` block above applied
there so it downloads the agent instead of building it.
