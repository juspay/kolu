...
```sh
kolu watch --states waiting,awaiting --held-for 60s --nag 5m --nag-count 3 --ignore-self
```
...
it work or kills it — or, with `--nag-count`, until the count runs out. The
nagging is **finite** when you cap it: three reminders about a parked terminal,
then quiet. Left uncapped, it repeats forever.
...
| `--nag-count N` | cap the nagging: after the first report, at most N more reminders at the `--nag` interval, then quiet about that terminal. A state change re-arms it — a terminal that resumes and stops again is a new event, with a first report and N reminders of its own. Refused without `--nag`: a cap on a repetition that never starts is nothing |
