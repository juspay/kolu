...
Six flags on `kolu watch` turn it into the alert that doesn't let go:
...
| `--ignore-self` | mute the terminal this process is running inside (`$KAVAL_TERMINAL_ID`). Refused if you are not in a kolu-owned PTY, and refused with `--host`, whose fleet has never heard of this terminal — pass `--ignore <id>` rather than guessing |
...
A heartbeat is the one line with no terminal on it — that is how you skip it.
...
`--ignore` / `--ignore-self` instead — a mute fails open, an id list fails
closed. (A single id still works, as a debugging tail — but tailing the one
id you also muted is refused: that watch could never match.)
...
reads either feed. `--heartbeat` adds a seventh, `{"kind":"heartbeat","at":<epoch
ms>}`, and it is the one kind a consumer is meant to skip — it carries no
terminal. The same knobs are [MCP tool parameters](/mcp) for a coding agent
supervising a fleet, except `--heartbeat`: over MCP, `watch_next` answers "is
anything there?" with its own `timeoutMs`, so a clock tick would only mix
