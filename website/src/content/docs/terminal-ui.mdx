...
| `--ignore <id>` | mute this terminal (repeatable). Fail-open: a stale or unknown id costs nothing, and every new terminal is still watched |
| `--heartbeat` | emit a timestamped alive line every interval so a quiet pipe means *nothing is matching*, not *the stream is dead*. Omit to stay silent when nothing is happening |
...
closed. (A single id still works, as a debugging tail.)
