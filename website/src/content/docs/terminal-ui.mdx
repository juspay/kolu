...
| `--held-for` | report a state only once it has *held* that long — `60s`, `5m`, `2h`, `500ms`. An agent handed more work inside the window is never reported at all |
...
timing. The same three knobs are [MCP tool parameters](/mcp) for a coding agent
supervising a fleet.
