...
dials nothing, so a dead endpoint still lists. Beside the tools ride the
member readers `get` / `keys` over the same live views (`terminals`,
`urgency`, …), each with a `--follow` form that streams the answer as ndjson.
...
a rejected flag — never reaches a face's handler, so the CLI library renders
the usage and `kolu` answers `1` on BOTH faces; `2` above is the exit the
face's own input validation writes when your spelling DID reach the daemon.
...
The same knobs are [MCP tool parameters](/mcp) for a coding agent supervising a
