...
description: "How kolu reads the live state of Claude Code, Codex, Grok, OpenCode, Pi, and Oh My Pi from each terminal — the shared state vocabulary shown on the dock and tile, and what each agent can and can't report."
order: 80
section: Agents
koluHero:
  eyebrow: awareness
...
[Pi](https://github.com/earendil-works/pi-mono), or
...
- **A redirected store, on macOS only** — a `PI_CODING_AGENT_DIR` (or
  ...
  redacts even same-user process environments since macOS 10.13. `--profile`
  flags and `--session-dir` still resolve, since those live in argv.
...
Unlike pi, an unresolvable directory is **not** replaced by a default: the
breadcrumb is the only anchor kolu has, so a run whose directory cannot be read
...
same answer. kolu's own default can be forced with `KOLU_OMP_DIR=<dir>` — the
escape hatch for tests and unusual setups.
