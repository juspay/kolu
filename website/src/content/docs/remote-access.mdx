...
<Aside type="caution" title="Remote terminals are alpha">
  Once you reach kolu, its **remote hosts** feature — putting another machine's
  terminals on your canvas over ssh — is an early preview. See
  [Remote Hosts](/remote-hosts) for how to enable it, add hosts, and the
  ssh-session limitations (the macOS Keychain, `ssh-agent`, the clipboard).
</Aside>
...
<CardGrid>
  ...
  <LinkCard title="Remote Hosts" href="/remote-hosts" description="put another machine's terminals on your canvas over ssh — enable, add hosts, switch, and the limitations" />
</CardGrid>
