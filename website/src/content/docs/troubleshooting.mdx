...
`ask_user_question` prompts). Claude, Codex, and OpenCode can also export the
session as a self-contained HTML chat log or full transcript — as can Grok
(from `chat_history.jsonl`). Other agents still run in tiles; they just do not
light up the dock.
