...
`grok` (Grok Build), and `pi`. For these, kolu reads live agent state into the dock
(thinking · tools · waiting · **awaiting you** — where the agent's own store
carries a signal for it, e.g. Grok's `ask_user_question` prompts; pi's
gates live in its TUI, so its dock row shows the other three). All five can also
export the session as a self-contained HTML chat log or full transcript. Other agents
