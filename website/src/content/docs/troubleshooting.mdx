...
`grok` (Grok Build), `pi`, and `omp` (Oh My Pi). For these, kolu reads live
agent state into the dock (thinking · tools · waiting · **awaiting you** —
...
reads off the rendered screen; pi's gates live in its TUI, so its dock row shows
the other three). All six can also export the session as a self-contained HTML
chat log or full transcript. Other agents still run in tiles; they just do not
light up the dock.
