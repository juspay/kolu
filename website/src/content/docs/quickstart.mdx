...
`kolu mcp` serves this host's terminals to a **coding agent** over MCP
(stdio) — a pure padi client, no web server. Register it with your agent and
it can create terminals, type into them (text and Enter as separate sends,
the way agent-driving protocols expect), wait for output to settle or an
agent state to land, read the screen, and watch which terminals need you:

```sh
# Claude Code
claude mcp add kolu -- nix --refresh run github:juspay/kolu -- mcp
# a padi on another machine, over ssh
claude mcp add kolu-zest -- nix --refresh run github:juspay/kolu -- mcp --host user@zest
```

The full wiring options and the tool/resource reference are on the [padi
page](/padi#the-agent-face--kolu-mcp) — `kolu mcp` is the agent face of the same
surface `padi-tui` reads.
