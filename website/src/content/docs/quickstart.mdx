...
system/org.nixos.nix-daemon` on macOS). The same applies to any machine you
[add as a remote host](/remote-hosts): kolu realises its agent there with
that host's own Nix, whose `nix.conf` alone decides where binaries may come
from — so put the cache in the remote machine's system config too, exactly as
above.
