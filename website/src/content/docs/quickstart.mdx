...
through kolu: provisioning fetches the agent's binaries into **this** machine's
store and copies them to the host, cross-architecture included. That copy
lands only if the host trusts it — your ssh user is in the host's
`trusted-users`, or the host trusts the cache's signing key. When it doesn't,
kolu says so in the connect log and the host builds the agent itself; the fix
is the same `nix.conf` block above, applied on that machine.
