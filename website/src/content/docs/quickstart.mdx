...
system/org.nixos.nix-daemon` on macOS). The same setting benefits any machine
you [add as a remote host](/remote-hosts): kolu builds its agent there with
that host's Nix, which faces the same trust rule.
