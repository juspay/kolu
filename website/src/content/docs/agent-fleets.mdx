...
<figure class="doc-shot">
  <img src="/shots/islands.webp" alt="A fleet of three agent terminals laid out across the canvas, each with its own repo group in the dock." loading="lazy" />
  <figcaption>Three agents, three repos, one canvas — the dock keeps the roster.</figcaption>
</figure>
...
id=$(kolu create --toplevel --worktree fix-parser -- claude --dangerously-skip-permissions)
...
<CardGrid>
  ...
  <LinkCard title="Add another machine" href="/remote-hosts" description="run the fleet across machines — the same loop over ssh, one tab per host" />
