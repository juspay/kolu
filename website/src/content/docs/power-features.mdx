...
| `#/t/<host>/<id>/code?path=<p>&line=<n>` | the Code tab, `<p>` open at line `<n>` |
...
<Aside type="note" title="Focus the installed app, don't spawn a second window">
  When kolu is installed as a [PWA](/install-pwa), an in-scope link focuses the
  already-open app window (via the manifest's `launch_handler`) instead of
  opening a duplicate — so a link from your chat or board jumps you straight to
  the terminal, in the window you already have.
</Aside>
