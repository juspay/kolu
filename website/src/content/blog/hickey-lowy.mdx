...
decisions that will be revisited, fast-moving and slow-moving
parts coupled when they should have stayed apart. Two different defects. Two different
...
other is blind to. A review that ran only one of them would have
signed off on code the other lens would have stopped.
...
review** has become that. And structural review is exactly what two orthogonal
...
Hickey's lens reads the code as a snapshot — a space-like
slice: what's tangled right now? Löwy's lens reads the same
code as a world-line — the time-like reading: what will pull
apart, and when? They are measuring different axes. Each is
blind to what lives only in the other.
...
Mis-scoped volatilities eventually become complected code. So the
complexity Löwy warns about starts on the time axis — _diversity
across customers and points in time_ — and surfaces on the space
axis, as tangle. Still, the two usually arrive out of phase. The spatial
...
One O(N) sweep over the set; it pushes only the ids whose
suffix actually changed, and the server republishes those —
including the other terminal that just started or stopped
colliding. `TerminalMetadata` carries
...
factoring is wrong — the way the code is carved into parts, the
real structure underneath both projections. Get it wrong and the
defect shows up in both at once: spatially (right now, in the propagation
chain) and temporally (in the mismatched clocks the fields were
bound to). Call it **binocular agreement**.
It's a particularly sharp signal when it happens. It's also the
minority. In PR #623, across three review passes, single-axis findings
outnumbered binocular ones several-to-one. The few binocular ones
that surfaced clustered in the later passes, after the revisions
went in. Most findings, including the two centerpieces of this
...
of issues — the ones that only emerge after the implementer
has made twenty design micro-choices by taste. Pre-implementation review is cheap; it
...
a _location_ only occasionally — and even when they do, they
rarely prescribe the same edit. Hickey wants you to decouple
...
right in a way only a third read can resolve — yours, after
theirs, synthesizing the two. The fix you ship is rarely either
agent's literal proposal.
...
PR #623 shipped seven refactor commits past where my taste
would have called it done and merged. Every one of them made the diff smaller. That's
