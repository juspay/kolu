...
<svg viewBox="0 0 680 300" role="img" aria-label="One surface spec, derived three ways. At the top, defineSurface declares the five primitives — cells, collections, streams, events, and procedures — each with its own Zod schema. From that single spec the framework derives three artifacts: surface.contract (the typed oRPC contract, built at compile time and shared by both ends), implementSurface (the server router plus a ctx the domain code writes through), and surfaceClient (the client .use() hooks). There is no second copy: the spec is the only source of truth for the schemas, the defaults, and the types." style="width:100%;max-width:680px;display:block;margin:1.75rem auto;">
  ...
  <line x1="122" y1="146" x2="122" y2="184" stroke="var(--color-kolu-primary)" stroke-width="1.2" />
  <polygon points="122,192 117,183 127,183" fill="var(--color-kolu-primary)" />
  ...
  <rect x="30" y="192" width="184" height="66" rx="6" fill="none" stroke="var(--color-kolu-primary)" stroke-width="1.2" />
...
It's internal-only, by the way. `@kolu/surface` ships inside Kolu, not as its own package, and the API is still moving. Don't reach for it from outside yet. If you're building on it from inside, the full framework docs live at [kolu.dev/surface](/surface) — [why surfaces exist](/surface/why-surfaces) and the five shapes, then [serving them](/surface/the-server-half), [running them as a daemon](/surface/the-daemon-spine), and [mirroring them across an ssh fleet](/surface/across-the-hosts).
...
[the docs](/surface) · [`@kolu/surface` source](https://github.com/juspay/kolu/tree/master/packages/surface) · [the runnable example](https://github.com/juspay/kolu/tree/master/packages/surface/example) · [Kolu's surface declaration](https://github.com/juspay/kolu/blob/master/packages/common/src/surface.ts)
