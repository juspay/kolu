...
unbundled/typo'd language fails the build loudly, and a fence added while
`astro dev` runs needs a dev-server restart (the list is derived when the
config evaluates). Unit pins live in `test/*.test.mjs`, run by
