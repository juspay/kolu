...
- **Socket-backed clients use `connectSurface` / `connectSurfaces`**: They wire the default-on heartbeat / half-open watchdog in, so `health().live` catches a silent half-open (laptop sleep / wifi roam). Hand-building `surfaceClient + websocketLink` (no heartbeat) is the documented escape hatch only — a direct/stdio link or a minimal example.
