...
- The coordinator's live status artifact is `orchestrator.html` in the coordinator's PWD — a COORDINATOR-LOCAL, git-untracked file (never staged: a careless `git add` once swept it into the docs branch — pathspec-stage around it), self-contained HTML with a 30s `<meta http-equiv="refresh">`, light/dark via `prefers-color-scheme`. It is updated IN THE SAME TURN as every board-changing event — a merge, a PR opening, a lane state change, a dispatch. A dashboard updated "when convenient" is stale by construction and the human notices before the coordinator does (the recorded failure). FORMAT (srid's ruling: "less words, more graph"): flows, not prose — each lane is one row of pill nodes joined by arrows (`note → gate → build → gauntlet → CI → PR`), state carried by color class (done green · running pulsing amber · waiting-on-human indigo · blocked red · queued dashed), detail lives in hover `title` attrs never in visible sentences; a merge-queue pill row; lineage rows for queued dependency chains; done work auto-archives to a compact shipped row the same turn it closes — the human reads the board to decide, not to commemorate.
