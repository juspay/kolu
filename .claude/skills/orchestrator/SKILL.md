...
- Give agents facts, never hypotheses or suspicions — fed bias voids an independent review. And a root-cause MECHANISM you have not reproduced is a hypothesis, not a fact: a brief states the observable SYMPTOM as its ground truth and labels every un-reproduced mechanism an explicit hypothesis to falsify — it never stamps one "verified" / "coordinator-verified." The false-certainty label is the worst bias of all, because it talks the implementer straight past its own falsify-the-inherited-diagnosis law (the very discipline /be §2 exists to enforce), so the lane builds against the phantom instead of grounding it — and the more forensic the brief reads, the harder that pull. The recorded failure: a brief opened "root cause coordinator-verified: provisionAgent's presence-based skip + a stale kaval gcroot → the renewed padi bakes a 5.0 kaval," three inferred mechanisms none of which was reproduced; ground truth at HEAD and on the boxes falsified all three, the real cause was a cross-supervisor livelock a layer down, and the lane spent days chasing the phantom (and nearly shipped a four-move fix against machinery that did not exist) before regrounding it. A refuted coordinator claim gets corrected at the source (the issue, the brief), not just conceded in chat.
