...
**Verify on the shipped artifact — never claim "verified"/"works"/"live" off a frame or a proxy.** A "works" handoff is a *promise to the user*, so it must be backed by watching the real thing do the real thing — not a green test, not a screenshot, not the dev/source path. Two traps that have shipped false "verified" claims to the user this run: **(1) Value-over-time / live UI** (a clock, counter, countdown, progress, any cadence tick) is **dynamic by job** — a single rendered frame or screenshot is structurally blind to a freeze, because a frozen clock and a live one are *pixel-identical in one frame*. Never claim "live"/"updates every second" without confirming **frame N+1 ≠ frame N** at the displayed cadence (this is the `/evidence` *"a value-over-time display is MOTION, watch it tick"* bar applied at §2, not only at §5 capture). **(2) Source ≠ shipped bundle.** Verify against the **built artifact the user actually runs** (`dist/`, the Nix-built binary, the prod bundle) — never a `bun run`/source/dev path: a dev preload, a `bunfig`/loader hook, or a `chdir` into the package can silently inject behavior (e.g. a reactive-build redirect) that masks a bug present *only* in the shipped bundle — a near-whole session was burned chasing a "frozen clock" that was the build's non-reactive output, hidden by the dev preload in every source-path harness. Corollary: when a harness says "pass" but the user *saw* it broken, the **harness is wrong** — the §2 "a repro that 'works' isn't a repro" rule applies to verification too: fix what you measure (run the real bundle, full-frame compare) before trusting the green.

**A green dev-shell `check` does not certify the Nix build.** Touching a package manifest (`package.json`, `pnpm-lock.yaml`, `bun.lock`) invalidates the Nix fixed-output-derivation hash that the prod/CI build rides (`pnpmDeps`/`fetchPnpmDeps` hash, a `bun.nix` FOD) — but `just check` runs in the dev shell over a plain `pnpm install`/`bun install`, **not** the FOD, so it stays green while every Nix-built consumer (the main app, sibling packages, CI) breaks with `ERR_PNPM_NO_OFFLINE_TARBALL`. After any manifest change, **regenerate the FOD hash and confirm a Nix build of an affected target is green** before calling it done — don't let a green dev-shell `check` stand in for it. (The hash-update mechanics live in the `nix-typescript` skill.)
